  ...
  values: [
    {
      v: 'ready',
      label: 'Ready',
      hint: 'phase "Ready" — the titlebar pill reads "Agent ready" with a green dot.',
    },
    {
      v: 'starting',
      label: 'Starting',
      hint: 'phase "Bootstrap" — <span class="agent warn">, dot --warning-dot, text "Agent starting".',
    },
    {
      v: 'lost',
      label: 'Lost',
      hint: 'takeAgentConnectionLoss returned a message: the .stopwrap full-window stop covers everything below the titlebar. Retry sets this back to ready. The pill itself still reads "Agent ready" — the phase is a separate fact.',
    },
  ],
});
  ...
  values: [
    {
      v: 'ok',
      label: 'Reachable',
      hint: 'server.state "ok", lease { fresh, 12 d } — the world FIXTURE = applyLease(RAW, "fresh") starts in.',
    },
    {
      v: 'lapsed',
      label: 'Check-in expired',
      hint: 'applyLease(world, "lapsed"): server.state "lease-lapsed", lease { lapsed, null }. Work and Engineering stop, the sidebar captions become "Connection error", All items drops to 10 rows behind a band, the Alerts badge goes 2 → 3.',
    },
    {
      v: 'checkin-unavailable',
      label: 'Status unavailable',
      hint: 'server.state "lease-unavailable": FOKS has no signed check-in. Takeover "Server status unavailable", action Review server.',
    },
    {
      v: 'unavailable',
      label: 'Stores unlistable',
      hint: 'the catalog could not list Acme’s stores: world.unavailableStores = [acct:work, team:eng]. Store fact, not a server one — takeover "Store connection failed".',
    },
    {
      v: 'unprobed',
      label: 'Never checked',
      hint: 'server.state "never-probed": takeover "Server not checked yet", action Review server.',
    },
    {
      v: 'blocked',
      label: 'Access blocked',
      hint: 'server.state "blocked": a protocol safety check refused the server. Takeover "Server access blocked", action Review server.',
    },
  ],
});
...
M.app({
  ...
  values: [
    { v: 'off', label: 'Unlocked', hint: 'locked:false — the ordinary shell.' },
    {
      v: 'locked',
      label: 'Locked',
      hint: '.app-lock dialog "Unlock FOKS"; the mock bridge’s mechanism is "password", so the copy says "your operating-system password".',
...
M.app({
  ...
  values: [
    { v: 'ok', label: 'Loaded', hint: 'the world loaded — VaultShell.' },
    {
      v: 'loading',
      label: 'Loading',
      hint: '<div class="app-loading">Connecting to the local agent…</div>.',
    ...
    },
    {
      v: 'error',
      label: 'Load error',
      hint: '.app-lock alertdialog "Couldn’t load FOKS" (curly apostrophe) with the normalized error message and Retry.',
  ...
  values: [
    {
      v: 'none',
      label: 'None',
      hint: 'the mock bridge supplies none: appInfo is { version 0.3.0, agentSocket /private/foks/agent.sock }.',
    },
    {
      v: 'local',
      label: 'Local',
      hint: 'a launcher-prepared, already authenticated profile: the shorter (3-step) first run and an extra Settings › About line.',
    },
  ],
});
  ...
  values: [
    {
      v: 'no',
      label: 'Idle',
      hint: 'aria-label "Refresh", title "Refresh vaults and groups".',
    },
    {
      v: 'yes',
      label: 'In flight',
      hint: 'disabled; both aria-label and title become "Refreshing vaults and groups".',
    },
  ],
});
...
D.deviceIdFor = function (idHex) {
  return D.DEVICE_ID_PREFIX + String(idHex).slice(2);
};
D.syntheticDevice = {
  name: 'MacBook Pro',
  role: 'owner',
  current: true,
  id: '04' + new Array(65).join('8'),
};
...
D.kindLabel = function (kind) {
  return D.KINDS[kind].label;
};
...
D.kindOf = function (item) {
  ...
  return /^password:/m.test(item.value || '') ||
    item.path.indexOf('/logins/') === 0
    ? 'Password'
    : 'Resource';
};
...
D.parseRole = function (wire) {
  ...
  if (wire && typeof wire !== 'string') {
    ...
    if (!k) return null;
    return k === 'member'
      ? { kind: k, visibility: wire.visibility == null ? 0 : wire.visibility }
      : { kind: k };
  }
...
D.visibilityOf = function (role) {
  return role.visibility == null ? 0 : role.visibility;
};
...
/* --- readers.ts --- */
D.storeOf = function (ref, w) {
  return byId((w || D).stores, ref);
};
D.partiesOf = function (ref) {
  return D.parties[ref] || [];
};
...
D.peopleLabel = function (count) {
  return count + ' ' + (count === 1 ? 'person' : 'people');
};
D.peopleGroups = function (parties) {
  var groups = parties.filter(function (p) {
    return p.party_kind !== 'user';
  }).length;
...
D.readersOf = function (item) {
  ...
  return D.partiesOf(store.id).filter(function (p) {
    return (
      D.admissionActive(p, store.id) && D.admits(p.destination_role, item.read)
    );
  });
};
...
D.readableBy = function (item) {
  ...
  return {
    label: D.peopleLabel(readers.length),
    title: readers.map(D.partyName).join(', '),
  };
};
D.actionableGroupMember = function (party) {
  return !!(
    ...
    party.label !== 'you' &&
    D.partiesOf(party.store).filter(function (c) {
      return c.username === party.username;
    }).length === 1
  );
};
  ...
  return stores
    ...
    .concat(
      stores.filter(function (s) {
        return s.kind === 'team' && s.team_kind === 'named';
      }),
    )
    .concat(
      stores.filter(function (s) {
        return s.kind === 'team' && s.team_kind === 'adhoc';
      }),
    );
};
...
D.itemsIn = function (ref, items) {
  return (items || D.items).filter(function (i) {
    return i.store === ref;
  });
};
D.itemAt = function (ref, path) {
  return D.items.filter(function (i) {
    return i.store === ref && i.path === path;
  })[0];
};
D.itemKey = function (item) {
...
var ACME = {
  ...
  unavailable: {
    server: 'ok',
    lease: { state: 'fresh', expires_in: '12 d' },
    catalog: true,
  },
  ...
  switch (state) {
    case 'blocked':
      return {
        ...
        detail:
          'A security verification failed for ' +
          serverName +
          '. ' +
          subject +
          '.',
        action: 'Review server',
        actionKind: 'review-server',
      };
    case 'lease-unavailable':
      return {
        title: 'Server status unavailable',
        detail:
          'Cannot verify server status for ' +
          serverName +
          '. ' +
          subject +
          '.',
...
D.world = function (s) {
  ...
  var unavailable = acme.catalog
    ? ['acct:work', 'team:eng']
    : D.unavailableStores.slice();
  ...
  var stores = D.stores.map(function (store) {
    ...
    else if (srv && srv.state === 'lease-unavailable')
    ...
    var out = {
      ...
      heading:
        state === 'normal' && !rosterFailed && !fedFailed ? description : '',
      failure: rosterFailed
        ? fedFailed
          ? 'both'
          : 'roster'
        : fedFailed
          ...
          : null,
      readable:
        unavailable.indexOf(store.id) < 0 &&
        !!srv &&
        srv.state === 'ok' &&
  ...
  stores.forEach(function (store) {
    ...
    if (!buckets[key]) {
      buckets[key] = { state: store.state, stores: [] };
      order.push(key);
    }
  ...
  var bands = order.map(function (key) {
    ...
    var names = bucket.stores.map(function (st) {
      return st.name;
    });
    var joined =
      names.length < 2
    ...
    var text =
      bucket.state === 'blocked'
        ? joined +
          ' ' +
          verb +
          ' unavailable because security verification failed for ' +
          serverName +
          '.'
        : bucket.state === 'lease-unavailable'
          ? joined +
            ' ' +
            verb +
            ' unavailable because server status could not be verified for ' +
            serverName +
            '.'
          : bucket.state === 'lease-lapsed'
            ? joined +
              ' ' +
              verb +
              ' unavailable because the connection to ' +
              serverName +
              ' expired.'
            : bucket.state === 'catalog-unavailable'
              ? joined +
                ' ' +
                verb +
                ' unavailable because the latest item list could not be loaded.'
              : joined +
                ' ' +
                verb +
                ' unavailable because ' +
  ...
  var items = D.items.filter(function (item) {
    return (
      item.kind !== 'Folder' &&
      storeById[item.store] &&
      storeById[item.store].readable
    );
  });

  var leaseState =
    acme.server === 'lease-lapsed'
      ? 'lapsed'
      : acme.server === 'lease-unavailable'
        ? 'unavailable'
        : 'fresh';
  ...
  var STOPPED_NOTE = {
    'lease-unavailable': {
      detail:
        'The agent has no signed check-in for this server. Its stores are hidden until it gets one.',
      action: 'Inspect',
    },
    blocked: {
      detail:
        'The server’s history no longer matches what this Mac pinned. Its stores are hidden. Open the server to see the reported error.',
      action: 'Inspect',
    },
    'never-probed': {
      detail:
        'This server has not been checked. Its stores are hidden until it is checked and its identity pinned.',
      action: 'Check',
    },
  };
  var alerts = D.alerts.filter(function (n) {
    return n.id !== 'lease-acme' || leaseState === 'lapsed';
  });
  ...
  if (acme.catalog) {
    alerts = alerts.concat(
      unavailable.map(function (ref, i) {
        return {
          id: 'catalog-store-' + i,
          severity: 'warn',
          title: 'Could not list one store on acme',
          detail:
            (storeById[ref] ? storeById[ref].name : ref) +
            ' could not be listed: the server connection could not be established.',
          action: 'Retry',
        };
      }),
    );
  ...
  return {
    ...
    storeById: storeById,
    navOrder: nav,
    vaults: nav.filter(function (st) {
      return st.kind === 'account';
    }),
    groups: nav.filter(function (st) {
      return st.kind === 'team' && st.team_kind === 'named';
    }),
    shares: nav.filter(function (st) {
      return st.kind === 'team' && st.team_kind === 'adhoc';
    }),
    items: items,
    bands: bands,
    alerts: alerts,
...
D.captions = {
  fresh: {
    'acct:personal': 'foks.example.net',
    'acct:work': 'foks.acme-corp.com',
    'team:eng': '5 people · 1 group',
    'team:household': '2 people',
    'team:homelab': 'Setup incomplete',
  },
  lapsed: {
    'acct:personal': 'foks.example.net',
    'acct:work': 'Connection error',
    'team:eng': 'Connection error',
    'team:household': '2 people',
    'team:homelab': 'Setup incomplete',
  },
};
D.goldens = {
  ...
  readers: {
    'team:eng|/deploy/production-token': D.readersOf(
      D.itemAt('team:eng', '/deploy/production-token'),
    ).length,
    'team:eng|/deploy/staging-token': D.readersOf(
      D.itemAt('team:eng', '/deploy/staging-token'),
    ).length,
    'team:eng|/release/bundle.tar': D.readersOf(
      D.itemAt('team:eng', '/release/bundle.tar'),
    ).length,
    'team:household|/wifi/guest-password': D.readersOf(
      D.itemAt('team:household', '/wifi/guest-password'),
    ).length,
  },
  engineering: D.peopleGroups(
    D.partiesOf('team:eng'),
  ) /* "5 people · 1 group" */,
