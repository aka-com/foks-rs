...
(function () {
  ...
  function card(note) {
    ...
    return (
      ...
      UI.btn(esc(note.action), {
        ...
        attrs: live
          ? 'data-act="call" data-fn="alertsRetry" data-arg="' +
            esc(note.id) +
            '"'
          : undefined,
      }) +
  ...
  M.fns.alertsRetry = function () {
    M.hint(
      'onRefreshWorld() — the catalog load is retried. No toast; the card stays until a load answers differently.',
    );
  };
        ...
        values: [
          {
            v: 'no',
            label: 'Fixture',
            hint: 'notesNow(world) — 2 entries, or 3 while Acme’s check-in is lapsed.',
          },
          {
            v: 'yes',
            label: 'Empty',
            hint: 'no notes at all: <p class="hint">Nothing needs attention on this Mac.</p>',
          },
          {
            v: 'retry',
            label: '+ retryable',
            hint: 'one catalog-* note whose action is Retry, so its button is the only live one on the pane.',
          },
        ],
      },
    ],
