...
(function () {
  ...
  TEAMS.forEach(function (ref) {
    BASE.parties[ref] = D.partiesOf(ref).slice();
  });

  /* team:eng → 'eng', for compact `applied` tokens. */
  var SHORT = {
    'team:eng': 'eng',
    'team:household': 'household',
    'team:homelab': 'homelab',
    'team:platform': 'platform',
  };
  var LONG = {
    eng: 'team:eng',
    household: 'team:household',
    homelab: 'team:homelab',
  ...
  function createdStore(o) {
    var account =
      BASE.stores.filter(function (st) {
        return st.id === o.account;
      })[0] ||
      BASE.stores.filter(function (st) {
  ...
  function userParty(ref, username, role, visibility, idHex, generation) {
    var dest =
      role === 'Member'
        ? { role: 'Member', visibility: visibility }
        : { role: role };
    return {
      store: ref,
      username: username,
      party_kind: 'user',
      generation: generation == null ? 1 : generation,
      locally_manageable: true,
      party_id_hex: idHex,
      source_role: dest,
      destination_role: dest,
      name: username,
      initials: D.initials(username),
      hue: D.hue(username),
    };
  }
    ...
    if (op === 'add') {
      list.push(
        userParty(
          ...
          +bits[4] || 0,
          '01' +
            '3333333333333333333333333333333333333333333333333333333333333339'.slice(
              0,
              64,
            ),
        ),
      ...
      list.forEach(function (p, i) {
        ...
        next.destination_role =
          bits[3] === 'Member'
            ? { role: 'Member', visibility: +bits[4] || 0 }
            : { role: bits[3] };
        list[i] = next;
      });
    } else if (op === 'remove') {
      D.parties[ref] = list.filter(function (p) {
        return p.username !== bits[2];
      });
    } else if (op === 'admit') {
      var remote = D.stores.filter(function (st) {
        return st.alias === bits[2] && st.kind === 'team';
      })[0];
      if (!remote) return;
      var host = D.servers.filter(function (srv) {
        return srv.id === remote.server;
      })[0];
      var vis = +bits[3] || 0;
      var count =
        D.federation.filter(function (f) {
          return f.store === ref;
        }).length + 1;
      ...
      D.federation = D.federation.map(function (entry) {
        return entry.store === ref && entry.operation_id_hex === bits[2]
          ? Object.assign({}, entry, { active: true })
          : entry;
      });
    }
  }
  ...
  function syncWorld(s, ref) {
    ...
    D.stores = BASE.stores.map(function (st) {
      return st.id === 'team:homelab'
        ? Object.assign({}, st, { active: resumed })
        : st;
    });
    ...
    if (ref && failure) {
      if (failure === 'roster' || failure === 'both')
        D.groupDetailFailures.push(
          Object.assign({ store: ref }, FAILURES.roster),
        );
      if (failure === 'federation' || failure === 'both')
        D.groupDetailFailures.push(
          Object.assign({ store: ref }, FAILURES.federation),
        );
    }
  ...
  function restoreFixture() {
    ...
    D.stores = BASE.stores.map(function (st) {
      return st.id === 'team:homelab'
        ? Object.assign({}, st, { active: BASE.homelabActive })
        : st;
    });
  }
  ...
  function ensureCreated(o) {
    ...
    SHORT[store.id] = o.alias;
    LONG[o.alias] = store.id;
    ...
    PLATFORM_PAGES.forEach(function (pg) {
      pg.gref = store.id;
      pg.nav = store.id;
    });
    if (
      D.stores.some(function (st) {
  ...
  function sortRoster(list) {
    return list
      ...
      .sort(function (a, b) {
        var rank =
          D.roleRank(b.p.destination_role) - D.roleRank(a.p.destination_role);
        if (rank) return rank;
  ...
  function itemsOf(w, ref) {
    return w.items.filter(function (item) {
      return item.store === ref;
    });
  }
  function readsOf(w, party) {
    return itemsOf(w, party.store).filter(function (item) {
      ...
      return (
        !!readers &&
        readers.some(function (c) {
          return c.party_id_hex === party.party_id_hex;
        })
      );
    });
  }
  function canTarget(party) {
    return D.actionableGroupMember(party);
  }
  function partySubtitle(party) {
    if (party.party_kind !== 'user') {
      var host =
        party.team_name && party.team_name.indexOf(' @ ') >= 0
      ...
      return (
        ...
        (party.scoped_host_id_hex
          ? ' · host <code>' + esc(party.scoped_host_id_hex) + '</code>'
          : '')
      );
    }
    var machine = !!(party.note && party.note.indexOf('service account') >= 0);
    return (
      (machine ? 'machine' : 'person') + ' · generation ' + party.generation
    );
  }
  function roleChip(role, extra) {
    ...
    return (
      '<span class="rolecell">' +
      UI.chip(esc(roleName(role))) +
      (parsed && parsed.kind === 'member'
        ? '<small>visibility ' + D.visibilityOf(parsed) + '</small>'
        : '') +
  ...
  function canCreateInStore(w, ref) {
    ...
    return (
      D.partiesOf(ref).filter(function (p) {
        return (
          p.label === 'you' &&
          p.party_kind === 'user' &&
          p.locally_manageable &&
  ...
  function serverNav(store) {
    return (
      'data-act="go" data-page="srv-server" data-set=\'{"v.profile":"' +
      esc(store.server) +
      '"}\''
    );
  }
  ...
  function ghero(s, store) {
    ...
    return (
      ...
      (unavailable
        ? UI.btn(
            store.state === 'lease-lapsed' ? 'Open server' : 'Review server',
        ...
        : '<span class="menuwrap">' +
          UI.btn(M.icon('more'), {
            ...
            attrs:
              ...
              (open ? 'true' : 'false') +
              '"' +
              ' data-act="set" data-key="v.menu" data-val="' +
              (open ? '' : 'more') +
              '"',
          }) +
          '</span>') +
  ...
  function partyRow(w, store, party, selected, manageable) {
    ...
    var extras = [
      party.party_kind !== 'user'
        ? fmtRole(party.source_role) + ' at ' + shortName(party)
        : null,
    ...
    return (
      ...
      (selected ? 'true' : 'false') +
      '"' +
      ' data-act="call" data-fn="gTogglePanel" data-arg="' +
      esc(key) +
      '">' +
      '<span class="who2">' +
      ...
      (actionable
        ? '<button type="button" title="Change role or remove" aria-label="Change role or remove"' +
          ' data-act="call" data-fn="gOpenPanel" data-arg="' +
          esc(key) +
          '">' +
          M.icon('more') +
          '</button>'
  ...
  function peopleTab(s, w, store, selected, manageable) {
    ...
    if (failure) {
      inner = UI.notice({
        ...
        body: '<p>' + esc(failure.message) + '</p>',
        actions: failure.retryable
          ? UI.btn('Refresh', {
              attrs: 'data-act="toast" data-text="Group roster refreshed"',
            })
          : undefined,
      });
    } else if (inactive) {
      ...
      inner =
        '<div class="rhead"><h2>People &amp; groups</h2>' +
        '<span class="n">' +
        esc(D.peopleGroups(parties)) +
        '</span>' +
        (showAdd
          ? '<div class="right">' +
            UI.btn('Add a group', {
      ...
      inner += parties.length
        ? '<div class="rt"><div class="hdr"><span>Who</span><span>Role</span><span></span></div>' +
          ...
          '</div>'
        : '<div class="callout">' +
          '<span class="kico" style="background: var(--chip-bg); color: var(--muted);">' +
          M.icon('people') +
          '</span>' +
    ...
    return (
      '<div class="roster">' + situationBand(store, 'people') + inner + '</div>'
    );
  }

  function federationSection(s, w, store, manageable) {
    var entries = D.federation.filter(function (entry) {
      return entry.store === store.id;
    });
    var inactive = store.kind === 'team' && store.active === false;
    ...
    if (failure) {
      inner = UI.notice({
        ...
        actions: failure.retryable
          ? UI.btn('Refresh', {
              attrs: 'data-act="toast" data-text="Group federation refreshed"',
            })
          : undefined,
      });
    } else if (inactive) {
      ...
      inner =
        '<div class="rhead"><h2>Groups on other servers</h2>' +
        (showAdd
          ? '<div class="right">' +
            UI.btn('Add a group', {
              icon: 'plus',
      ...
      inner += entries.length
        ? '<div class="rt fed"><div class="hdr"><span>Group</span><span>Role here</span>' +
          ...
          entries
            .map(function (entry) {
              var party = parties.filter(function (c) {
                return c.party_id_hex === entry.remote_team_id_hex;
              })[0];
              var remoteName =
                (w.serverById[entry.remote_profile] || {}).name ||
                entry.remote_profile;
              var readable = party ? readsOf(w, party).length : 0;
              return (
                '<div class="prow"><span class="who2">' +
                (party
                  ? UI.avatar(party, { className: 'pav' })
                  : '<span class="pav team" style="background: var(--c-team);">' +
                    M.icon('people') +
                    '</span>') +
                '<span class="t"><b><span>' +
                esc(entry.remote_team_alias) +
                '</span></b>' +
                '<small>on ' +
                esc(remoteName) +
                ' · host <code>' +
                esc(entry.remote_host_id_hex) +
                '</code></small>' +
                '</span></span>' +
                roleChip(entry.destination, 'for every member') +
                '<span class="reads">' +
                UI.chip(entry.active ? 'Active' : 'Inactive', {
                  tone: entry.active ? 'ok' : 'warn',
                }) +
                '<small>' +
                (entry.active
                  ? esc(D.plural(readable, 'item')) + ' readable'
                  : 'members read nothing here') +
                '</small>' +
                '</span>' +
                '<span class="ver">' +
                esc(entry.operation_id_hex) +
                '</span>' +
                '<span class="cellact">' +
                (!entry.active && entry.operation_id_hex
                  ? UI.btn('Restore access', {
                      ...
                      disabled: !manageable,
                      attrs:
                        'data-act="call" data-fn="gRestore" data-arg="' +
                        esc(entry.operation_id_hex) +
                        '"',
                    })
                  : '') +
  ...
  function partyPanel(w, store, party, manageable) {
    ...
    readable.forEach(function (item) {
      readableIds[item.path] = true;
    });
    var machine =
      party.party_kind === 'user' &&
      !!(party.note && party.note.indexOf('service account') >= 0);
    var here = fmtRole(party.destination_role);
    var active = D.admissionActive(party, party.store);
    var kind = party.party_kind === 'user' ? 'person' : 'group';
    var pinned = party.scoped_host_id_hex
      ? w.servers.filter(function (srv) {
          return srv.host_id && srv.host_id === party.scoped_host_id_hex;
        })[0]
      : undefined;
    var body =
      '<div class="sec">Can read<span class="n">· ' +
      readable.length +
      ' of ' +
      items.length +
      '</span></div>';
    ...
    body += !active
      ...
      : readable.length < items.length
        ? '<p class="hint">Items you don\'t have permission to view are dimmed.</p>'
        : '';
    body +=
      '<div class="sec">Details</div><div class="meta">' +
      '<b>Role here</b><span>' +
      esc(here) +
      '</span>' +
      (party.party_kind !== 'user'
        ? '<b>At source</b><span>' +
          esc(fmtRole(party.source_role)) +
          ' in ' +
          esc(shortName(party)) +
          '</span>' +
          '<b>Host</b><code>' +
          esc(party.scoped_host_id_hex) +
          '</code>' +
          (pinned
            ? '<b>Pin</b><span>matches this Mac’s pin</span>'
            : party.scoped_host_id_hex
              ? '<b>Pin</b><span>local pin fact unavailable</span>'
              : '')
        : '') +
      '<b>Generation</b><span>' +
      esc(party.generation) +
      '</span>' +
      '<b>Party id</b><code>' +
      esc(party.party_id_hex) +
      '</code>' +
      '<b>Managed</b><span>' +
      (party.locally_manageable ? 'from this server' : 'from its own group') +
      '</span>' +
      '</div>';
    if (machine) {
      body +=
        '<p class="hint">' +
        esc(party.note.charAt(0).toUpperCase() + party.note.slice(1)) +
        '.</p>';
    }
    var foot =
      manageable && canTarget(party)
        ? '<div class="dfoot">' +
          (demotionFor(party)
            ? UI.btn('Lower role…', {
                attrs:
                  'data-act="call" data-fn="gSheetTarget" data-arg="demote|' +
                  esc(shortName(party)) +
                  '"',
              })
            : '') +
          UI.btn('Remove…', {
            variant: 'danger',
            attrs:
              'data-act="call" data-fn="gSheetTarget" data-arg="remove|' +
              esc(shortName(party)) +
              '"',
          }) +
          '</div>'
        : '';
    return (
      '<aside class="details"><div class="dh">' +
      UI.avatar(party, { className: 'pav' }) +
      '<span class="t"><h2>' +
      esc(shortName(party)) +
      (party.label ? UI.chip('you', { tone: 'you' }) : '') +
  ...
  function removableOf(store, manageable) {
    ...
    return D.partiesOf(store.id)
      .map(function (party, index) {
        return { party: party, index: index };
      })
      .filter(function (row) {
        return canTarget(row.party);
      })
      .sort(function (l, r) {
        var rank =
          D.roleRank(l.party.destination_role) -
          D.roleRank(r.party.destination_role);
        if (rank) return rank;
        var lr = D.parseRole(l.party.destination_role);
        var rr = D.parseRole(r.party.destination_role);
        var vis =
          (lr && lr.kind === 'member' ? D.visibilityOf(lr) : 0) -
  ...
  function settingsTab(s, w, store, manageable) {
    var parties = D.partiesOf(store.id);
    var mine = parties.filter(function (p) {
      return p.label === 'you';
    })[0];
    ...
    var seniors = parties
      .filter(function (p) {
        return (
          p.party_kind === 'user' &&
          p.label !== 'you' &&
          D.roleRank(p.destination_role) >= 2
        );
      })
      .map(shortName);
    ...
    var rows =
      UI.insetRow({ label: 'Name', value: esc(store.name) }) +
      UI.insetRow({
        label: 'Server',
        value: esc(store.serverName),
        action: UI.btn('Open server', { size: 'sm', attrs: serverNav(store) }),
      }) +
      UI.insetRow({
        label: 'Your account',
        value: esc(
          (account ? account.username : store.account) +
            ' · ' +
            (mine ? fmtRole(mine.destination_role) : '—'),
        ),
      }) +
      UI.insetRow({
        label: 'Owner',
        value: owner ? esc(shortName(owner)) : '—',
      }) +
      UI.insetRow({
        label: 'Group ID',
        value: '<code>' + esc(store.team_id_hex) + '</code>',
        action: UI.btn('Copy', {
          size: 'sm',
          icon: 'copy',
          attrs: 'data-act="toast" data-text="Group ID copied."',
        }),
      });
    ...
    var danger =
      dangerRow(
        ...
        '<span class="menuwrap">' +
          UI.btn('Remove…' + M.icon('chev', null, { cls: 'chevron' }), {
            variant: 'danger',
  ...
  function inspectResponse(w, store, tab) {
    ...
    return {
      people: D.partiesOf(store.id).map(function (party) {
        ...
        if (party.scoped_host_id_hex)
          out.scoped_host_id_hex = party.scoped_host_id_hex;
      ...
      federation: D.federation
        ...
        .map(function (entry) {
          ...
          if (entry.operation_id_hex)
            out.operation_id_hex = entry.operation_id_hex;
  ...
  function inspectToggle(s, w, store, tab) {
    return UI.toggle({
      ...
      body:
        '<pre>' +
        esc(JSON.stringify(inspectResponse(w, store, tab), null, 1)) +
        '</pre>',
    });
  }
    ...
    return UI.body(
      UI.notice({
        ...
        actions: UI.btn(notice.action, {
          ...
          attrs:
            notice.actionKind === 'finish-setup'
  ...
  function stepper(label, dec, inc, decOff, incOff) {
    return (
      ...
      UI.btn('−', {
        size: 'sm',
        disabled: decOff,
        attrs: 'data-act="set" data-key="v.visibility" data-val="' + dec + '"',
      }) +
      '<span>' +
      esc(label) +
      '</span>' +
      UI.btn('+', {
        size: 'sm',
        disabled: incOff,
        attrs: 'data-act="set" data-key="v.visibility" data-val="' + inc + '"',
      }) +
      '</div>'
    );
  }
  ...
  function inviteMessageFor(w, store) {
    ...
    var ownerAccount = ownerStore
      ? D.accounts.filter(function (a) {
          return a.store === ownerStore.id;
        })[0]
      : undefined;
    var inviter = ownerAccount
      ? ownerAccount.username.split('.')[0]
      : 'the group Admin';
    return (
      "Hi Jules — I'd like to add you to " +
      store.name +
      ' on FOKS.\n' +
      ...
      '4. Then tell ' +
      inviter +
      ' your username — there are no invite links; I add you by username.\n' +
      'Once I have, ' +
      store.name +
      ' shows up under Groups in your app.'
    );
  }

  function creationAccounts(w) {
    return w.stores.filter(function (c) {
      return c.kind === 'account' && canCreateInStore(w, c.id);
    });
  }
  ...
  function groupSheet(s, w, store, kinds) {
    ...
    if (!kind) return '';
    var target =
      kind === 'demote' || kind === 'remove' ? targetOf(s, store, sheet) : null;
    ...
    var creationAccount =
      ...
      accounts.filter(function (a) {
        return a.account === 'work';
      })[0] ||
      accounts[0];
    var remotes = remotesFor(w, store);
    var remoteId = v(s, 'remote', '');
    var remote =
      remotes.filter(function (r) {
        return r.alias === remoteId || r.id === remoteId;
      })[0] || remotes[0];
    ...
    var visibility =
      s.v.visibility == null || s.v.visibility === '' ? null : +s.v.visibility;
    var server =
      kind === 'create'
        ? creationAccount
          ? (w.serverById[creationAccount.server] || {}).name
          : undefined
        : store.serverName;
    var failure =
      kind === 'admit'
        ? failureFor(store.id, 'federation')
        : ['add', 'demote', 'remove'].indexOf(kind) >= 0
          ? failureFor(store.id, 'roster')
          : undefined;
    ...
    var title =
      kind === 'invite'
        ...
        : kind === 'add'
          ...
          : kind === 'demote'
            ? 'Lower ' + (target ? shortName(target) + '’s' : 'their') + ' role'
            : kind === 'remove'
              ? 'Remove ' +
                (target ? D.partyName(target) : 'them') +
                ' from ' +
                store.name +
                '?'
              : kind === 'admit'
    ...
    var subtitle =
      kind === 'create'
        ...
        : kind === 'invite'
          ...
          : kind === 'add'
            ? 'They must already have an account on ' + server
            : kind === 'demote'
              ? (target ? fmtRole(target.destination_role) : '') +
                ' in ' +
                store.name +
                ' today'
              : kind === 'remove'
    ...
    var maxVis =
      currentRole && currentRole.kind === 'member'
        ? D.visibilityOf(currentRole) - 1
        : 0;
    ...
    if (failure) {
      body += UI.notice({
        title:
          (failure.source === 'roster' ? 'Roster' : 'Federation') +
          ' unavailable',
        body:
          '<p>' +
          esc(failure.message) +
          ' Close this sheet and refresh before making changes.</p>',
      });
    }
    ...
    if (kind === 'invite') {
      ...
      body +=
        '<p>They need an account on ' +
        esc(server) +
        ' before you can add them. Send this message, then add their username.</p>' +
        UI.inset(
          UI.insetRow({
            label: 'Message',
            value:
              ...
              UI.btn('Copy', {
                size: 'sm',
                attrs: 'data-act="toast" data-text="Message copied."',
              }),
          }),
        ) +
          ...
          UI.insetRow({
            ...
            UI.insetRow({
              ...
              value:
                '<span>' +
                esc(server) +
                '<span class="hint">They enter this address during setup.</span></span>',
            }) +
            UI.insetRow({
              label: 'Signup invite',
              value:
                '<span>Optional<span class="hint">Required only if the server requires an invite.</span></span>',
            }) +
            UI.insetRow({
      ...
      body +=
        UI.inset(textField('Username', '_r_2_', 'v.username', username)) +
        UI.sectionLabel('Role in ' + esc(store.name)) +
        UI.inset(
          UI.radioGroup(
            ['Member', 'Admin', 'Owner'].map(function (next) {
              return UI.radioCard({
                selected: role === next,
                title: next,
                detail:
                ...
                attrs:
                  'data-act="set" data-key="v.role" data-val="' + next + '"',
              });
            }),
        ...
        esc(store.name) +
        ' will appear when their app checks the server.</p>';
    } else if (kind === 'demote') {
      var cards = [];
      if (currentRole && currentRole.kind === 'owner') {
        cards.push(
          UI.radioCard({
            selected: demoteRole === 'Admin',
            title: 'Admin',
      ...
      cards.push(
        UI.radioCard({
          ...
          selected: false,
          title:
            (target ? esc(fmtRole(target.destination_role)) : 'Current role') +
            ' ' +
            UI.chip('current'),
          detail:
            'Roles can only be lowered here. To raise a role, remove and re-add the person.',
        }),
      );
      body +=
        '<p>Roles can only be lowered from here. To raise one, remove the person and add them again at the higher role.</p>' +
        UI.inset(UI.radioGroup(cards, { label: 'New role' })) +
        (demoteRole === 'Member'
          ? stepper(
              'Visibility ' + demoteVis,
              demoteVis - 1,
              demoteVis + 1,
              demoteVis <= -32768,
              demoteVis >= maxVis,
      ...
      body +=
        '<p>Removing blocks future reads and rekeys the group. This user may retain a local copy of their current records.</p>';
      ...
      body +=
        UI.sectionLabel('Group') +
        UI.inset(
          remotes.length
            ? UI.radioGroup(
                remotes.map(function (group) {
                  return UI.radioCard({
                    selected: !!remote && remote.id === group.id,
                    title: esc(group.alias),
                    detail: 'on ' + esc(group.serverName),
                    attrs:
                      'data-act="set" data-key="v.remote" data-val="' +
                      esc(group.alias) +
                      '"',
                  });
                }),
            ...
            : UI.insetRow({
                label: 'Group',
                value: '<span class="dim">No eligible remote group</span>',
              }),
        ) +
        ...
        UI.inset(
          UI.insetRow({
            label: 'Role',
            value:
              UI.chip('Member') + '<span class="dim">for every member</span>',
          }) +
            UI.insetRow({
              label: 'Visibility',
              value: String(avis),
              action:
                UI.btn('−', {
                  ...
                  disabled: avis <= -32768,
                  attrs:
                    'data-act="set" data-key="v.visibility" data-val="' +
                    (avis - 1) +
                    '"',
                }) +
                UI.btn('+', {
        ...
        '<p class="fn">' +
        esc(store.name) +
        ' automatically syncs members from that group. This connection cannot be removed from this screen.</p>';
    } else if (kind === 'create') {
      body +=
        UI.inset(textField('Name', '_r_5_', 'v.name', name)) +
        '<p class="fn">Others find it as <code>' +
        esc(teamAlias || '…') +
          ...
          accounts.length
            ? UI.radioGroup(
                accounts.map(function (account) {
                  var who = D.accounts.filter(function (c) {
                    return c.store === account.id;
                  })[0];
                  return UI.radioCard({
                    selected:
                      !!creationAccount && account.id === creationAccount.id,
                    title: esc(account.serverName),
                    detail: 'as ' + esc(who ? who.username : account.account),
                    attrs:
                      'data-act="set" data-key="v.account" data-val="' +
                      esc(account.id) +
                      '"',
                  });
                }),
            ...
            : UI.insetRow({
                label: 'Account',
                value:
                  '<span class="dim">No account can create a group.</span>',
              }),
        ) +
    ...
    if (kind === 'remove') {
      primary = UI.btn('Remove and rekey', {
        ...
        attrs:
          'data-act="call" data-fn="gApply" data-arg="remove:' +
          esc(SHORT[store.id] || store.id) +
          ':' +
      ...
      var label =
        kind === 'invite'
          ? 'Copy message'
          : kind === 'add'
            ? 'Add ' + (String(username).trim() || 'someone')
            : kind === 'demote'
              ? 'Change role'
              : kind === 'admit'
                ? 'Admit ' + (remote ? remote.alias : 'group')
      ...
      var arg =
        kind === 'invite'
          ...
          : kind === 'add'
            ? 'add:' +
              (SHORT[store.id] || store.id) +
              ':' +
              String(username).trim() +
              ':' +
              role +
              ':' +
              (visibility == null ? 0 : visibility)
            : kind === 'demote'
              ? 'demote:' +
                (SHORT[store.id] || store.id) +
                ':' +
             ...
             StoreRef has a colon in it. */
                  ...
                  createdToken({
                    ...
                    kind: createKind,
                    account: creationAccount ? creationAccount.id : 'acct:work',
                  });
    ...
    return sheetFrame({
      ...
      glyph:
        kind === 'create'
          ? '<span class="kico md group" style="background: ' +
            UI.rgb(D.hue(name || 'group')) +
            ';">' +
            esc((String(name).trim() || 'G').slice(0, 1)) +
            '</span>'
          : mark(store, 'md'),
      title: esc(title),
      ...
      footer:
        UI.btn('Cancel', {
          attrs: 'data-act="set" data-key="v.sheet" data-val=""',
        }) + primary,
    });
  }
  ...
  function inviteDialog(s, w) {
    ...
    var account = store
      ? D.accounts.filter(function (a) {
          return a.store === store.id;
        })[0]
      : undefined;
    var ok = !!(store && account);
    var message = ok
      ? '1. Install FOKS: https://foks.app/download\n' +
        '2. When it asks for a server address, type ' +
        store.serverName +
        '\n' +
        '3. Create your account with username firstname.lastname\n' +
        '4. Then tell ' +
        account.username +
        ' your username — there are no invite links, so that is what I add.'
      : '';
    var body = ok
      ? UI.inset(
          UI.insetRow({
        ...
        UI.inset(
          UI.insetRow({
            ...
            UI.insetRow({
              label: 'Server',
              value:
                esc(store.serverName) +
                '<small>They enter this address during setup.</small>',
            }) +
            UI.insetRow({
              label: 'Signup invite',
              value:
                'Optional<small>Required only if the server requires an invite.</small>',
            }) +
            UI.insetRow({
              label: 'When they reply',
              value:
    ...
    return sheetFrame({
      ...
      title: ok
        ? 'Invite to ' + esc(store.serverName)
      ...
      menuHtml: removable
        .map(function (party) {
          return (
            ...
            esc(shortName(party)) +
            '</b><small>' +
            esc(fmtRole(party.destination_role)) +
            ' · reads ' +
            readsOf(w, party).length +
            ' of ' +
            total +
            '</small></span></button>'
          );
  ...
  function pageAfter(s) {
    groupsAfter();
    placeMenu(s || M.s);
    restoreFixture();
  }

  function tabOf(s, page) {
    return v(s, 'tab', page.gtab);
  }
  ...
  function renderGroup(page, s) {
    ...
    var federationManageable =
      manageable && !failureFor(store.id, 'federation');
    ...
    var peopleCount =
      D.partiesOf(store.id).length +
      D.federation.filter(function (e) {
        return e.store === store.id;
      }).length;
    main += UI.tabs({
      ...
      items: [
        Object.assign(
          ...
          rosterFailure || federationFailure ? {} : { count: peopleCount },
          {
            attrs:
              'data-act="go" data-page="' +
              esc(page.gsiblings.people) +
              '" data-set=\'{"v.tab":"","v.panel":""}\'',
          },
        ),
        {
          id: 'settings',
    ...
    var selected =
      !rosterFailure && chosen
        ? D.partiesOf(store.id).filter(function (p) {
            return shortName(p) === chosen;
          })[0] || null
        : null;

    var wrap =
      tab === 'people'
        ? peopleTab(s, w, store, selected, rosterManageable) +
          federationSection(s, w, store, federationManageable)
  ...
  function overlayGroup(page, s) {
    ...
    if (
      menu === 'rekey' &&
      !unavailable &&
      tabOf(s, page) === 'settings' &&
      removableOf(store, manageable).length
    )
      out += rekeyMenu(w, store, manageable);
    return out;
  }

  function unavailableMain() {
    return (
      UI.pageHeader({ title: 'Group unavailable', subtitle: '' }) +
      UI.body(
        UI.notice({
  ...
  function panelValues(ref) {
    return [{ v: '', label: 'None' }].concat(
      (BASE.parties[ref] || []).map(function (p) {
        return { v: shortName(p), label: shortName(p) };
      }),
    );
  }
  function targetValues(ref) {
    return [{ v: '', label: 'Default' }].concat(
      (BASE.parties[ref] || [])
        .filter(function (p) {
          return D.actionableGroupMember(p);
        })
        .map(function (p) {
          return { v: shortName(p), label: shortName(p) };
        }),
  ...
  var GROUP_SHEET_VALUES = [
    { v: '', label: 'None' },
    {
      v: 'add',
      label: 'add',
      hint: 'Add someone to <group> — username, role radio, visibility stepper.',
    },
    {
      v: 'demote',
      label: 'demote',
      hint: 'Lower priya.n’s role (an Admin target: Member + the disabled current card).',
    },
    {
      v: 'demote-owner',
      label: 'demote-owner',
      hint: 'The same sheet on sam.ortiz (an Owner target: an extra Admin option).',
    },
    {
      v: 'remove',
      label: 'remove',
      hint: 'Remove dana.okafor from <group>? — an alertdialog.',
    },
    {
      v: 'party-remove',
      label: 'party-remove',
      hint: 'The refusal: the target is forced locally_manageable:false, so the confirm is disabled behind the Notice.',
    },
    {
      v: 'admit',
      label: 'admit',
      hint: 'Add a group to <group> — the remote-group radio and the visibility row.',
    },
    {
      v: 'invite',
      label: 'invite',
      hint: 'Deep link only: GroupSheetKind includes it and a render test pins it, but nothing in the app opens it — map §7.1.',
    },
  ];
  var SETTINGS_SHEET_VALUES = [
    { v: '', label: 'None' },
    {
      v: 'create',
      label: 'create',
      hint: 'Create a group — named vs ad-hoc, the account radio, the live slug footnote.',
    },
    {
      v: 'join-invite',
      label: 'join-invite',
      hint: 'The Settings invite dialog (settings-screen.tsx:989) — different copy from the group invite sheet.',
    },
  ];
  var COMMON_CONTROLS = [
    {
      key: 'tab',
      label: 'Tab',
      note: 'GroupSettingsScreen tab state; null = the page’s own tab.',
      values: [
        { v: '', label: 'Page default' },
        { v: 'people', label: 'People' },
        { v: 'settings', label: 'Group settings' },
      ],
    },
    {
      key: 'sheet',
      label: 'Sheet',
      note: 'GroupSheet (groups-screen.tsx:1070). `create` is the Settings host’s, so it is not offered here.',
      values: GROUP_SHEET_VALUES,
    },
    {
      key: 'menu',
      label: 'Menu',
      note: 'The two portalled menus: the hero “Group actions” menu and the Danger “Removable members” menu.',
      values: [
        { v: '', label: 'None' },
        { v: 'more', label: 'Group actions' },
        { v: 'rekey', label: 'Removable members' },
      ],
    },
    {
      key: 'role',
      label: 'Role draft',
      note: 'The add sheet’s role radio, and the demote sheet’s new role.',
      values: [
        { v: '', label: 'Default' },
    ...
    {
      key: 'visibility',
      label: 'Visibility draft',
      note: 'The visibility stepper (add / demote / admit). Bounds −32768…32767.',
      values: [
        { v: '', label: 'Default' },
        { v: '0', label: '0' },
        { v: '-1', label: '-1' },
    ...
    },
    {
      key: 'remote',
      label: 'Remote group',
      note: 'The admit sheet’s candidate: team ∧ active ∧ named ∧ readable ∧ a different server from this group.',
      values: [
        { v: '', label: 'First eligible' },
    ...
    },
    {
      key: 'gkind',
      label: 'Create kind',
      note: 'The create sheet’s Named / Ad-hoc radio (ad-hoc sends name: ""; the Name field stays on screen either way).',
      values: [
        { v: '', label: 'Named' },
    ...
    },
    {
      key: 'name',
      label: 'Create name',
      note: 'The create sheet’s Name field; the slug footnote and the “Create X” label track it live. “(cleared)” is the field typed empty — the slug reads “…” and the primary “Create group”.',
      values: [
        { v: '', label: 'Platform' },
        { v: 'Ops', label: 'Ops' },
        { v: 'Design squad', label: 'Design squad' },
    ...
    },
    {
      key: 'username',
      label: 'Add username',
      note: 'The add sheet’s Username field; the primary reads “Add <username>” and is disabled when the field is “(cleared)”.',
      values: [
        { v: '', label: 'jules.park' },
        { v: 'nils', label: 'nils' },
        { v: EMPTY, label: '(cleared)' },
    ...
    },
    {
      key: 'account',
      label: 'Account',
      note: 'The create sheet’s server/account radio and the Settings invite dialog’s account (shared with 70-settings.js).',
      values: [
        { v: '', label: 'Default' },
        { v: 'acct:work', label: 'acct:work' },
        { v: 'acct:personal', label: 'acct:personal' },
      ],
    },
    {
      key: 'applied',
      label: 'Applied',
      note: 'The post-mutation world, replayed from tokens so the roster, the sidebar caption and the reader counts all move together. Several can be combined with commas.',
      values: [
        { v: '', label: 'Nothing' },
    ...
    {
      key: 'adhoc',
      label: 'Homelab resumed',
      note: 'resumeGroupCreation landed: Homelab becomes an active ad-hoc group (info band, 0 people, no add affordances).',
      values: [
        { v: '', label: 'No' },
        { v: 'yes', label: 'Yes' },
      ],
    },
    {
      ...
      note: 'world.groupDetailFailures — CODE-DERIVED. No ?state= alias reaches these in the app (map §6, §7.3); the copy comes from groups-screen.tsx and vault-shell.render.test.tsx:3230.',
      values: [
        { v: '', label: 'None' },
        { v: 'roster', label: 'Roster (retryable)' },
        { v: 'federation', label: 'Federation (final)' },
        { v: 'both', label: 'Both' },
      ],
    },
    {
      key: 'inspect',
      label: 'Inspect response',
      note: 'The disclosure under the tab body.',
      values: [
        { v: '', label: 'Collapsed' },
        { v: '1', label: 'Expanded' },
      ],
    },
    {
      ...
      values: [
        { v: '', label: 'Not created' },
        {
          v: 'platform|Platform|named|acct:work',
          label: 'Platform · named · work',
        },
    ...
    return [
      {
        key: 'panel',
        label: 'Party panel',
        note: 'The selected party — the aside and the .body.group-panel-open class.',
        values: panelValues(ref),
      },
      {
        key: 'target',
        label: 'Sheet target',
        note: 'Which party the demote / remove sheet acts on.',
        values: targetValues(ref),
      },
  ...
  function groupPages(o) {
    ...
    [
      ...
      var page = M.page({
        ...
        render: function (s) {
          return renderGroup(this, s);
        },
        overlay: function (s) {
          return overlayGroup(this, s);
        },
        after: pageAfter,
      });
      page.gref = o.ref;
  ...
  function groupsSection(s, w) {
    var accounts = w.stores.filter(function (st) {
      return st.kind === 'account';
    });
    var canCreate = accounts.some(function (st) {
      return canCreateInStore(w, st.id);
    });
    ...
    var out =
      UI.sectionLabel('Create a group') +
      UI.inset(
        UI.insetRow({
          label: 'New group',
    ...
    if (!canCreate) {
      out +=
        '<p class="fn">No available account can create a group right now. Add an account, or restore access to a server.</p>';
    }
    if (attention.length) {
      out +=
          ...
          attention
            .map(function (store) {
              return UI.insetRow({
                ...
                value:
                  '<span class="who2">' +
                  UI.stack(D.partiesOf(store.id), { size: 'lg' }) +
                  ...
                  '</small></span></span>' +
                  UI.chip(
                    store.state === 'inactive' ? 'Inactive' : 'Unavailable',
                    { tone: 'warn' },
                  ),
                action: UI.btn('Open', {
                  size: 'sm',
    ...
    out +=
      UI.sectionLabel('Invite someone') +
      UI.inset(
        accounts.length
          ? accounts
              .map(function (store) {
                ...
                return UI.insetRow({
                  ...
                  action: UI.btn(
                    account
                      ? 'Invite as ' + esc(account.username) + '…'
                      : 'Invite someone…',
                    {
                      size: 'sm',
                      disabled: !account,
                      attrs:
                        'data-act="call" data-fn="gInvite" data-arg="' +
                        esc(store.id) +
                        '"',
                    },
                  ),
          ...
          : UI.insetRow({
              label: 'None',
              value: 'No account on this Mac yet.',
            }),
        { className: 'settings-inset' },
      );
    return out;
    ...
    controls: [
      {
        key: 'sheet',
        label: 'Sheet',
        note: 'The two sheets settings-screen.tsx mounts: the create sheet and the invite dialog.',
        values: SETTINGS_SHEET_VALUES,
      },
      COMMON_CONTROLS.filter(function (c) {
        return c.key === 'account';
      })[0],
      COMMON_CONTROLS.filter(function (c) {
        return c.key === 'gkind';
      })[0],
      ...
      COMMON_CONTROLS.filter(function (c) {
        return c.key === 'applied';
      })[0],
      COMMON_CONTROLS.filter(function (c) {
        return c.key === 'adhoc';
      })[0],
      COMMON_CONTROLS.filter(function (c) {
        return c.key === 'failure';
      })[0],
      COMMON_CONTROLS.filter(function (c) {
        return c.key === 'created';
      })[0],
    ],
    render: function (s) {
      var t = M.globalTakeover(s);
  ...
  M.page({
    ...
    render: function (s) {
      ...
      var toolbar = UI.toolbar(
        '<span class="menuwrap">' +
          UI.btn('New' + M.icon('chev', null, { cls: 'chevron' }), {
            variant: 'primary',
          ...
          UI.segmented({
            ...
            items: [{ id: 'All', label: 'All' }].concat(
              D.KIND_LIST.map(function (k) {
                return {
                  id: k,
                  label: D.KINDS[k].plural,
                  title: D.KINDS[k].blurb,
                };
              }),
            ),
          }) +
          UI.spacer() +
          '<span class="menuwrap">' +
          UI.btn('Sort by name' + M.icon('chev', null, { cls: 'chevron' }), {
            attrs: 'aria-haspopup="menu" aria-expanded="false"',
          }) +
          '</span>' +
          UI.segmented({
  ...
  M.fns.gOpenPanel = function (name) {
    M.set('v.panel', name);
  };
  ...
  M.fns.gSheetTarget = function (arg) {
    ...
    M.patch(
      Object.assign({}, DRAFTS, { 'v.sheet': bits[0], 'v.target': bits[1] }),
    );
    M.render();
  };
  ...
  M.fns.gRestore = function (operationId) {
    pushOp(
      'restore:' +
        (SHORT[currentStoreRef()] || currentStoreRef()) +
        ':' +
        operationId,
    );
    M.render();
  ...
  M.fns.gApply = function (token) {
    ...
    if (bits[0] === 'add')
      message = 'Add someone to ' + store.name + ' completed';
    else if (bits[0] === 'demote')
      message = 'Lower ' + bits[2] + '’s role completed';
    else if (bits[0] === 'remove')
      message = 'Remove ' + bits[2] + ' from ' + store.name + '? completed';
    else if (bits[0] === 'admit')
      message = 'Add a group to ' + store.name + ' completed';
    pushOp(token);
    M.patch({
      'v.sheet': null,
      'v.target': null,
  ...
  function groupsEscape() {
    return baseEscape();
  }
  M.fns.escape = groupsEscape;
  groupsAfter = function () {
    M.fns.escape = groupsEscape;
  };
})();
