...
(function () {
  ...
  M.settingsAccounts = function (w) {
    ...
    return (w || D.world(M.s)).stores.filter(function (st) {
  ...
  M.settingsSelected = function (s, w) {
    ...
    for (var i = 0; i < accounts.length; i++)
      if (accounts[i].id === want) return accounts[i];
    return undefined;
  };
    ...
    return (
      ...
      UI.segmented({
        ...
        items: accounts.map(function (store) {
          ...
          var server = w.serverById[store.server]
            ? w.serverById[store.server].name
            : store.server;
          return {
            id: store.id,
  ...
  M.settingsFrame = function (s, sectionId, bodyHtml, opts) {
    ...
    var nav = SECTIONS.map(function (item) {
      return (
        ...
        (item.id === sectionId ? 'nav on' : 'nav') +
        '"' +
        ' data-act="call" data-fn="settingsNav" data-arg="' +
        item.page +
        '">' +
        M.icon(item.icon) +
        '<span class="t">' +
    ...
    var head = UI.pageHeader({
      ...
      subtitle:
        opts.subtitle === undefined
          ? 'Accounts, servers, keys, groups and this Mac'
          : opts.subtitle,
    });
    var main =
      (opts.switcher ? M.settingsSwitcher(s, opts.switcherKey) : '') +
      (bodyHtml || '');
    return UI.main(
      head +
        UI.body(
  ...
  function glyph(danger) {
    return (
      ...
      (danger ? 'danger' : '') +
      '">' +
      M.icon(danger ? 'trash' : 'gear') +
      '</span>'
    );
  }
  function closeBtn(label) {
    return UI.btn(esc(label), {
      attrs: 'data-act="call" data-fn="settingsClose"',
    });
  }
  ...
  function devicesOf(s, ref) {
    ...
    if (ref === 'acct:personal') {
      rows = D.devices.map(function (d) {
        return {
          ...
          role: d.role,
          current: d.current,
          id: D.deviceIdFor(d.id_hex),
        };
      });
    } else {
      rows = [
        {
          name: D.syntheticDevice.name,
          role: D.syntheticDevice.role,
          current: D.syntheticDevice.current,
          id: D.syntheticDevice.id,
        },
      ];
    }
    ...
    if (mode === 'plus08') {
      rows = rows.concat([
        { name: null, role: 'owner', current: false, id: KEY_ID },
      ]);
    }
    if (mode === 'keycurrent') {
  ...
  function enrollmentsOf(s) {
    ...
    if (mode === 'pending')
      rows = rows.concat([{ alias: 'half-enrolled', state: 'pending' }]);
    return rows;
  }
  function cardsOf() {
    return D.yubi.cardsConnected;
  }
  ...
  function snapshotOf(w, profile) {
    ...
    return {
      host: true,
      leaseRequired: true,
      lease: profile === 'acme' ? 'lapsed' : 'fresh',
    };
  }
        ...
        ? devices
            .map(function (device) {
              var action = device.current
                ...
                : device.id.indexOf('04') === 0
                  ? UI.btn('Remove…', {
                      ...
                      disabled: stopped,
                      attrs:
                        'data-act="call" data-fn="settingsRemoveOpen" data-arg="' +
                        esc(device.id) +
                        '"',
                    })
              ...
              return UI.insetRow({
                label: esc(
                  device.name ||
                    (device.id.indexOf('08') === 0 ? 'YubiKey' : 'Device'),
                ),
                value:
                  '<b>' +
                  esc(device.role) +
    ...
    return h(
      ...
      UI.inset(
        UI.insetRow({
          ...
          value:
            '<b>' +
            esc(account ? account.username : 'Identity unavailable') +
            '</b>',
      ...
      UI.inset(
        UI.insetRow({
          ...
          action: UI.btn('Start pairing…', {
            ...
            disabled: stopped,
            attrs:
              'data-act="go" data-page="set-devices" data-set=\'{"v.sheet":"pair-offer","v.seg":"","v.started":""}\'',
          }),
        }) +
          UI.insetRow({
            label: 'On this Mac',
            ...
            action: UI.btn('Accept or resume…', {
              disabled: stopped,
              attrs:
                'data-act="go" data-page="set-devices" data-set=\'{"v.sheet":"pair-accept","v.seg":"","v.started":""}\'',
            }),
          }),
        { className: 'settings-inset' },
      ),
      ...
      UI.inset(
        UI.insetRow({
          ...
          value: loading
            ...
            : backups.length
              ? esc(
                  D.plural(backups.length, 'authenticated enrollment') +
                    ' on this account',
                )
          ...
          action: UI.btn(backups.length ? 'Enroll another…' : 'Enroll…', {
            disabled: stopped,
            attrs:
              'data-act="go" data-page="set-devices" data-set=\'{"v.sheet":"phrase-prepare","v.written":""}\'',
          }),
        }) +
          UI.insetRow({
            label: 'Recover here',
            ...
            action: UI.btn('Recover…', {
              disabled: stopped,
              attrs:
                'data-act="go" data-page="set-devices" data-set=\'{"v.sheet":"recover"}\'',
            }),
          }),
  ...
  function keysBody(s) {
    ...
    var hasComplete = yubi.some(function (e) {
      return e.state === 'complete';
    });
    var hasPending = yubi.some(function (e) {
    ...
    var connected = loading
      ...
      : cards.length
        ...
        : UI.insetRow({
            label: 'None',
            value: 'No card is connected right now.',
          });
    ...
    var rows = YUBI_ACTIONS.map(function (entry) {
      ...
      return UI.insetRow({
        ...
        action: UI.btn(esc(label.indexOf(' ') >= 0 ? 'Open…' : label), {
          ...
          title: title,
          attrs:
            'data-act="go" data-page="set-keys" data-set=\'{"v.sheet":"yubi-' +
            id +
            '"}\'',
        }),
      });
    ...
    return h(
      ...
      stopped
        ? UI.band({
            severity: 'crit',
            label: 'Security-key access is stopped',
      ...
      UI.inset(
        UI.insetRow({
          label: 'New account',
          value: 'Keys live on the card from the start.',
          action: UI.btn('Create on YubiKey…', {
            variant: 'primary',
            disabled: stopped,
            attrs:
              'data-act="go" data-page="set-keys" data-set=\'{"v.sheet":"enrol"}\'',
          }),
        }) +
          UI.insetRow({
            label: 'Existing account',
            value:
              'Make a currently connected card an owner device for a software account on this Mac.',
            action: UI.btn('Provision…', {
              disabled: stopped,
              attrs:
                'data-act="go" data-page="set-keys" data-set=\'{"v.sheet":"provision"}\'',
            }),
          }),
        { className: 'settings-inset' },
      ),
      ...
      UI.inset(
        UI.insetRow({
          ...
          value: 'Rotates account keys controlled by this YubiKey.',
          action: UI.btn('Revoke…', {
            variant: 'danger',
            disabled: stopped || !hasComplete,
            attrs:
              'data-act="go" data-page="set-keys" data-set=\'{"v.sheet":"revoke"}\'',
          }),
        }),
  ...
  function accountStatus(w, store, pending) {
    ...
    var lapsed =
      state === 'lease-lapsed' ||
      (!pending && snap.host && snap.lease === 'lapsed');
    var stopped =
      inventoryUnavailable ||
      lapsed ||
      state === 'blocked' ||
      state === 'never-probed' ||
      (!pending && (!snap.host || snap.lease !== 'fresh'));
    return {
      server: srv,
      stopped: stopped,
      pending: !!pending,
      inert: stopped || !!pending,
      label: pending
        ? 'Reading check-in…'
        : inventoryUnavailable
          ? 'connection error'
    ...
    return h(
      ...
      stores
        .map(function (store) {
          ...
          var current =
            st.stopped || st.pending
              ? null
              : devicesOf(s, store.id).filter(function (d) {
                  return d.current;
                })[0];
          function pass(mode, label) {
            return UI.btn(esc(label), {
              ...
              disabled: st.inert,
              attrs:
                'data-act="call" data-fn="settingsPassphraseOpen" data-arg="' +
                mode +
                '|' +
                esc(store.id) +
                '"',
          ...
          return (
            '<div>' +
            UI.sectionLabel(
              esc(srv ? srv.name : store.server) +
                (srv && srv.label ? ' · ' + esc(srv.label) : ''),
            ) +
            UI.inset(
              UI.insetRow({
                label: 'Username',
                value:
                  ...
                  esc(
                    D.accounts
                      ...
                      .map(function (a) {
                        return a.username;
                      })[0] || 'Identity unavailable',
                  ) +
                  '</b> ' +
                  UI.chip(esc(st.label), {
                    tone: st.stopped ? 'warn' : 'default',
                  }),
              }) +
                UI.insetRow({
                  label: 'Account alias',
  ...
  function aboutBody(s) {
    ...
    return h(
      ...
      UI.inset(
        UI.insetRow({
          ...
          UI.insetRow({
            ...
            valueClass: 'mono',
            value:
              esc(info ? info.agentSocket : 'Reading app info…') +
              '<small>Local connection used by this desktop app.</small>',
            action: info
              ? UI.btn('Copy', {
                  size: 'sm',
                  attrs: 'data-act="toast" data-text="Socket path copied"',
                })
              : undefined,
          }) +
          UI.insetRow({
            ...
            action: ready
              ? undefined
              : UI.btn('Retry connection', {
                  variant: 'primary',
                  attrs: 'data-act="call" data-fn="settingsConnectionRetry"',
                }),
          }),
        { className: 'settings-inset' },
      ),
      UI.toggle({
        ...
        body:
          '<pre>' +
          esc(JSON.stringify({ phase: w.agent.phase }, null, 2)) +
          '</pre>',
        attrs:
  ...
  function unavailableBody(s) {
    var w = D.world(s);
    var stores = M.settingsAccounts(w);
    var here = MINE[s.page]
      ? s.page === 'set-unavailable'
        ? 'set-devices'
        : s.page
      : 'set-devices';
    return UI.notice({
      ...
      body:
        '<p>This account is no longer associated with this Mac, and no other account has been selected in its place.</p>' +
        (stores.length
          ? ''
          : '<p>This Mac has no active account. Add and verify a server, then create or recover an account.</p>'),
      actions:
        UI.btn('Refresh the catalog', {
          variant: 'primary',
          attrs: 'data-act="toast" data-text="Refreshed the catalog"',
        }) +
        stores
          .map(function (store) {
            var srv = w.serverById[store.server];
            return UI.btn(
              esc(store.account) + ' · ' + esc(srv ? srv.name : store.server),
              {
                attrs:
  ...
  function phrasePrepareSheet(s) {
    ...
    return sheet({
      ...
      subtitle:
        esc(x.account ? x.account.username : '') +
        ' on ' +
        esc(x.server ? x.server.name : ''),
      body: UI.inset(
        field({ key: 'alias', label: 'Backup alias', value: alias }),
      ),
      footer:
        closeBtn('Cancel') +
        UI.btn('Prepare phrase', {
          variant: 'primary',
          disabled: !String(alias).trim(),
          attrs:
            'data-act="go" data-page="set-devices" data-set=\'{"v.sheet":"phrase","v.written":""}\'',
        }),
    });
  }
  function phraseSheet(s) {
    ...
    return sheet({
      ...
      subtitle:
        ...
        esc(x.server ? x.server.name : ''),
      body:
        '<p>This phrase is shown once and cannot be copied. Write it down now. The agent stores only the public key.</p>' +
        '<div class="words">' +
        D.backupPhraseWords
          .map(function (word, i) {
            return (
          ...
          .join('') +
        '</div>' +
        '<label class="checkline" data-act="set" data-key="v.written" data-val="' +
        (written ? '' : '1') +
        '">' +
        '<input type="checkbox"' +
        (written ? ' checked' : '') +
        '>I have written these down</label>',
      footer: UI.btn('Done', {
        variant: 'primary',
        disabled: !written,
  ...
  function pairSheet(s) {
      ...
      items: [
        {
          id: 'offer',
          label: 'From this Mac',
          attrs: 'data-act="call" data-fn="settingsPairMode" data-arg="offer"',
        },
        {
          id: 'accept',
          label: 'On this Mac',
          attrs: 'data-act="call" data-fn="settingsPairMode" data-arg="accept"',
        },
      ],
    });
    if (mode === 'offer') {
      body +=
        '<p>Select Start, enter the pairing phrase on the other Mac, then select Finish here. Resume opens the pending offer.</p>' +
        (started
          ? UI.inset(
              UI.insetRow({
      ...
      body +=
        '<p>Enter the pairing phrase from the other Mac. Resume continues a pending acceptance.</p>' +
        UI.inset(
    ...
    if (mode === 'offer') {
      footer +=
        UI.btn('Resume offer', {
          attrs:
            'data-act="call" data-fn="settingsPairResume" data-arg="offer"',
        }) +
        UI.btn('Finish', {
          disabled: !started,
          attrs:
            'data-act="call" data-fn="settingsPairDone" data-arg="Pairing finished; refreshed authenticated devices"',
        }) +
        UI.btn('Start', {
          variant: 'primary',
          attrs: 'data-act="set" data-key="v.started" data-val="1"',
        });
    } else {
      footer +=
        UI.btn('Resume acceptance', {
          disabled: !alias,
          attrs:
            'data-act="call" data-fn="settingsPairResume" data-arg="accept"',
        }) +
        UI.btn('Accept', {
          variant: 'primary',
          disabled: !(alias && device && phrase),
          attrs:
            'data-act="call" data-fn="settingsPairDone" data-arg="Pairing accepted; refreshed authenticated devices"',
        });
    }
    return sheet({
  ...
  function recoverSheet(s) {
    ...
    return sheet({
      ...
      body:
        '<p>Recovery adds this Mac as a new owner device. If interrupted, resume it from Alerts with the same phrase.</p>' +
        UI.inset(
          field({ key: 'alias', label: 'Local alias', value: alias }) +
            ...
            UI.insetRow({
              ...
              forId: 'sf-tokens',
              value:
                '<textarea id="sf-tokens" data-bind="v.tokens" data-live>' +
                esc(tokens) +
                '</textarea>',
            }),
        ),
  ...
  function removeDeviceSheet(s) {
    ...
    var device =
      ...
      (id
        ? null
        : rows.filter(function (d) {
            return !d.current && d.id.indexOf('04') === 0;
          })[0]);
    if (!device) return '';
    ...
    return sheet({
      ...
      body:
        '<p>This device loses future access to the account. Any data previously downloaded to this device will remain until removed; change any sensitive secrets if the device is not under your control.</p>' +
      ...
      footer:
        closeBtn('Cancel') +
        UI.btn('Remove device', {
          variant: 'danger',
          disabled: typed !== expected,
          attrs:
            'data-act="call" data-fn="settingsRemoveDevice" data-arg="' +
            esc(device.id) +
            '"',
        }),
    });
  ...
  function enrolSheet(s) {
    ...
    var pin = val(s, 'pin', ''),
      puk = val(s, 'puk', '');
    var slot1 = val(s, 'slot1', '0x82'),
      slot2 = val(s, 'slot2', '0x83');
    var tries1 = val(s, 'tries1', '3'),
      tries2 = val(s, 'tries2', '3');
    var slotOk =
      /^0x[0-9a-fA-F]{2}$/.test(slot1) &&
      /^0x[0-9a-fA-F]{2}$/.test(slot2) &&
      slot1 !== slot2;
    var triesOk =
      +tries1 > 0 && +tries1 <= 255 && +tries2 > 0 && +tries2 <= 255;
    return sheet({
      title: 'Create a YubiKey account',
      subtitle:
        'A new account on ' +
        esc(x.store ? x.store.server : '') +
      ...
      body:
        ...
        (card
          ? '<p><b>YubiKey ' +
            esc(card.serial) +
            '</b> is connected. FOKS cannot identify an existing alias from the card serial.</p>'
          : UI.band({
              label: 'Connect a YubiKey.',
              text: 'No card is connected.',
            })) +
        UI.inset(
          field({ key: 'alias', label: 'Alias', value: alias }) +
            ...
            inputRow({
              ...
              type: 'password',
              value: val(s, 'invite', ''),
              hint: 'Optional; cleared on submission.',
            }),
        ) +
        '<details class="adv"' +
        (s.v.advanced === '1' ? ' open' : '') +
        '>' +
        '<summary data-act="set" data-key="v.advanced" data-val="' +
        (s.v.advanced === '1' ? '' : '1') +
        '">Advanced</summary>' +
        UI.inset(
          field({
      ...
      footer:
        closeBtn('Cancel') +
        UI.btn('Prepare card and create account', {
          variant: 'primary',
          disabled: !(
            card &&
            String(alias).trim() &&
          ...
          field({ key: 'alias', label: 'Target alias', value: alias }) +
            ...
            (cards.length
              ? UI.insetRow({
                  ...
                  forId: 'sf-serial',
                  value:
                    '<select id="sf-serial" data-bind="v.serial">' +
                    cards
                      .map(function (card) {
                        return (
                          '<option value="' +
  ...
  function yubiSheet(s, action) {
    ...
    return sheet({
      ...
      body:
        '<p>The values below go only to the local agent and are cleared when submitted.</p>' +
        UI.inset(
          UI.insetRow({
            label: 'Key alias',
            value: '<b>' + esc(alias || 'No key enrolled') + '</b>',
          }) +
            (needsPin
              ? inputRow({
                  key: 'pin',
  ...
  function revokeSheet(s) {
    var complete = enrollmentsOf(s).filter(function (e) {
      return e.state === 'complete';
    })[0];
    if (!complete) return '';
    ...
    return sheet({
      danger: true,
      title: 'Revoke ' + esc(alias) + '?',
      subtitle: 'Disconnects this key and updates account security',
      body:
        '<p>This YubiKey will immediately lose access to your account. Any data previously cached on devices using this key will remain until cleared. Enter the key alias to confirm revocation.</p>' +
        UI.inset(
          inputRow({
            key: 'confirm',
            label: 'Confirm',
      ...
      footer:
        closeBtn('Cancel') +
        UI.btn('Revoke ' + esc(alias), {
          variant: 'danger',
          disabled: typed !== alias,
          attrs:
            'data-act="call" data-fn="settingsRevoke" data-arg="' +
            esc(alias) +
            '"',
        }),
    });
      ...
      footer:
        ...
        UI.btn(
              ...
              : 'Change passphrase',
          {
            ...
            attrs:
              'data-act="call" data-fn="settingsPassphraseRun" data-arg="' +
              mode +
              '"',
          },
        ),
  ...
  function goProfileSheet() {
    return sheet({
      ...
      body: UI.inset(
        '<p>The official FOKS client was not found in its standard location.</p>' +
          UI.btn('Scan again', {
            attrs:
              'data-act="hint" data-text="discoverGoProfiles() — the mock always answers { installed:false, candidates:[] }."',
          }),
      ),
  ...
  function overlayFor(s) {
    ...
    switch (open) {
      case 'phrase-prepare':
        return phrasePrepareSheet(s);
      case 'phrase':
        return phraseSheet(s);
      case 'pair-offer':
      case 'pair-accept':
        return pairSheet(s);
      case 'recover':
        return recoverSheet(s);
      case 'remove-device':
        return removeDeviceSheet(s);
      case 'enrol':
        return enrolSheet(s);
      case 'provision':
        return provisionSheet(s);
      case 'revoke':
        return revokeSheet(s);
      case 'passphrase':
        return passphraseSheet(s);
      case 'go-profile':
        return goProfileSheet(s);
      default:
        return '';
    }
  }
  ...
  function clearSheet() {
    SHEET_KEYS.forEach(function (k) {
      M.set('v.' + k, null, { silent: true });
    });
  }
  M.fns.settingsClose = function () {
    clearSheet();
    M.render();
  };
  ...
  M.fns.settingsRemoveDevice = function (id) {
    var device = devicesOf(M.s, 'acct:personal').filter(function (d) {
      return d.id === id;
    })[0];
    for (var i = 0; i < D.devices.length; i++) {
      if ('04' + D.devices[i].id_hex.slice(2) === id) {
    ...
    M.render();
    M.toast(
      'Removed ' + ((device && device.name) || 'device') + ' from this account',
    );
  };
  M.fns.settingsPhraseDone = function () {
    var store = M.settingsSelected(M.s, D.world(M.s));
    if (store)
      backupsOf(store.id).push({ alias: M.s.v.alias || 'paper-backup' });
    clearSheet();
    M.render();
  ...
  M.fns.settingsPairResume = function (which) {
    var held =
      M.s.v.pending === '1' || (which === 'offer' && M.s.v.started === '1');
    if (!held) {
      return M.toast(
        which === 'offer'
  ...
  M.fns.settingsEnrol = function () {
    ...
    D.yubi.accounts.push({
      alias: alias,
      server: store ? store.server : 'personal',
      serial: cardsOf()[0] ? cardsOf()[0].serial : 0,
      state: 'complete',
    });
    clearSheet();
    M.render();
  ...
  M.fns.settingsRevoke = function (alias) {
    D.yubi.accounts = D.yubi.accounts.filter(function (a) {
  ...
  M.fns.settingsNav = function (page) {
    clearSheet();
    if (M.s.v.account && !M.settingsSelected(M.s))
      M.set('v.account', null, { silent: true });
  ...
  var ACCOUNT_VALUES = [
    {
      v: 'acct:personal',
      label: 'personal · foks.example.net',
      hint: 'rae on foks.example.net — never stopped.',
    },
    {
      v: 'acct:work',
      label: 'work · foks.acme-corp.com',
      hint: 'rae.chen on foks.acme-corp.com — stopped whenever Acme is not ok.',
    },
  ];
  var ACCOUNT_CONTROL = {
    key: 'account',
  ...
  var ACCOUNT_CONTROL_STALE = {
    ...
    values: ACCOUNT_VALUES.concat([
      {
        v: 'acct:nope',
        label: 'acct:nope (stale)',
        hint: 'no such store: the crit notice sits above the pane, which still renders.',
      },
    ]),
  };
  var ACCOUNTS_CONTROL = {
    key: 'accounts',
    label: 'Accounts on this Mac',
    ...
    values: [
      {
        v: '',
        label: 'Two accounts',
        hint: 'the fixture: acct:personal and acct:work.',
      },
      {
        v: 'none',
        label: 'None at all',
        hint: 'no account store: MacsSection and AccountSection each draw their Notice, and UnavailableAccount grows its second paragraph. Settings-only — the sidebar still lists the fixture’s vaults.',
      },
    ],
  };
  var LOADING_CONTROL = {
    key: 'loading',
    label: 'Reads in flight',
    ...
    values: [
      { v: '', label: 'Loaded' },
      {
        v: '1',
        label: 'Reading…',
        hint: '“Loading devices…”, “Loading keys…”, “Reading check-in…”, “Reading app info…”. Nothing new is disabled by it except the 13 key rows, whose title becomes “Reading this account…”.',
      },
    ],
  };
  var DEVLIST_CONTROL = {
    ...
    values: [
      {
        v: 'fixture',
        label: 'Fixture',
        hint: 'MacBook Pro (current) + Travel Mac (04… → Remove…).',
      },
      {
        v: 'plus08',
        label: '+ 08… key',
        hint: 'an unnamed 08… device: label "YubiKey", chip "managed under Security keys".',
      },
      {
        v: 'keycurrent',
        label: 'Current is a key',
        hint: 'the current device id starts 08 → chip "current security key".',
      },
      {
        v: 'none',
        label: 'No devices',
        hint: 'InsetRow "None" / "No devices connected to this account."',
      },
    ],
  };
    ...
    values: [
      {
        v: 'fixture',
        label: 'Fixture',
        hint: 'one complete enrollment, "primary key".',
      },
      {
        v: 'pending',
        label: '+ pending',
        hint: 'adds a pending enrollment, so Resume enrollment goes live.',
      },
      {
        v: 'none',
        label: 'No enrollment',
        hint: 'no enrollment: all 13 rows and Revoke disabled, "No complete enrollment found".',
      },
    ],
  };
    ...
    values: [
      {
        ...
        hint: 'Resume offer (before Start) and Resume acceptance raise “no longer pending”, in a red toast.',
      },
      {
        v: '1',
        label: 'One pending',
        hint: 'Resume offer re-reveals the phrase; Resume acceptance finishes it.',
      },
    ],
  };
  var TYPED_CONTROL = {
    key: 'typed',
    label: 'Confirmation typed',
  ...
  M.page({
    ...
    controls: [
      ...
      { key: 'sheet', label: 'Sheet', values: SHEET_VALUES_MACS },
      {
        ...
        label: 'Pairing direction',
        values: [
          { v: '', label: 'The sheet decides' },
          { v: 'offer', label: 'From this Mac' },
          { v: 'accept', label: 'On this Mac' },
        ],
      },
      {
        key: 'started',
        label: 'Offer started',
        note: 'startDevicePairing returned; the phrase row shows and Finish goes live.',
        values: [
          { v: '', label: 'No' },
          { v: '1', label: 'Phrase shown' },
        ],
      },
      PENDING_CONTROL,
      {
    ...
    render: function (s) {
      ...
      return {
        main: M.settingsFrame(s, 'macs', pane.body, {
  ...
  M.page({
    ...
    controls: [
      ...
      },
      {
        key: 'advanced',
        label: 'Advanced open',
        note: '<details class="adv"> in the Enroll sheet.',
        values: [
          { v: '', label: 'Closed' },
          { v: '1', label: 'Expanded' },
        ],
    ...
    render: function (s) {
      ...
      if (t) return t;
      return {
        main: M.settingsFrame(s, 'keys', sectionBody(s, keysBody).body),
      };
    },
    overlay: overlayFor,
    after: selectDefaultAccount,
  ...
  M.page({
    ...
    controls: [
      ...
      },
      {
        ...
        label: 'Passphrase action',
        values: [
          { v: '', label: 'The button decides' },
          { v: 'set', label: 'Set it' },
          { v: 'change', label: 'Change it' },
          { v: 'verify', label: 'Verify it' },
        ],
      },
      ACCOUNTS_CONTROL,
      DEVLIST_CONTROL,
  ...
  M.page({
    ...
    render: function (s) {
      ...
      if (t) return t;
      return {
        main: M.settingsFrame(
          s,
          'about',
          sectionBody(s, aboutBody, false).body,
        ),
      };
    },
  ...
  M.page({
        ...
        values: [
          {
            v: 'acct:nope',
            label: 'acct:nope',
            hint: 'no such store in the catalog — UnavailableAccount.',
          },
          {
            v: 'acct:personal',
            label: 'personal · foks.example.net',
            hint: 'resolves, so the pane is the ordinary MacsSection.',
          },
          {
            v: 'acct:work',
            label: 'work · foks.acme-corp.com',
            hint: 'ditto, for the Acme account.',
          },
        ],
      },
      ACCOUNTS_CONTROL,
    ...
    render: function (s) {
      ...
      if (t) return t;
      return {
        main: M.settingsFrame(s, 'macs', sectionBody(s, macsBody).body),
      };
    },
