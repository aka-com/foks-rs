...
(function () {
  ...
  var GO_CANDIDATES = [
    {
      candidateId: 'rae',
      username: 'rae',
      serverHint: 'foks.example.net',
      hostId:
        '0231c2aa07e4b1d86c3f52a09e7d41c8b6f0a2d3e95c17b48f6a0d2e3c5b719a4f',
      userId:
        '0138a4c95e77b21d6f0a3c8b5d2e9f14a7b0c3d6e9f2a5b8c1d4e7f0a3b6c9d2e5',
      deviceId:
    ...
    },
    {
      ...
      hostId:
        '024d17e390a5c2f8e16d7b39c04a8e5f2d1c6b7a9038e4f5d6c1a2b3e7f0948d1c',
      userId:
        '01c4f70b2ae35d1986c0f4a72b5d8e13c6a9f0b3d6e9c2f5a8b1d4e7f0a3c6b9d2',
      deviceId:
        '02c1e08d5f3a94b7d21e6f0c8a3b5d7e9f1a2b3c4d5e6f708192a3b4c5d6e7f809',
      role: 'Owner',
      storageKind: 'passphrase',
      hidden: false,
      provisional: false,
      pairable: true,
      copyable: false,
    },
    {
      candidateId: 'draft',
      username: null,
      serverHint: 'foks.partner.dev',
      hostId:
        '0204c8b19e3f6a2d75c081b4e9f3a6d2c5b8e1f4a7d0c3b6e9f2a5d8c1b4e7f0a3',
      userId:
        '019e2b7f4c1a8d05e3b6f9c2a5d8e1b4f7a0c3d6e9b2f5a8c1d4e7b0f3a6c9d2e5',
      deviceId:
        '0217c4e9b2f5a8d1c4e7f0a3b6c9d2e5f8a1b4c7d0e3f6a9b2c5d8e1f4a7b0c3d6',
      role: 'Member',
      storageKind: 'noise-file',
      hidden: false,
      provisional: true,
      pairable: false,
      copyable: false,
  ...
  function candidateOf(s) {
    ...
    if (!id) return null;
    return (
      GO_CANDIDATES.filter(function (c) {
        return c.candidateId === id;
      })[0] || null
    );
  }
  ...
  function localReady(s) {
    return s.v.result ? s.v.result === 'ready' : s.managed === 'local';
  }
  ...
  function adminOf(f) {
    return f.admin || 'the group admin';
  }
  function adminShortOf(f) {
    return adminOf(f).split('.')[0];
  }
  ...
  function input(o) {
    return (
      '<input' +
      at('aria-label', o.ariaLabel) +
      at('placeholder', o.placeholder) +
      (o.spellcheck === false ? ' spellcheck="false"' : '') +
      (o.id ? ' id="' + esc(o.id) + '"' : '') +
      (o.type ? ' type="' + esc(o.type) + '"' : '') +
      (o.disabled ? ' disabled=""' : '') +
      ' value="' +
      esc(o.value == null ? '' : o.value) +
      '" style=""' +
  ...
  function setupSide(o) {
    ...
    if (o.managedNoAccount) {
      foot += navBtn(
        'server',
        'Use another server',
        'data-act="go" data-page="fr-address"',
      );
      foot += navBtn(
        'person',
        'Recover account',
        'data-act="go" data-page="frx-existing" data-set=\'{"v.back":"local","v.path":"own","v.recovery":"","v.phrase":"","v.refusal":""}\'',
        !o.recoverEnabled,
      );
    }
    ...
    return (
      '<nav class="side setup-side" aria-label="Setting up">' +
  ...
  function statusSlot(o) {
    return (
      UI.sectionLabel('Status', { as: 'side' }) +
      (o.left < 5
        ? M.navRow({
            active: true,
            glyph: M.icon('flag'),
  ...
  function pane(o) {
    return (
      '<main class="main first-run-main">' +
      (o.header
        ? UI.pageHeader({ title: o.header.title, subtitle: o.header.subtitle })
        : '') +
      '<div class="body pb"><div class="pane' +
      (o.wide ? ' wide' : '') +
      '">' +
      (o.body || '') +
      '</div></div>' +
  ...
  function dd(o) {
    ...
    return (
      ...
      (open ? ' open=""' : '') +
      '>' +
      '<summary data-act="set" data-key="' +
      esc(o.key || 'v.disclosure') +
      '" data-val="' +
      esc(open ? '' : o.val || 'open') +
      '">' +
      esc(o.summary) +
    ...
    controls: [
      {
        key: 'path',
        label: 'Seeded path',
        note: 'Only a boot-seeded entry (`?state=boot&path=…`) brings the checkpoint path into the fork; a plain `who` ignores it and reads the pick.',
        values: [
          { v: '', label: 'None' },
          { v: 'own', label: 'Own' },
      ...
      {
        key: 'init',
        label: 'Local client state',
        note: 'checkpoint.initialized. `?state=boot` and a fresh native launch land here with it false: Continue is disabled and reads Initializing... while initializeClientState runs in the background, and the agent pill still says starting. A failure puts the agent’s sentence in a `.crit` alert above the options with a small Try again that re-runs it (first-run-screen.tsx, who pane).',
        values: [
          { v: '', label: 'Ready' },
          { v: 'initializing', label: 'Initializing' },
          { v: 'error', label: 'Helper failed' },
        ],
      },
      {
        key: 'choice',
        label: 'Picked',
        note: 'pendingPath — screen state, not the checkpoint. `who` is the one step that ignores checkpoint.path entirely: SetupSidebar reads the pick instead (first-run-screen.tsx:281-283), so this page declares no `path`.',
        values: [
          { v: '', label: 'Nothing' },
          { v: 'own', label: 'On my own' },
          { v: 'invited', label: 'Invited' },
        ],
      },
      {
        key: 'refusal',
        label: 'Go CLI scan',
        note: 'discoverGoProfiles() rejected, so the fork is drawn with the scan error above the options rather than the chooser (first-run-screen.tsx:1806-1810). Native only — the mock bridge answers {installed:false}, so no capture has it.',
        values: [
          { v: '', label: 'No error' },
          { v: 'scan', label: 'Scan failed' },
        ],
      },
    ],
    render: function (s) {
      ...
      var opts = JOINING.map(function (o, i) {
        ...
        return (
          ...
          (on ? 'true' : 'false') +
          '" tabindex="' +
          (on || (picked === null && i === 0) ? '0' : '-1') +
          '" class="' +
          (on ? 'opt on' : 'opt') +
          '"' +
          ' data-act="set" data-key="v.choice" data-val="' +
          o.path +
          '">' +
          '<span class="pip" aria-hidden="true"></span>' +
          M.icon(o.icon, null, { cls: 'ic' }) +
          '<span class="txt"><span class="ptitle">' +
          esc(o.title) +
          '</span>' +
          '<span class="bd">' +
          esc(o.detail) +
      ...
      var body =
        ...
        (failed
          ? '<div class="crit" role="alert"><p>Setup stopped before this Mac was ready: The agent did not become ready after initialization.</p>' +
            UI.btn('Try again', {
              size: 'sm',
              attrs: 'data-act="set" data-key="v.init" data-val=""',
            }) +
            '</div>'
          : '') +
        '<div class="opts" role="radiogroup" aria-label="How are you joining">' +
        opts +
        '</div>' +
        next +
        '<div class="actions">' +
          ...
          attrs:
            picked && !initializing
              ? 'data-act="go" data-page="' +
                (picked === 'invited' ? 'fri-address' : 'fr-address') +
                '" data-set=\'{"v.path":"' +
                picked +
                '","v.choice":"","v.returning":"","v.card":"","v.refusal":"","v.made":"","v.recovery":"","v.phrase":""}\''
              : '',
        }) +
      ...
      return {
        ...
        titlebar: initializing
          ? M.renderTitlebar({ agent: 'starting', refreshing: s.refreshing })
          : undefined,
        ...
        side: setupSide({
          current: 0,
          path: picked || (initializing ? s.v.path || null : null),
        }),
      ...
      ? refusal === 'bad'
        ...
        : refusal === 'mismatch'
          ...
          : (address || 'The empty address') +
            ' did not answer, so nothing was saved'
      : null;
    var body =
      '<h1>' +
      (invited ? 'Select a server address' : 'Select a server') +
      '</h1>' +
      '<p class="lead">' +
      SERVER_LEAD +
      ' ' +
      (invited
        ? 'Enter the server address from ' + esc(adminShortOf(f)) + '.'
        : '') +
      '</p>' +
      ...
      (message
        ? '<div class="crit"><b>' +
          esc(message) +
          '</b>FOKS could not connect to this address. Check the address and your network connection, then try again. No changes were saved.</div>'
        : '') +
      (invited && state === 'no-address'
        ? '<div class="pcard"><h3>Ask ' +
          esc(adminShortOf(f)) +
          ' this</h3>' +
        ...
        : invited
          ? '<p class="hint">' +
            lnk(
              'They sent nothing?',
              'data-act="go" data-page="fri-no-address"',
            ) +
            '</p>'
          : '');
    return {
  ...
  var KNOWN_SERVERS = {
    'foks.example.net': 'own',
    'foks.acme-corp.com': 'invited',
  };
  M.fns.frCheck = function (arg) {
    var path = arg === 'invited' ? 'invited' : 'own';
    var field = document.querySelector(
      '#frame input[aria-label="Server address"], #frame .checked-address input',
    );
    var value = field ? field.value.trim() : '';
    if (!value) {
      M.go(path === 'invited' ? 'fri-address' : 'fr-address', {
        'v.path': path,
        'v.refusal': 'empty',
      });
      return;
    ...
    M.go(path === 'invited' ? 'fri-checked' : 'fr-checked', {
      'v.refusal': '',
      'v.probe': value,
    });
  };
  ...
  var ADDRESS_CONTROLS = [
    {
      key: 'refusal',
      label: 'Refusal',
      note: 'The client-side refusal an empty address raises: .inset.err with no .crit, still on `address`. Clearing the field and pressing the button reaches it too.',
      values: [
        { v: '', label: 'None' },
        { v: 'empty', label: 'Empty address' },
      ],
    },
    {
      key: 'returning',
      label: 'Returning',
      note: 'checkpoint.returning — the "You already have an account on this server." hint, and Continue routes to `existing`.',
      values: [
        { v: '', label: 'First time' },
        { v: 'yes', label: 'Returning' },
      ],
    },
    {
      key: 'result',
      label: 'Check result',
      note: 'What the next check answers: acceptance on `checked`, or a failure that lands on `error`.',
      values: [
        { v: 'new', label: 'Inserted' },
  ...
  M.page({
    ...
    controls: [
      ...
      },
      {
        key: 'refusal',
        label: 'Message',
        note: 'The .crit headline: the client fallback, or one of the two mock bridge refusals (both of which end in a full stop) — `did not answer` from an ordinary check, `did not match` when a Go candidate’s host ID is not the one that answered.',
        values: [
          { v: '', label: 'Fallback' },
          { v: 'bad', label: 'Did not answer' },
          { v: 'mismatch', label: 'Did not match' },
        ],
      },
      {
        key: 'returning',
  ...
  function checkedPane(s, path) {
    ...
    var facts_ =
      '<div class="facts">' +
      '<div><span class="k">Lookup name</span><code>' +
      esc(report.lookupName) +
      '</code></div>' +
      '<div><span class="k">Confirmed name</span><code>' +
      esc(report.canonicalName) +
      '</code></div>' +
      '<div><span class="k">Chain sequence</span><span>' +
      report.chain +
      '</span></div>' +
      '<div><span class="k">Merkle epoch</span><span>' +
      report.epoch.toLocaleString('en-US') +
      '</span></div>' +
      '<div class="wide"><span class="k">Host ID</span><code>' +
      report.hostId.match(/.{1,4}/g).join(' ') +
      '</code></div>' +
      '</div>';
    var dbody =
      facts_ +
      ...
      UI.toggle({
        ...
        open: s.v.disclosure === 'inspect',
        body: '<pre>' + esc(JSON.stringify(report, null, 1)) + '</pre>',
        attrs:
          'data-act="set" data-key="v.disclosure" data-val="' +
          (s.v.disclosure === 'inspect' ? 'open' : 'inspect') +
          '"',
      });
    var body =
      ...
      esc(report.canonicalName) +
      ' answered ' +
      UI.chip('Pinned on this Mac') +
      '</h3>' +
      '<p>Saved as <b>' +
      esc(report.canonicalName) +
      '</b>. The host ID is pinned on this Mac.</p>' +
      '<details class="dd"' +
      (open ? ' open=""' : '') +
      '>' +
      '<summary data-act="set" data-key="v.disclosure" data-val="' +
      (open ? '' : 'open') +
      '">Details</summary>' +
      '<div class="dbody">' +
      dbody +
    ...
    var cont =
      s.v.returning === 'yes'
        ? 'data-act="go" data-page="frx-existing" data-set=\'{"v.back":"checked","v.path":"' +
          path +
          '","v.recovery":"","v.phrase":"","v.refusal":""}\''
        : 'data-act="go" data-page="' +
          (path === 'invited' ? 'fri-account' : 'fr-account') +
          '"';
    return {
      side: setupSide({ current: 1, path: path }),
      main: pane({
        ...
        foot: foot({
          back:
            'data-act="go" data-page="' +
            (path === 'invited' ? 'fri-address' : 'fr-address') +
            '"',
  ...
  var CHECKED_CONTROLS = [
    {
      key: 'disclosure',
      label: 'Disclosure',
      note: 'Closed = `checked`; open = `compare` (the same screen with the Details block open); inspect also opens the raw response.',
      values: [
        { v: '', label: 'Closed' },
        { v: 'open', label: 'Compare' },
        { v: 'inspect', label: 'Inspect' },
      ],
    },
    {
      key: 'result',
      label: 'Acceptance',
      note: 'The `acceptance` the probe returned, visible in the inspected response.',
      values: [
        { v: 'new', label: 'Inserted' },
        { v: 'same', label: 'Unchanged' },
        { v: 'advanced', label: 'Advanced' },
    ...
    },
    {
      key: 'returning',
      label: 'Returning',
      note: 'Continue goes to `existing` instead of `account`.',
      values: [
        { v: '', label: 'First time' },
        { v: 'yes', label: 'Returning' },
      ],
    ...
    var body =
      ...
      UI.inset(
        UI.insetRow({
          label: 'Username',
          forId: u,
          value: input({ id: u, value: uname, bind: 'v.username' }),
        }) +
          UI.insetRow({
            label: 'This Mac’s name',
            forId: dev,
            value: input({ id: dev, value: dname, bind: 'v.dev' }),
          }),
      ) +
      '</div>' +
      '<div>' +
      UI.sectionLabel('Optional') +
      UI.inset(
        UI.insetRow({
          label: 'Email (optional)',
          forId: mail,
          value: input({ id: mail, placeholder: 'you@example.net', value: '' }),
        }) +
          UI.insetRow({
            label: 'Invite (optional)',
            forId: inv,
            value: input({ id: inv, value: '' }),
          }),
      ) +
      '</div></div>' +
      ...
      (s.v.refusal === 'resume'
        ? '<p class="crit">That account setup is no longer pending.</p>'
        : '') +
      lnk(
        'I already have an account on this server',
        'data-act="go" data-page="frx-existing" data-set=\'{"v.back":"account","v.path":"' +
          path +
          '","v.recovery":"","v.phrase":"","v.refusal":""}\'',
    ...
    return {
      ...
      main: pane({
        ...
        foot: foot({
          back:
            'data-act="go" data-page="' +
            (path === 'invited' ? 'fri-checked' : 'fr-checked') +
            '"',
          children: UI.btn(
            made
              ? 'Continue'
                ...
                : 'Create my account',
            {
              ...
              attrs: live
                ? 'data-act="go" data-page="' +
                  protectPage +
                  '" data-set=\'{"v.written":"","v.refusal":"","v.made":"yes"}\''
                : '',
            },
          ),
        }),
  ...
  var ACCOUNT_CONTROLS = [
    {
      ...
      note: 'listPendingOperations reports a matching account-signup row with no target: the primary becomes `Resume account setup`.',
      values: [
        { v: '', label: 'None' },
        { v: 'yes', label: 'Resumable' },
      ],
    },
    {
      key: 'made',
      label: 'Account exists',
      note: 'checkpoint.account — set once the account is created, so `‹ Back` from Save recovery phrase returns to a pane whose primary reads `Continue` and does nothing but go on. The managed-local pane also disables its fields.',
      values: [
        { v: '', label: 'Not yet' },
        { v: 'yes', label: 'Already created' },
      ],
    },
    {
      key: 'refusal',
      label: 'Refusal',
      note: 'The `.crit` this pane can carry: the bridge’s answer when a resumed account signup is no longer pending (mock-bridge.ts:761-765).',
      values: [
        { v: '', label: 'None' },
        { v: 'resume', label: 'Not pending' },
      ],
    },
    {
      key: 'username',
      label: 'Username',
      freeText: true,
      note: 'What is typed into the Username field. `Create my account` is refused while it or This Mac’s name is empty — type the field empty, or use the “(cleared)” chip, which is how a chip writes an empty string.',
      values: [
        { v: '', label: 'the fixture username' },
        { v: 'rae.chen', label: 'rae.chen' },
        { v: EMPTY, label: '(cleared)' },
      ],
    },
    {
      key: 'dev',
      label: 'This Mac’s name',
      freeText: true,
      note: 'The device-name field, the pane’s other required one.',
      values: [
        { v: '', label: 'the fixture name' },
        { v: 'Studio Mac', label: 'Studio Mac' },
        { v: EMPTY, label: '(cleared)' },
      ],
    },
  ];
  ...
  function protectPane(s, path, sheetOpen) {
    ...
    var written =
      !sheetOpen && (s.v.written === '1' || s.v.written === 'saved');
    ...
    var pass = rr(),
      conf = rr();
    var passValue = s.v.pass || '',
      confValue = s.v.conf || '';
    var mismatch = passValue !== confValue;
    var body =
      '<h1>Save recovery phrase</h1>' +
      '<p class="lead">Right now this Mac holds the only key to ' +
      esc(f.username) +
      ...
      '</div>' +
      '<div class="pcard"><h3>YubiKey ' +
      UI.chip('Later') +
      '</h3>' +
      '<p>Enroll a hardware key later from <b>Settings › Security keys</b>.</p>' +
      dd({
        open: s.v.disclosure === 'open',
        summary: 'What to know first',
        body:
          '<div class="dbody"><ol>' +
      ...
      UI.btn('Enroll a YubiKey…', {
        attrs: 'data-act="set" data-key="v.refusal" data-val="yubikey"',
      }) +
      '</div>' +
      ...
      UI.btn('Show my phrase', {
        ...
        attrs: spent
          ? ''
          : 'data-act="go" data-page="fr-phrase" data-set=\'{"v.path":"' +
            path +
            '","v.written":"' +
            (written ? '1' : '') +
            '"}\'',
      ...
      (s.v.refusal === 'yubikey'
        ? '<p class="crit">A YubiKey is enrolled later from Settings › Security keys.</p>'
        : s.v.refusal === 'commit'
          ? '<p class="crit">The prepared backup phrase no longer matches.</p>'
          : '');
    return {
  ...
  var PROTECT_CONTROLS = [
    {
      key: 'written',
      label: 'Backup phrase',
      note: 'checkpoint.backupCommitted. `Committed` is the sheet’s Done just pressed: the phrase is still held in memory until this step is left, so Show my phrase stays live for another look (82fa1e4b7). `Committed earlier` is a stored checkpoint with nothing held: the button is disabled.',
      values: [
        { v: '', label: 'Not written' },
        { v: '1', label: 'Committed' },
        { v: 'saved', label: 'Committed earlier' },
      ],
    },
    {
      key: 'disclosure',
      label: 'Disclosure',
      note: 'The YubiKey card’s `What to know first`.',
      values: [
        { v: '', label: 'Closed' },
        { v: 'open', label: 'What to know' },
      ],
    },
    {
      key: 'refusal',
      label: 'Message',
      note: '`Enroll a YubiKey…` sets a message rather than doing anything; the sheet’s `Done` can also land one here, when commitOwnerBackup refuses the phrase it was handed (mock-bridge.ts:774-779).',
      values: [
        { v: '', label: 'None' },
        { v: 'yubikey', label: 'YubiKey notice' },
        { v: 'commit', label: 'Phrase no longer matches' },
      ],
    },
    {
      key: 'pass',
      label: 'Passphrase',
      note: 'What is typed in the Passphrase field. Continue is disabled while it and Confirm disagree; both empty is the app\u2019s default and goes on without one.',
      values: [
        { v: '', label: 'Empty' },
        { v: 'hunter2', label: 'Typed' },
      ],
    },
    {
      key: 'conf',
      label: 'Confirm',
      note: 'The Confirm field. Set one and not the other to see the refusal.',
      values: [
        { v: '', label: 'Empty' },
        { v: 'hunter2', label: 'Typed' },
      ],
  ...
  M.page({
    ...
    controls: [
      {
        key: 'ticked',
        label: 'Checkbox',
        note: 'phraseWritten — `Done` stays disabled until the box is ticked. Ticking is not committing: `written` (checkpoint.backupCommitted) only flips when Done runs, and the box opens clear every time.',
        values: [
          { v: '', label: 'Not ticked' },
          { v: '1', label: 'Written down' },
        ],
      },
      {
        key: 'result',
        label: 'Prepared phrase',
        note: 'prepareOwnerBackup runs in an effect when the sheet opens; until it answers there are no tokens to show and `Done` is refused whatever the checkbox says (first-run-screen.tsx:2665-2674).',
        values: [
          { v: '', label: 'Prepared' },
          { v: 'preparing', label: 'Still preparing' },
        ],
      },
      {
        key: 'path',
    ...
    overlay: function (s) {
      ...
      var close =
        'data-act="go" data-page="' +
        back +
        '" data-set=\'{"v.ticked":"","v.written":"' +
        (s.v.written ? esc(s.v.written) : '') +
        '"}\'';
      var words = prepared
        ? '<div class="words">' +
          D.backupPhraseWords
            .map(function (w, i) {
              return (
                '<div class="word"><i>' + (i + 1) + '</i>' + esc(w) + '</div>'
              );
            })
            .join('') +
          '</div>'
      ...
      return UI.sheet({
        ...
        backdropAttrs: close,
        body:
          '<p>Anyone with these words can access your account. Store them somewhere other than this Mac.</p>' +
          words +
          '<button type="button" aria-label="I have written these 17 words down" class="check' +
          (ticked ? ' on' : '') +
          '"' +
          ' data-act="set" data-key="v.ticked" data-val="' +
          (ticked ? '' : '1') +
          '">' +
          '<span class="bx">' +
          (ticked ? '✓' : '') +
          '</span>I have written these 17 words down</button>',
        footer:
          UI.btn('Not now', { attrs: close }) +
          UI.btn('Done', {
            variant: 'primary',
            disabled: !ticked || !prepared,
            attrs:
              ticked && prepared
                ? 'data-act="go" data-page="' +
                  back +
                  '" data-set=\'{"v.written":"1","v.ticked":""}\''
                : '',
          }),
      });
    },
    after: function () {
      ...
      M.fns.escape = function () {
        ...
        M.go(M.s.v.path === 'invited' ? 'fri-protect' : 'fr-protect', {
          'v.ticked': null,
        });
      };
    },
    ...
    controls: [
      {
        key: 'disclosure',
        label: 'Disclosure',
        note: 'The `Details` block under the Check now card.',
        values: [
          { v: '', label: 'Closed' },
          { v: 'open', label: 'Details open' },
        ],
      },
      {
        key: 'result',
        label: 'Check answers',
        note: 'What the next `Check now` finds: the group (→ `added`), or an answer discover() will not act on, which only sets a message (first-run-screen.tsx:1443-1473).',
        values: [
          { v: '', label: 'Engineering' },
          { v: 'ambiguous', label: 'Not listed yet' },
          { v: 'no-store', label: 'Store not ready' },
          { v: 'other-account', label: 'Different account' },
        ],
      },
      {
        key: 'checked',
        label: 'Checked',
        note: 'Whether a check has already run — the chip reads `Checked just now` and the `.res` block echoes the message.',
        values: [
          { v: '', label: 'Not checked yet' },
          { v: 'yes', label: 'Checked' },
        ],
      },
    ],
    render: function (s) {
      ...
      var WAIT_MESSAGES = {
        ambiguous:
          'Checked just now — ' +
          f.groupName +
          ' is not listed unambiguously yet.',
        'no-store':
      ...
      var message =
        (s.v.checked === 'yes' && WAIT_MESSAGES[s.v.result]) || null;
      ...
      var sentence =
        ...
        f.username +
        ' on ' +
        f.report.canonicalName +
        ' to ' +
        f.groupName;
      var left =
        '<div class="col">' +
        '<div class="pcard"><h3>Send ' +
        esc(short) +
        ' this</h3>' +
        UI.copyBox({
          display: '“' + esc(sentence) + '”',
          attrs: 'data-act="toast" data-text="Sentence copied."',
        }) +
        '<p>' +
        esc(short) +
        ' needs your username exactly as written. Nothing else — no code, no link.</p></div>' +
        UI.inset(
          checkRow(
            M.icon('people'),
            esc(f.groupName) + ' will appear under GROUPS',
            'FOKS lists groups after it checks the server.',
          ) +
            checkRow(
              M.icon('eye'),
              'You’ll see what your role lets you read',
      ...
      var right =
        ...
        '<span class="status">' +
        UI.chip(message ? 'Checked just now' : 'Not checked yet') +
        (message ? 'Not yet — only Personal is listed.' : '') +
        '</span></div>' +
        ...
        UI.band({
          ...
          text: '<b>Check now</b> searches for groups using your authenticated account. FOKS also checks when it opens.',
        }) +
        (message ? '<div class="res">' + esc(message) + '</div>' : '') +
        '<p class="note">You can use <b>Personal</b> while you wait. If ' +
        esc(short) +
        ' has already added you, confirm that they entered <code>' +
        esc(f.username) +
        '</code>.</p>' +
        dd({
          open: s.v.disclosure === 'open',
          summary: 'Details',
          body:
            '<p>Checks your account (' +
            esc(f.username) +
            ') on ' +
            esc(f.report.canonicalName) +
            ' and updates your group list.</p>',
        }) +
        '</div>' +
        '<div class="pcard"><h3>Use your Personal vault</h3>' +
        '<p>' +
        PERSONAL_FIXED +
        ' Put your own logins in <b>Personal</b> now; nothing in it is visible to ' +
        esc(f.groupName) +
        '.</p>' +
        ...
        UI.btn('Open Personal', {
          attrs:
            'data-act="go" data-page="fri-checklist-invited" data-set=\'{"v.progress":"4"}\'',
        }) +
        '</div></div>';
      var body =
        '<h1>Waiting for ' +
  ...
  M.fns.frCreateGroup = function () {
    ...
    var alias = name
      ...
      .replace(/^-+|-+$/g, '');
    if (!alias) {
      M.set('v.refusal', 'name');
      return;
    }
    if (TAKEN_GROUP_ALIASES.indexOf(alias) >= 0) {
      M.set('v.refusal', 'alias');
      return;
    }
    ...
    controls: [
      {
        key: 'refusal',
        label: 'Refusal',
        note: 'The client refuses a name with no letters or numbers; the bridge refuses an alias `team:<alias>` already holds — which the prefilled `Household` is.',
        values: [
          { v: '', label: 'None' },
          { v: 'alias', label: 'Alias exists' },
          { v: 'name', label: 'No letters or numbers' },
        ],
      },
    ],
    render: function (s) {
      ...
      var body =
        ...
        '<p class="lead">' +
        PERSONAL_FIXED +
        ' Start one now, or skip and do it later from <b>Groups</b>.</p>' +
        UI.sectionLabel('Your first group') +
        UI.inset(
          UI.insetRow({
            label: 'Group name',
            forId: nameId,
            value: input({ id: nameId, value: f.groupName }),
          }),
        ) +
        (s.v.refusal === 'alias'
          ? '<p class="crit">That group alias already exists.</p>'
          : s.v.refusal === 'name'
            ? '<p class="crit">Enter a group name containing letters or numbers.</p>'
            : '') +
        /* First run makes named groups only (953b05785): the kind radio is
             gone and the hint says what a name is for. */
        '<p class="hint">Others on <b>' +
        esc(f.report.canonicalName) +
        '</b> can look up this group by name. You can add people by username after creating a group.</p>';
      return {
        ...
        main: pane({
          ...
          foot: foot({
            ...
            children:
              lnk(
                'Skip this step',
                'data-act="go" data-page="fr-checklist-own" data-set=\'{"v.progress":"4","v.refusal":""}\'',
              ) +
              UI.btn('Create group', {
                variant: 'primary',
                attrs: 'data-act="call" data-fn="frCreateGroup"',
              }),
          }),
        }),
      };
  ...
  var SOL_PARTY = {
    ...
    party_id_hex:
      '01' + '6666666666666666666666666666666666666666666666666666666666666666',
    source_role: { role: 'Member', visibility: 0 },
    destination_role: { role: 'Member', visibility: 0 },
    name: 'sol',
    initials: D.initials('sol'),
    hue: D.hue('sol'),
  };
  ...
  function groupPane(s, page, state) {
    ...
    var store =
      found && found.active !== false && found.readable ? found : null;
    var items = store
      ? w.items.filter(function (i) {
          return i.store === storeId;
        })
      : [];
    var openAttrs = store
      ? 'data-act="go" data-page="' + esc(M.storePages[storeId]) + '"'
      : '';
    var notice = UI.notice({
      ...
      body: invited
        ? '<p>' +
          ...
          ' added <code>' +
          esc(f.username) +
          '</code> as a Member, and ' +
          esc(f.groupName) +
          ' is listed here now: its ' +
          items.length +
        ...
        : '<p>You are the group Owner. To invite others, share the server address (' +
          esc(f.report.canonicalName) +
      ...
      actions:
        UI.btn('Dismiss', { attrs: 'data-act="go" data-page="all"' }) +
        UI.btn('Open your vault', {
          variant: 'primary',
          disabled: !store,
          attrs: store ? openAttrs : '',
        }),
    });
    var rest;
    if (invited) {
      rest =
        '<div class="hdr"><span></span><span>Name</span><span>Readable by</span><span>Version</span><span></span></div>' +
        items
          .slice(0, 4)
          .map(function (item) {
            return (
              '<div class="row">' +
              UI.kindIcon(D.kindOf(item)) +
              '<span class="name">' +
              esc(D.nameOf(item.path)) +
              '<small>' +
              esc(item.path) +
              '</small></span>' +
              '<span>' +
              UI.chip(esc(D.readableBy(item).label)) +
              '</span>' +
              '<span class="n">v' +
              esc(item.version) +
              '</span><span></span></div>'
      ...
      rest =
        ...
        UI.btn('New', { attrs: 'data-act="go" data-page="all"' }) +
        '</div>';
    }
    return {
  ...
  function addedDetails(s, storeId) {
    ...
    if (!item)
      return '<aside class="details"><div class="dh"><span class="t"><h2>Details</h2></span></div></aside>';
    ...
    return (
      ...
      ' bytes</span>' +
      '<b>Read role</b>' +
      UI.chip(esc(D.formatRole(item.read) || item.read)) +
      '<b>Write role</b>' +
      UI.chip(esc(D.formatRole(item.write) || item.write)) +
      '</div>' +
    ...
    controls: [
      {
        key: 'discovered',
        label: 'Roster',
        note: 'Reached by pressing `Check now`, discovery has already made `sol` a Member of Engineering, so the roster is 6 people; a deep link to `?state=added` has not run it and shows 5.',
        values: [
          { v: '', label: 'Deep link (5)' },
          { v: 'yes', label: 'After Check now (6)' },
        ],
      },
    ],
    status: function () {
      return statusSlot({ left: 5, checklist: 'fri-checklist-invited' });
    },
    render: function (s) {
      var t = M.globalTakeover(s);
      if (t) return t;
      rid = 0;
      return withDiscovery(s, function () {
        return groupPane(s, M.pages['fri-added'], 'added');
      });
    },
  });
  M.page({
    ...
    controls: [
      {
        key: 'group',
        label: 'Group',
        note: 'checkpoint.group.name: the fixture group on a deep link, or the name typed on `create-group` — `Platform`, the same group 60-groups’ create sheet makes.',
        values: [
          { v: '', label: 'Household (fixture)' },
          { v: 'Platform', label: 'Platform (created)' },
        ],
      },
    ],
    status: function () {
      return statusSlot({ left: 5, checklist: 'fr-checklist-own' });
    },
    render: function (s) {
      var t = M.globalTakeover(s);
      if (t) return t;
  ...
  function checklistPane(s, path, page) {
    ...
    var row5Actions =
      ...
      UI.btn(invited ? 'Check now' : 'Create a group…', {
        ...
        attrs:
          'data-act="go" data-page="' +
          (invited ? 'fri-waiting' : 'fr-create-group') +
          '"',
      }) +
      '</span>';
    var body =
      ...
      UI.inset(
        checkRow(
          ...
          checkRow(
            '✓',
            invited ? 'Their server' : 'A server',
            '<code>' +
              esc(f.report.canonicalName) +
              '</code> · host ID <code>' +
              esc(f.report.hostId.slice(0, 10)) +
              '…</code> · checked and pinned on this Mac.',
          ) +
          checkRow(
            '✓',
            'Your account',
          ...
          checkRow(
            invited ? '5' : '!',
            invited
              ? 'Waiting for ' + esc(adminOf(f)) + ' to add you'
              : 'Create a group',
            invited
              ? esc(f.groupName) +
                  ' isn’t listed yet. Check again after ' +
                  esc(adminShortOf(f)) +
                  ' adds you, or send them the sentence above.'
              : PERSONAL_FIXED +
                  ' You become its Owner and add people by username from the group’s settings.',
            row5Actions,
            invited
              ? UI.band({
                  label: 'Group discovery',
                  text: 'Check now searches for groups using your authenticated account.',
                })
              : '',
          ),
        { className: 'checklist' },
  ...
  var CHECKLIST_CONTROLS = [
    {
      key: 'progress',
      label: 'Steps done',
      note: 'completedFirstRunSteps: initialized + profile + account + (passphrase | backup) + (added | group). Skipped work does not count.',
      values: [
        { v: '3', label: '3 done' },
        { v: '4', label: '4 done' },
      ],
  ...
  M.page({
    ...
    status: function (s) {
      return statusSlot({
        ...
        note:
          FACTS.invited.groupName +
          ' will appear under Groups once you’re added. FOKS checks for new groups when it opens and when you click Check now.',
      });
    },
    controls: CHECKLIST_CONTROLS,
    render: function (s) {
    ...
    controls: [
      {
        ...
        note: 'sharedServerStatus: `Checking` until the report lands, `Ready` once it does, or the error block when the profile is not usable. Unset it and the app-wide `Managed profile` key decides — the mock bridge names none, which is why the capture sits at Checking.',
        values: [
          { v: '', label: 'From `managed`' },
          { v: 'ready', label: 'Report ready' },
          { v: 'checking', label: 'Still checking' },
          { v: 'error', label: 'Not usable' },
        ],
      },
    ],
    render: function (s) {
      ...
      var body =
        ...
        '<span class="local-server-mark" aria-hidden="true">' +
        M.icon('server') +
        '</span>' +
        '<span class="local-server-title"><b>Local server</b><small>On this Mac</small></span>' +
        '<span class="local-ready"><i></i>' +
        (ready ? 'Ready' : 'Checking') +
        '</span></div>' +
        '<dl class="local-server-facts"><dt>Address</dt><dd><code>' +
        esc(LOCAL.probe) +
        '</code></dd>' +
        ...
        (s.v.result === 'error'
          ? '<div class="crit"><b>Local server unavailable</b>The managed local server is not ready.' +
            '<div class="btns">' +
            UI.btn('Check again', {
              attrs: 'data-act="set" data-key="v.result" data-val="ready"',
            }) +
            '</div></div>'
          : '') +
        '<div class="local-alternates">' +
        UI.sectionLabel('Other ways to begin') +
        '<div class="btns">' +
        UI.btn('Connect to another server…', {
          attrs:
            'data-act="go" data-page="fr-address" data-set=\'{"v.path":"own"}\'',
        }) +
      ...
      return {
        side: setupSide({
          local: true,
          current: 0,
          managedNoAccount: true,
          recoverEnabled: ready,
        }),
        main: pane({
          body: body,
          foot: foot({
            children: UI.btn('Continue', {
              ...
              disabled: !ready,
              attrs: ready
                ? 'data-act="go" data-page="frl-account" data-set=\'{"v.made":"","v.disclosure":""}\''
                : '',
            }),
          }),
        }),
  ...
  M.page({
    ...
    controls: [
      {
        ...
        note: 'The <details class="local-more"> holding Email and Invite.',
        values: [
          { v: '', label: 'Closed' },
          { v: 'open', label: 'Expanded' },
        ],
      },
      {
        key: 'pending',
        label: 'Pending operation',
        note: 'A matching account-signup row makes the primary read `Resume account setup`.',
        values: [
          { v: '', label: 'None' },
          { v: 'yes', label: 'Resumable' },
        ],
      },
      {
        key: 'made',
        label: 'Account exists',
        note: 'checkpoint.account — `‹ Back` from Recovery returns here: every field is disabled and the primary reads `Continue`.',
        values: [
          { v: '', label: 'Not yet' },
          { v: 'yes', label: 'Already created' },
        ],
      },
      {
        key: 'refusal',
        label: 'Refusal',
        note: 'The `.crit` between the fields and the recover link: the bridge’s answer when a resumed account signup is no longer pending.',
        values: [
          { v: '', label: 'None' },
          { v: 'resume', label: 'Not pending' },
        ],
      },
      {
        ...
        label: 'Username',
        freeText: true,
        note: 'The Username field. `Create my account` is refused while it or This Mac’s name is empty; both go disabled once the account exists.',
        values: [
          { v: '', label: 'rae' },
          { v: 'rae.chen', label: 'rae.chen' },
          { v: EMPTY, label: '(cleared)' },
    ...
    render: function (s) {
      ...
      var body =
        ...
        '<label class="local-field-row"><span>Username</span>' +
        input({
          value: uname,
          disabled: made,
          bind: made ? null : 'v.username',
        }) +
        '</label>' +
        '<label class="local-field-row"><span>This Mac’s name</span>' +
        input({ value: dname, disabled: made, bind: made ? null : 'v.dev' }) +
        '</label>' +
        '</div>' +
        '<details class="local-more"' +
        (open ? ' open=""' : '') +
        '>' +
        '<summary data-act="set" data-key="v.disclosure" data-val="' +
        (open ? '' : 'open') +
        '">More options</summary>' +
        '<label class="local-field-row"><span>Email (optional)</span>' +
        input({ placeholder: 'you@example.net', value: '', disabled: made }) +
        '</label>' +
        '<label class="local-field-row"><span>Invite (optional)</span>' +
        input({ value: '', disabled: made }) +
        '</label>' +
        '</details>' +
        (s.v.refusal === 'resume'
          ? '<p class="crit">That account setup is no longer pending.</p>'
        ...
        lnk(
          ...
          'data-act="go" data-page="frx-existing" data-set=\'{"v.back":"local-account","v.path":"own","v.made":"' +
            (made ? 'yes' : '') +
            '","v.recovery":"","v.phrase":"","v.refusal":""}\'',
          'local-recover-link',
        );
      return {
        ...
        side: setupSide({
          local: true,
          current: 1,
          managedNoAccount: !made,
          recoverEnabled: localReady(s),
        }),
        main: pane({
          body: body,
          foot: foot({
            ...
            children: UI.btn(
                  ...
                  : 'Create my account',
              {
                ...
                disabled: !live,
                attrs: live
                  ? 'data-act="go" data-page="frl-protect" data-set=\'{"v.disclosure":"","v.written":"","v.result":"","v.made":"yes"}\''
                  : '',
              },
            ),
          }),
    ...
    controls: [
      {
        key: 'disclosure',
        label: 'Phrase',
        note: 'state=protect (collapsed) vs state=phrase (the 17 words inline).',
        values: [
          { v: '', label: 'Collapsed' },
          { v: 'open', label: 'Shown' },
        ],
      },
      {
        key: 'written',
        label: 'Backup phrase',
        note: 'phraseWritten while the phrase is on screen — `Start using FOKS` stays disabled until the box is ticked. `Already committed` is checkpoint.backupCommitted instead: the collapsed card’s button then reads `Recovery phrase saved` and is spent (first-run-screen.tsx:2455-2463). Collapsing does not commit, so that only comes back with a stored checkpoint.',
        values: [
          { v: '', label: 'Not ticked' },
          { v: '1', label: 'Written down' },
          { v: 'saved', label: 'Already committed' },
        ],
      },
      {
        key: 'result',
        label: 'Prepared phrase',
        note: 'prepareOwnerBackup runs in an effect when the phrase is revealed; until it answers the card says so instead of listing tokens.',
        values: [
          { v: '', label: 'Prepared' },
          { v: 'preparing', label: 'Still preparing' },
        ],
      },
      {
        key: 'refusal',
        label: 'Refusal',
        note: 'The `.crit` under the cards: `Start using FOKS` commits the phrase, and the bridge refuses one it did not prepare (mock-bridge.ts:774-779).',
        values: [
          { v: '', label: 'None' },
          { v: 'commit', label: 'Phrase no longer matches' },
        ],
      },
      {
        key: 'returning',
        label: 'Returning',
        note: "checkpoint.returning — this pane’s `‹ Back` is `go(returning ? 'existing' : 'account')` (first-run-screen.tsx:2389), so after a recovery it returns to `existing` rather than to Create your account.",
        values: [
          { v: '', label: 'First time' },
          { v: 'yes', label: 'Recovered' },
        ],
      },
    ],
    render: function (s) {
      ...
      var card = shown
        ? (prepared
            ? '<div class="words">' +
              ...
              '</div>'
            : '<p>Preparing your phrase…</p>') +
          '<label class="local-confirm"><input type="checkbox"' +
          (ticked ? ' checked=""' : '') +
          ' style="" data-act="set" data-key="v.written" data-val="' +
          (ticked ? '' : '1') +
          '">' +
        ...
        : /* Collapsed the button is the reveal — or, once the phrase has been
          ...
          UI.btn(committed ? 'Recovery phrase saved' : 'Show recovery phrase', {
            ...
            attrs: committed
              ? ''
              : 'data-act="set" data-key="v.disclosure" data-val="open"',
          });
      var body =
        ...
        '<div class="local-recovery-head"><h2>Backup phrase</h2>' +
        UI.chip('Recommended') +
        '</div>' +
      ...
      return {
        ...
        main: pane({
          ...
          foot: foot({
            ...
            note: lnk(
              'Do this later',
              'data-act="go" data-page="frl-local-done"',
            ),
            ...
            children: UI.btn('Start using FOKS', {
              variant: 'primary',
              disabled: shown && (!ticked || !prepared),
              attrs:
                shown && (!ticked || !prepared)
                  ? ''
                  : 'data-act="go" data-page="frl-local-done"',
            }),
          }),
    ...
    controls: [
      {
        ...
        note: 'The preview body has three branches (first-run-screen.tsx:2513-2519) and the fixture only ever reaches the last: `Personal vault unavailable` when `addedStore` does not resolve, `No items yet` at zero, else plural(n, "item") — whose singular the fixture never shows either.',
        values: [
          { v: '', label: '7 items (fixture)' },
          { v: 'one', label: '1 item' },
          { v: 'yes', label: 'No items yet' },
          { v: 'gone', label: 'Store unresolved' },
        ],
      },
    ],
    render: function (s) {
      ...
      var store =
        s.v.empty === 'gone'
          ? null
          : w.storeById
            ? w.storeById['acct:personal']
            : null;
      var count = w.items.filter(function (i) {
        return i.store === 'acct:personal';
      }).length;
      ...
      var body =
        ...
        '<div class="local-vault-preview"><div class="local-vault-head">' +
        M.icon('vault') +
        '<b>Personal</b><code>' +
        esc(FACTS.own.report.canonicalName) +
        '</code></div>' +
  ...
  M.page({
    ...
    controls: [
      {
        key: 'card',
        label: 'Cards',
        note: 'The bridge behind the pane. Both cards are always drawn; only the recovery card’s `Account alias` row is native-only (first-run-screen.tsx:2222-2229), and a selected Go CLI candidate also retitles the pairing card — and adds the copy card, but only when that candidate is copyable, which the `Go CLI candidate` chips decide.',
        values: [
          { v: '', label: 'Web bridge' },
          { v: 'pair', label: 'Native bridge' },
          { v: 'cli', label: 'Go CLI candidate' },
        ],
      },
      {
        key: 'path',
        label: 'Path',
        note: '`existing` is not path-forced: reached from the invited `account` screen it names their server and the username you picked there, and the sixth sidebar step still reads `Wait to be added`.',
        values: [
          { v: 'own', label: 'Own' },
          { v: 'invited', label: 'Invited' },
        ],
      },
      {
        key: 'pending',
        label: 'Pending operation',
        note: 'A matching account-recovery row makes the recover button read `Resume recovery`. `Resume acceptance` is always drawn.',
        values: [
          { v: '', label: 'None' },
          { v: 'yes', label: 'Resumable' },
        ],
      },
      {
        key: 'refusal',
        label: 'Refusal',
        note: 'The `.crit` under the cards (first-run-screen.tsx:2360): `Resume acceptance` with nothing pending, or `Resume recovery` with no matching account-recovery row (mock-bridge.ts:790-800).',
        values: [
          { v: '', label: 'None' },
          { v: 'resume', label: 'Pairing not pending' },
          { v: 'recovery', label: 'Recovery not pending' },
        ],
      },
      {
        key: 'back',
        label: 'Back target',
        note: 'existingBack: `account` (came from Create an account), `checked` (returning), `local` (the managed `Recover an existing account…`), `local-account` (the same link on the managed Create your account pane, which sets existingBack to `account` — still the managed world, so ‹ Back returns to that pane).',
        values: [
          { v: '', label: 'account' },
          { v: 'checked', label: 'checked' },
          { v: 'local', label: 'local' },
          { v: 'local-account', label: 'local · account' },
        ],
      },
      {
        key: 'made',
        label: 'Account exists',
        note: 'checkpoint.account. The managed sidebar foot rows are `managedLocal && !account` (first-run-screen.tsx:3140-3149), so they go once the account is made — which is the state the `local-existing` capture froze.',
        values: [
          { v: '', label: 'Not yet' },
          { v: 'yes', label: 'Already created' },
        ],
      },
      {
        key: 'recovery',
        label: 'Backup phrase',
        note: 'The typed recovery phrase — `Recover` is disabled while it is empty (first-run-screen.tsx:2247-2252). Type in the field to fill it.',
        values: [
          { v: '', label: 'Empty' },
          { v: 'typed', label: 'Typed' },
        ],
      },
      {
        ...
        note: 'The typed pairing phrase — `Accept pairing` is disabled while it is empty; `Resume acceptance` never needs it.',
        values: [
          { v: '', label: 'Empty' },
          { v: 'typed', label: 'Typed' },
        ],
    ...
    render: function (s) {
      ...
      var protectPage = local
        ? 'frl-protect'
        : path === 'invited'
          ? 'fri-protect'
          : 'fr-protect';
      var accountPage = local
        ? 'frl-account'
        : path === 'invited'
          ? 'fri-account'
          : 'fr-account';
      ...
      if (native) {
        var ra = rr();
        recoverRows += UI.insetRow({
          label: 'Account alias',
          forId: ra,
          value: input({ id: ra, value: alias }),
        });
      }
      var ph = rr(),
        dv = rr();
      ...
      recoverRows += UI.insetRow({
        label: 'This Mac’s name',
        forId: dv,
        value: input({ id: dv, value: f.deviceName }),
      });

      var card1 =
        '<div class="pcard"><h3>Recover with your backup phrase</h3>' +
        ...
        '<div class="btns">' +
        UI.btn(s.v.pending === 'yes' ? 'Resume recovery' : 'Recover', {
          variant: 'primary',
          disabled: !phrase,
          attrs: phrase ? onward : '',
        }) +
      ...
      var card2 =
        ...
        (cli
          ? '<p>In Terminal, switch the official FOKS CLI to this account, then run:</p>' +
            UI.copyBox({
              display: '<code>foks --simple-ui key assist</code>',
              attrs: 'data-act="toast" data-text="Command copied."',
            }) +
        ...
        UI.inset(
          UI.insetRow({
            ...
            forId: pa,
            value: input({
              ariaLabel: 'Pairing account alias',
              id: pa,
              value: alias,
            }),
          }) +
        ...
        UI.btn('Resume acceptance', {
          attrs: 'data-act="set" data-key="v.refusal" data-val="resume"',
        }) +
        '</div></div>';
      ...
      if (cand && cand.copyable) {
        var ca = rr();
        card3 =
          '<div class="pcard"><h3>Copy this Mac’s CLI device</h3>' +
          ...
          UI.inset(
            UI.insetRow({
              ...
              forId: ca,
              value: input({
                ariaLabel: 'Copied account alias',
                id: ca,
                value: alias,
              }),
            }),
          ...
          UI.btn('Copy existing device', { attrs: onward }) +
          '</div>';
      }

      var body =
        '<h1>Add this Mac to your account</h1>' +
        '<p class="lead">Your account already exists on ' +
        esc(canonical) +
        ...
        '</div>' +
        (s.v.refusal === 'resume'
          ? '<p class="crit">That pairing acceptance is no longer pending.</p>'
          : s.v.refusal === 'recovery'
            ? '<p class="crit">That recovery is no longer pending.</p>'
            : '') +
        '<p class="hint">This Mac will become a device for <code>' +
        esc(f.username) +
      ...
      return {
        ...
        main: pane({
          ...
          foot: foot({
            ...
            children: UI.btn('I don’t have an account yet', {
              attrs: 'data-act="go" data-page="' + accountPage + '"',
            }),
          }),
        }),
      };
    ...
    controls: [
      {
        key: 'choice',
        label: 'Candidate',
        note: 'The chosen candidateId; `Connect selected account` stays disabled until one is picked.',
        values: [
          { v: '', label: 'Nothing' },
          { v: 'rae', label: 'First candidate' },
          { v: 'ops', label: 'Second candidate' },
          { v: 'draft', label: 'Third (unusable)' },
        ],
      },
    ],
    render: function (s) {
      ...
      var rows = GO_CANDIDATES.map(function (c) {
        ...
        return UI.insetRow({
          ...
          value:
            '<span>' +
            esc(c.serverHint || 'Server ' + goShort(c.hostId)) +
            ' ' +
            UI.chip(status) +
            '</span>' +
            '<small>' +
      ...
      var body =
        ...
        UI.btn('Connect selected account', {
          ...
          disabled: !picked,
          attrs: picked
            ? 'data-act="go" data-page="fr-address" data-set=\'{"v.path":"own","v.returning":"yes","v.card":"cli","v.made":"","v.recovery":"","v.phrase":""}\''
            : '',
        }) +
        UI.btn('Set up another account', {
          attrs:
            'data-act="go" data-page="fr-who" data-set=\'{"v.choice":""}\'',
        }) +
        '</div>';
      return {
        side: setupSide({ current: 0, path: null }),
