  ...
  */
  ...
  var esc = M.esc;
  var uid = 0;
  function nextId(prefix) { uid += 1; return (prefix || 'm') + uid; }
  ...
  function raw(extra) { return extra ? ' ' + extra : ''; }
  function classes() {
    var out = [];
  ...
  var VARIANT_CLASS = { primary: 'primary', plain: '', danger: 'danger', quiet: 'icon' };
  UI.btn = function (label, o) {
    o = o || {};
    var cls = ['btn', VARIANT_CLASS[o.variant || 'plain'], o.size === 'sm' ? 'cap' : '', o.on ? 'on' : '', o.className || '']
      .filter(Boolean).join(' ');
    ...
    return '<button' + labels + raw(o.attrs) +
      ' type="' + (o.type || 'button') + '" class="' + cls + '"' +
      (o.on === undefined ? '' : ' aria-pressed="' + (o.on ? 'true' : 'false') + '"') + '>' +
      (o.icon ? M.icon(o.icon) : '') + (label == null ? '' : label) + '</button>';
  };

  /* -------------------------------------------------- Chip / Tag / Badge */
  UI.chip = function (text, o) {
    o = o || {};
    var cls = ['chip', (o.tone && o.tone !== 'default') ? o.tone : '', o.className || ''].filter(Boolean).join(' ');
    return '<span class="' + cls + '"' + attr('title', o.title) + raw(o.attrs) + '>' + (text == null ? '' : text) + '</span>';
  };
  UI.tag = function (text, o) {
    o = o || {};
    return '<span class="' + (o.tone === 'warn' ? 'tag warn' : 'tag') + '"' + attr('title', o.title) + '>' + text + '</span>';
  };
  /* Nothing at zero — the rule that an empty Alerts badge is absent, not "0". */
  UI.badge = function (count, label) {
    if (!count) return '';
    return '<span class="badge"' + attr('title', label) + '>' + esc(count) + '</span>';
  };
  ...
  UI.avatar = function (party, o) {
    ...
    if (party.party_kind !== 'user') {
      return '<span class="' + cls + ' team"' + attr('title', name) + ' style="background: var(--c-team);">' +
        M.icon('people') + '</span>';
    }
    var colour = party.hue || M.data.hue(name);
    return '<span class="' + cls + '"' + attr('title', name) + ' style="background: ' + UI.rgb(colour) + ';">' +
      esc(party.initials || M.data.initials(name)) + '</span>';
  };
  UI.stack = function (parties, o) {
    o = o || {};
    parties = parties || [];
    var cls = ['stack', o.size && o.size !== 'md' ? o.size : ''].filter(Boolean).join(' ');
    var inner = parties.length
      ? parties.slice(0, 2).map(function (p) { return UI.avatar(p); }).join('')
      /* No roster at all — the neutral glyph rather than a gap. */
      : '<span class="av" style="background: var(--c-none);">' + M.icon('people') + '</span>';
    return '<span class="' + cls + '"' + attr('title', o.title) + '>' + inner + '</span>';
  };

  /* ------------------------------------------------ KindIcon / KindGlyph */
  UI.kindIcon = function (kind, className) {
    return '<span class="' + classes('kic', kind, className) + '">' + M.icon(M.data.KINDS[kind].icon) + '</span>';
  };
  UI.kindGlyph = function (item, size) {
    if (M.data.isLogin(item)) {
      ...
      return '<span class="' + classes('kico', size) + '" aria-hidden="true" style="background: ' + UI.rgb(M.data.hue(site)) + ';">' +
        esc(site[0].toUpperCase()) + '</span>';
    }
    var kind = M.data.kindOf(item);
    return '<span class="' + classes('kico', size, kind) + '">' + M.icon(M.data.KINDS[kind].icon) + '</span>';
  };

  /* ------------------------------------------------- Inset / InsetRow / Field */
  UI.inset = function (rows, o) {
    o = o || {};
    var cls = [o.variant === 'preview' ? 'prev' : 'inset', o.off ? 'off' : '', o.className || ''].filter(Boolean).join(' ');
    return '<div class="' + cls + '"' + raw(o.attrs) + '>' + (rows || '') + '</div>';
  };
  UI.insetRow = function (o) {
    o = o || {};
    var rowClass = classes(o.variant === 'preview' ? 'irow' : 'fr', o.className);
    var label = o.label === undefined ? ''
      : (o.forId ? '<label class="k" for="' + esc(o.forId) + '">' + o.label + '</label>'
                 : '<span class="k">' + o.label + '</span>');
    var value = o.value === undefined ? ''
      : '<span class="' + classes('v', o.valueClass) + '">' + o.value + '</span>';
    var action = o.action === undefined ? '' : '<span class="a">' + o.action + '</span>';
    return '<div class="' + rowClass + '"' + raw(o.attrs) + '>' + label + value + action + '</div>';
  };
  UI.field = function (o) {
    o = o || {};
    ...
    var input = '<input' + attr('aria-label', o.label) +
      (o.mono ? ' class="mono"' : '') + attr('placeholder', o.placeholder) +
      attr('min', o.min) + attr('max', o.max) + ' id="' + esc(id) + '"' +
      ' type="' + (o.type || 'text') + '"' +
      attr('value', o.value == null ? '' : o.value) +
      (o.bind ? ' data-bind="' + esc(o.bind) + '"' + (o.live === false ? '' : ' data-live') : '') +
      raw(o.inputAttrs) + '>';
    return UI.insetRow({
      label: esc(o.label), forId: id, valueClass: o.mono ? 'mono' : undefined,
      value: input + (o.hint === undefined ? '' : '<small>' + o.hint + '</small>'),
      action: o.action, className: o.className, attrs: o.attrs,
    });
  };
  ...
  UI.sectionLabel = function (text, o) {
    o = o || {};
    if (o.as === 'side') return '<h6' + attr('class', o.className) + '>' + text + '</h6>';
    return '<div class="' + classes('sec', o.className) + '">' + text + (o.action || '') + '</div>';
  };

  /* --------------------------------------------------------- Notice / Band */
  UI.notice = function (o) {
    o = o || {};
    var cls = o.severity === 'crit' ? 'notice stop' : o.severity === 'info' ? 'notice info' : 'notice';
    return '<div class="' + cls + '"' + raw(o.attrs) + '>' +
      (o.eyebrow === undefined ? '' : '<div class="who">' + o.eyebrow + '</div>') +
      '<h2>' + o.title + '</h2>' + (o.body || '') +
      (o.footnote === undefined ? '' : '<p class="fn">' + o.footnote + '</p>') +
      (o.actions === undefined ? '' : '<div class="acts2">' + o.actions + '</div>') +
      '</div>';
  };
  UI.band = function (o) {
    o = o || {};
    var cls = o.severity === 'crit' ? 'band stop' : o.severity === 'info' ? 'band info' : 'band';
    return '<div class="' + cls + '"' + attr('title', o.title) + '>' +
      M.icon(o.severity === 'info' ? 'info' : 'alert') +
      '<span class="t">' + (o.label ? '<b>' + o.label + '</b> ' : '') + (o.text || '') + '</span>' +
  ...
  UI.segmented = function (o) {
    var on = o.value;
    return '<span class="' + (o.variant === 'icon' ? 'seg' : 'seg txt') + '" role="group"' +
      attr('aria-label', o.label) + '>' +
      o.items.map(function (item) {
        var sel = item.id === on;
        var hook = o.key ? ' data-act="set" data-key="' + esc(o.key) + '" data-val="' + esc(item.id) + '"' : raw(item.attrs);
        return '<button type="button" class="' + (sel ? 'on' : '') + '" aria-pressed="' + (sel ? 'true' : 'false') + '"' +
          attr('title', item.title) + hook + '>' +
          (item.icon ? M.icon(item.icon) : '') + (item.label === undefined ? '' : esc(item.label)) + '</button>';
      }).join('') + '</span>';
  };
  ...
  function menuBody(o) {
    return '<div class="menu"' + attr('aria-label', o.menuLabel) + ' role="menu">' + (o.menuHtml || '') + '</div>';
  }
  /* The portalled form of the same menu, for `overlay(s)`. */
  UI.menuPortal = function (o) {
    o = o || {};
    return '<div class="menu-portal"' + attr('style', o.style) + '>' + menuBody(o) + '</div>';
  };
  ...
  function queueMenu(o) {
    ...
    M.pendingMenus.push({ id: id, align: o.align === 'end' ? 'end' : 'start', menuLabel: o.menuLabel, menuHtml: o.menuHtml });
  ...
  UI.menuButton = function (o) {
    ...
    /* menus.tsx:113-115 — an icon-only trigger falls back to its title for the
       accessible name. */
    var ariaLabel = o.ariaLabel !== undefined ? o.ariaLabel
      : (o.label == null || o.label === '' ? (o.title !== undefined ? o.title : o.menuLabel) : undefined);
    var trigger = UI.btn(
      (o.label == null ? '' : o.label) +
        (o.trailingIcon === null ? '' : M.icon(o.trailingIcon || 'chev', null, { cls: 'chevron' })),
      {
        variant: o.variant, size: o.size, icon: o.icon, disabled: o.disabled,
        title: o.title, ariaLabel: ariaLabel, className: o.btnClass,
        attrs: 'aria-haspopup="menu" aria-expanded="' + (open ? 'true' : 'false') + '"' +
          raw(open ? o.closeAttrs : o.openAttrs) + (open ? queueMenu(o) : ''),
      });
    return '<span class="' + classes('menuwrap', o.className) + '">' + trigger + '</span>';
  };
  UI.splitButton = function (o) {
    o = o || {};
    var open = !!o.open;
    return '<span class="menuwrap"><span class="split">' +
      UI.btn(o.label, {
        variant: 'primary', icon: o.icon,
        attrs: (o.mainAttrs ? o.mainAttrs : 'aria-haspopup="menu" aria-expanded="' + (open ? 'true' : 'false') + '"' + raw(open ? o.closeAttrs : o.openAttrs)),
      }) +
      UI.btn(M.icon('chev', null, { cls: 'chevron' }), {
        variant: 'primary', ariaLabel: o.menuLabel,
        attrs: 'aria-haspopup="menu" aria-expanded="' + (open ? 'true' : 'false') + '"' +
          raw(o.toggleAttrs || (open ? o.closeAttrs : o.openAttrs)) +
          (open ? queueMenu({ align: o.align || 'start', menuLabel: o.menuLabel, menuHtml: o.menuHtml, menuId: o.menuId }) : ''),
      }) +
      '</span></span>';
  };
  ...
  M.drainMenus = function () {
    return M.pendingMenus.map(function (m) {
      return '<div class="menu-portal" data-menu-for="' + esc(m.id) + '">' +
        menuBody({ menuLabel: m.menuLabel, menuHtml: m.menuHtml }) + '</div>';
    }).join('');
  };
  ...
  UI.searchField = function (o) {
    ...
    return '<label class="search">' + M.icon('search') +
      '<input' + attr('placeholder', ph) + attr('aria-label', ph) + ' autocomplete="off" type="text"' +
      attr('value', o.value == null ? '' : o.value) +
  ...
  UI.sheet = function (o) {
    ...
    var panel = '<div class="' + cls + '" data-act="stop"><div class="hd">' + (o.glyph || '') +
      '<span class="t"><h2 id="' + esc(id) + '">' + o.title + '</h2>' +
    ...
    return '<div class="backdrop" role="' + (o.danger ? 'alertdialog' : 'dialog') + '" aria-modal="true" aria-labelledby="' + esc(id) + '" tabindex="-1"' +
      attr('aria-label', o.ariaLabel) + raw(o.backdropAttrs || o.closeAttrs) + '>' + panel + '</div>';
  };

  /* ----------------------------------------------------------------- Tabs */
  UI.tabs = function (o) {
    return '<div class="tabs" role="tablist"' + attr('aria-label', o.label) + '>' +
      o.items.map(function (item) {
        var on = item.id === o.value;
        var hook = item.attrs ? raw(item.attrs)
          : (item.page ? ' data-act="go" data-page="' + esc(item.page) + '"'
            : (o.key ? ' data-act="set" data-key="' + esc(o.key) + '" data-val="' + esc(item.id) + '"' : ''));
        return '<button type="button" role="tab" class="' + (on ? 'tab on' : 'tab') + '" aria-selected="' + (on ? 'true' : 'false') + '"' +
          hook + '>' + esc(item.label) +
          (item.count === undefined ? '' : '<span class="' + (item.count ? 'n' : 'n zero') + '">' + esc(item.count) + '</span>') +
          '</button>';
      }).join('') + '</div>';
  };
  ...
  UI.toggle = function (o) {
    ...
    var open = !!o.open;
    return '<div class="' + classes('toggle', o.className) + '">' +
      '<button type="button" class="toggle-trigger" aria-expanded="' + (open ? 'true' : 'false') + '" aria-controls="' + esc(id) + '"' +
      (o.disabled ? ' disabled' : '') + raw(o.attrs) + '>' +
      ...
      o.label + '</button>' +
      '<div id="' + esc(id) + '" class="toggle-body"' + (open ? '' : ' hidden') + '>' + (o.body || '') + '</div>' +
      '</div>';
  };
  ...
  UI.radioGroup = function (cards, o) {
    o = o || {};
    return '<div role="radiogroup"' + attr('aria-label', o.label) + ' class="' + classes('radios', o.className) + '">' +
      (Array.isArray(cards) ? cards.join('') : cards) + '</div>';
  };
  UI.radioCard = function (o) {
    var cls = ['radio', o.className || '', o.selected ? 'on' : '', o.off ? 'off' : ''].filter(Boolean).join(' ');
    return '<button type="button" role="radio" aria-checked="' + (o.selected ? 'true' : 'false') + '" class="' + cls + '"' +
      (o.disabled || o.off ? ' disabled' : '') + raw(o.attrs) + '>' +
      '<span class="rb"></span><span class="t"><b>' + o.title + '</b>' +
      (o.detail === undefined ? '' : '<small>' + o.detail + '</small>') + '</span>' +
      (o.tail || '') + '</button>';
  };
  ...
  function cardText(opt) {
    return '<span class="t"><b>' + opt.title + '</b>' +
      (opt.detail === undefined ? '' : '<small>' + opt.detail + '</small>') + '</span>';
  }
  UI.cardSelect = function (o) {
    o = o || {};
    var open = !!o.open;
    var chosen = null;
    (o.options || []).forEach(function (opt) { if (opt.id === o.value) chosen = opt; });
    return '<div class="card-select">' +
      '<button type="button" class="card-select-trigger" aria-haspopup="listbox" aria-expanded="' + (open ? 'true' : 'false') + '"' +
      attr('aria-label', o.label) + raw(open ? o.closeAttrs : o.triggerAttrs) + '>' +
      (chosen ? cardText(chosen) : '<span class="t"><b>' + (o.placeholder || 'Choose a vault') + '</b></span>') +
      M.icon('chev', null, { cls: 'card-select-chevron' }) + '</button></div>';
  };
  UI.cardSelectMenu = function (o) {
    ...
    return '<div class="menu-portal">' +
      '<div class="card-select-menu"' + attr('aria-label', o.label) + ' role="listbox">' +
      (o.options || []).map(function (opt) {
        ...
        return '<button type="button" role="option" aria-selected="' + (on ? 'true' : 'false') + '"' +
          ...
          ' class="' + ['card-select-option', on ? 'on' : '', opt.off ? 'off' : ''].filter(Boolean).join(' ') + '"' +
          (opt.off ? '' : ' tabindex="-1"' + raw(opt.attrs || (o.optionAttrs ? o.optionAttrs(opt.id) : ''))) + '>' +
          cardText(opt) + (on ? M.icon('check', null, { cls: 'card-select-check' }) : '') + '</button>';
      }).join('') + '</div></div>';
  };
  ...
  M.placeMenuPortal = function (o) {
    ...
    var trigger = document.querySelector(o.trigger || '#overlays .card-select-trigger');
    ...
    portal.style.left = (o.align === 'end'
      ? a.right - f.left - (portal.offsetWidth || a.width)
      : a.left - f.left) + 'px';
    portal.style.top = (a.bottom - f.top + (o.gap === undefined ? 4 : o.gap)) + 'px';
  ...
  UI.copyBox = function (o) {
    ...
    var copy = UI.btn(esc(o.label || 'Copy'), {
      attrs: o.attrs || ('data-act="toast" data-text="' + esc((o.label || 'Copy') === 'Copy' ? 'Copied' : o.label) + '"'),
    });
    var body = o.extra ? '<span class="copybox-actions">' + copy + o.extra + '</span>' : copy;
    return '<div class="copybox"><span class="v">' + (o.display === undefined ? esc(o.text) : o.display) + '</span>' + body + '</div>';
  };

  /* ----------------------------------------------------------- PageHeader */
  UI.pageHeader = function (o) {
    o = o || {};
    var head = '<div class="loc' + (o.lead ? '' : ' text-only') + '">' + (o.lead || '') +
      '<div class="loc-copy"><h1>' + esc(o.title) + '</h1>' +
      (o.subtitle ? '<small>' + esc(o.subtitle) + '</small>' : '') + '</div></div>';
    var tail = (o.tail || o.action)
      ? '<div class="header-action">' + (o.tail || '') + (o.action || '') + '</div>' : '';
    var search = o.search
      ? UI.searchField({
  ...
  /* -------------------------------------------------------------- Toolbar */
  UI.toolbar = function (inner) { return '<div class="toolbar">' + (inner || '') + '</div>'; };
  UI.spacer = function () { return '<span class="spacer"></span>'; };
  UI.body = function (inner, o) {
    o = o || {};
    return '<div class="' + classes('body', o.className) + '">' + (inner || '') + '</div>';
  };
  UI.main = function (inner, o) {
    o = o || {};
    return '<main class="' + classes('main', o.className) + '">' + (inner || '') + '</main>';
  };
  ...
  UI.listHeader = function (o) {
    ...
    function head(label, sortKey) {
      ...
      return '<button type="button" class="' + (on ? 'on' : '') + '" data-act="set" data-key="' + esc(key) + '" data-val="' + esc(sortKey) + '">' +
        esc(label + (on ? ' ↓' : '')) + '</button>';
    }
    return '<div class="hdr"><span></span>' + head('Name', 'name') +
      '<span>Server</span><span>Readable by</span>' + head('Version', 'version') + '</div>';
  };
  ...
  UI.listRow = function (o) {
    ...
    return '<div class="' + (o.selected ? 'row sel' : 'row') + '" role="button" tabindex="0" aria-pressed="' +
      (o.selected ? 'true' : 'false') + '"' + raw(o.attrs) + '>' +
      UI.kindIcon(kind) +
      '<span class="name"><span class="tt"><span>' + esc(M.data.nameOf(item.path)) + '</span>' +
      (prefix ? '<span class="pchip">' + esc(prefix) + '</span>' : '') + '</span>' +
      '<small>' + esc(storeName) + (o.searching ? ' · <code>' + esc(item.path) + '</code>' : '') + '</small></span>' +
      '<span class="n server">' + esc(o.server || '') + '</span>' +
      '<span>' + UI.chip(esc(count), { title: readers.title }) + '</span>' +
      '<span class="n">' + esc(item.version) + '</span></div>';
  };
  ...
  UI.emptyState = function (o) {
    ...
    return '<div class="empty"><div class="big">' + M.icon(meta ? meta.icon : 'key') + '</div>' +
      '<h2>' + esc(o.title || ('No ' + (meta ? meta.plural.toLowerCase() : 'items') + ' here')) + '</h2>' +
      '<p>' + esc(o.body || (meta || M.data.KINDS.Password).blurb) + '</p>' + (o.action || '') + '</div>';
  };
  UI.emptySearch = function (o) {
    o = o || {};
    return '<div class="empty"><h2>Nothing ' + (o.store ? 'in ' + esc(o.store) : 'here') +
      ' matches “' + esc(o.query) + '”</h2>' +
  ...
  M.renderTitlebar = function (s, page) {
    s = s || M.s;
    ...
    return '<div class="titlebar" data-tauri-drag-region="">' +
      ...
      '<span class="' + (starting ? 'agent warn' : 'agent') + '" data-tauri-drag-region=""><i data-tauri-drag-region=""></i>' +
      'Agent ' + (starting ? 'starting' : 'ready') + '</span>' +
      ...
      UI.btn('', {
        variant: 'quiet', className: 'global-refresh', icon: 'again',
        ariaFirst: true,
        ariaLabel: busy ? 'Refreshing vaults and groups' : 'Refresh',
        title: busy ? 'Refreshing vaults and groups' : 'Refresh vaults and groups',
        disabled: busy,
  ...
  function navRow(o) {
    var cls = ['nav', o.active ? 'on' : '', o.dimmed ? 'off' : ''].filter(Boolean).join(' ');
    return '<button type="button" class="' + cls + '"' + attr('title', o.title) +
      (o.active ? ' aria-current="page"' : '') + raw(o.attrs) + '>' +
      (o.glyph || '') +
      '<span class="t">' + esc(o.name) + (o.caption ? '<small>' + esc(o.caption) + '</small>' : '') + '</span>' +
      (o.tail || '') + '</button>';
  }
  M.navRow = navRow;
  ...
  M.renderSidebar = function (s, page, opts) {
    ...
    function storeRow(store) {
      return navRow({
        ...
        attrs: 'data-act="go" data-page="' + esc(M.storePages[store.id] || store.id) + '"',
      });
    }
    return '<nav class="side" aria-label="Places">' +
      navRow({ active: nav === 'all', glyph: M.icon('grid'), name: 'All items', attrs: 'data-act="go" data-page="all"' }) +
      UI.sectionLabel('Vaults', { as: 'side' }) +
      (w.vaults.length ? w.vaults.map(storeRow).join('') : '<p class="fn">No vaults yet</p>') +
      UI.sectionLabel('Groups', { as: 'side' }) +
      (w.groups.length ? w.groups.map(storeRow).join('') : '<p class="fn">No groups yet</p>') +
      /* The Shares section — heading and rows — is omitted entirely when
         there is no ad-hoc share. */
      (w.shares.length ? UI.sectionLabel('Shares', { as: 'side' }) + w.shares.map(storeRow).join('') : '') +
      ((page && page.status) ? (typeof page.status === 'function' ? page.status(s) : page.status) : '') +
      '<div class="foot">' +
      navRow({
        active: nav === 'alerts', glyph: M.icon('bell'), name: 'Alerts',
        tail: UI.badge(opts.alerts === undefined ? w.alertCount : opts.alerts, 'Open alerts'),
        attrs: 'data-act="go" data-page="alerts"',
      }) +
      navRow({
        active: nav === 'settings', glyph: M.icon('gear'), name: 'Settings',
        attrs: 'data-act="go" data-page="set-devices"',
      }) +
      '<div class="foot-separator"></div>' +
      navRow({
        active: nav === 'setup', glyph: M.icon('again'), name: 'Set up new vault',
  ...
  function appLockCard(s) {
    var mechanism = M.data.appLock.mechanism === 'biometry'
      ? 'Touch ID or your Mac password' : 'your operating-system password';
    return '<div class="app-lock" role="dialog" aria-modal="true" aria-labelledby="app-lock-title">' +
      ...
      (s.v.lockError ? '<p class="app-lock-error" role="alert">' + esc(s.v.lockError) + '</p>' : '') +
      UI.btn('Unlock', { variant: 'primary', attrs: 'data-act="call" data-fn="unlockApp"' }) +
      '</div></div>';
  }

  /* app-root.tsx:225-251 — note the curly apostrophe in "Couldn’t". */
  function bootErrorCard() {
    return '<div class="app-lock" role="alertdialog" aria-labelledby="app-boot-error-title">' +
      '<div class="app-lock-card">' +
      ...
      UI.btn('Retry', { variant: 'primary', attrs: 'data-act="set" data-key="boot" data-val="ok"' }) +
      '</div></div>';
  }
  ...
  /* write-workflows.tsx:788-822 — a sibling of `.app` inside `.window`,
     ...
     visible and live. */
  M.stopwrap = function () {
    return '<div aria-label="Local agent connection lost" class="stopwrap" role="alertdialog" aria-modal="true" tabindex="-1">' +
      UI.notice({
        severity: 'crit',
        ...
        actions: UI.btn('Retry', { variant: 'primary', attrs: 'data-act="call" data-fn="agentRetry"' }),
      }) + '</div>';
  };
  ...
  M.globalTakeover = function (s) {
    ...
    if (s.boot === 'loading') {
      return { takeover: '<div class="app-loading">Connecting to the local agent…</div>' };
    }
    return null;
  };
  ...
  M.renderGlobalOverlay = function () { return ''; };
