  ...
  (function () {
    ...
    M.flow({
      ...
      steps: [
        { label: 'Preparing this Mac', page: 'fr-boot', set: base({ agent: 'starting', 'v.path': 'own' }),
          note: 'The agent is still in Bootstrap — the amber pill is the titlebar’s, not this screen’s — and the checkpoint already says this Mac is on the own path.' },
        ...
        { label: 'Save recovery phrase', page: 'fr-protect', set: base({}),
          note: 'Three ways back in, in one step: a passphrase you type, a YubiKey you can add later, and the 17-token phrase you write down now.' },
        { label: 'Write these tokens down', page: 'fr-phrase', set: base({ 'v.path': 'own' }),
          note: 'The one-time sheet: Done stays disabled until “I have written these 17 tokens down” is ticked, and there is no copy button.' },
        { label: 'Phrase written down', page: 'fr-protect', set: base({ 'v.written': '1' }),
          note: 'Back on protect, the phrase card reads Written down and the reveal button is spent.' },
        { label: 'Create a group', page: 'fr-create-group', set: base({ 'v.choice': 'named' }),
    ...
    M.flow({
      ...
      steps: [
        ...
        { label: 'Phrase shown and ticked', page: 'frl-protect', set: base({ managed: 'local', 'v.disclosure': 'open', 'v.written': '1' }),
          note: 'The 17 tokens inline; Start using FOKS goes live once the box is ticked.' },
    ...
    M.flow({
      ...
      steps: [
        { label: 'Preparing this Mac', page: 'fr-boot', set: base({ agent: 'starting', 'v.path': 'invited' }),
          note: 'The same boot screen while the agent starts; the checkpoint path only changes the sixth sidebar label.' },
    ...
    M.flow({
      ...
      steps: [
        ...
        { label: 'Recover with the phrase', page: 'frx-existing', set: base({ 'v.path': 'own', 'v.card': '', 'v.recovery': 'typed', 'v.back': 'checked' }),
          note: 'The recover card asks for the 17 tokens and a name for this Mac — nothing else is needed to prove the account is yours.' },
    ...
    M.flow({
      ...
      steps: [
        ...
        { label: 'Refused: something is already there', page: 'store-personal', set: base({ 'v.sheet': 'exists', 'v.dest': 'acct:personal', 'v.epath': '/logins/github.com' }),
          note: 'Creating carries “must not exist”: nothing was created, nothing overwritten, and there is no create anyway.' },
    ...
    M.flow({
      ...
      steps: [
        ...
        { label: 'Someone else changed this first', page: 'store-personal', set: base({ 'v.sel': 'acct:personal|/logins/github.com', 'v.details': 'shown', 'v.sheet': 'conflict' }),
    ...
    M.flow({
      ...
      steps: [
        ...
        { label: 'Set a passphrase', page: 'set-account', set: base({ 'v.sheet': 'passphrase', 'v.seg': 'set' }),
          note: 'Whether one is currently set is not reported, so all three modes live in one sheet.' },
        ...
        { label: 'The 17 tokens', page: 'set-devices', set: base({ 'v.sheet': 'phrase', 'v.written': '' }),
        ...
        { label: 'Recover here', page: 'set-devices', set: base({ 'v.sheet': 'recover' }),
          note: 'The other half: a local alias, this Mac’s name and the 17 tokens, submitted for review.' },
      ],
    });
