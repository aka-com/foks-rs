...
(function () {
  ...
  var BASE_ITEMS = D.items.map(function (row) {
  ...
  var NAMED = {
    ...
    'created-file':
      'new~File~household~/documents/insurance.pdf~Member.0~Admin',
  ...
  var NAMED_VALUE = {
    'acct:personal|/logins/gitlab.com':
  ...
  function wireWord(wire) {
    ...
    return role === 'Member'
      ? 'Member.' + (wire.visibility || 0)
      : role || 'Owner';
  }
  function ops(s) {
    return String((s.v && s.v.applied) || '')
      .split(',')
      .filter(Boolean)
      .map(function (token) {
  ...
  function applyItemOp(token) {
    ...
    if (op === 'new') {
      ...
      var row = {
        ...
        size: 0,
        version: 1,
        read: parseWire(bits[4]),
        write: parseWire(bits[5]),
      };
      if (kind === 'File') {
        row.kind = 'File';
        row.size = (value || '').length;
      } else if (kind === 'Link') {
        row.kind = 'Link';
        row.target = value || '';
        row.size = row.target.length;
      } else {
        row.size = (value || '').length;
        D.plaintext[key] = value || '';
      }
      D.items.push(row);
      return;
    }
    ...
    if (op === 'rm') {
      D.items = D.items.filter(function (r) {
        return !(r.store === ref && r.path === bits[2]);
      });
  ...
  function syncItems(s) {
    D.items = BASE_ITEMS.map(function (row) {
      return Object.assign({}, row);
    });
  ...
  function viewOf(s) {
    return vv(s, 'view', 'list');
  }
  function kindFilter(s) {
    return vv(s, 'kind', 'All');
  }
  function sortOf(s) {
    return vv(s, 'sort', 'name');
  }
  function queryOf(s) {
    return s.v.search || '';
  }
  ...
  function defaultCreateStore(w) {
    ...
    for (var i = 0; i < order.length; i++)
      if (canCreateInStore(w, order[i].id)) return order[i].id;
  ...
  function editableValue(item, learned) {
    ...
    for (var i = 0; i < lines.length; i++)
      if (lines[i].indexOf('password: ') === 0) return learned;
    if (!/password: .*/.test(item.value)) return learned;
    return item.value.replace(/password: .*/, function () {
      return 'password: ' + learned;
    });
  }
  ...
  var SORTS = {
    name: function () {
      return function (a, b) {
        return D.nameOf(a.path).localeCompare(D.nameOf(b.path));
      };
    },
    kind: function () {
      return function (a, b) {
        return (
          D.KIND_LIST.indexOf(D.kindOf(a)) - D.KIND_LIST.indexOf(D.kindOf(b)) ||
    ...
    group: function (w) {
      return function (a, b) {
        return (
          ((w.storeById[a.store] || {}).name || '').localeCompare(
            (w.storeById[b.store] || {}).name || '',
          ) || D.nameOf(a.path).localeCompare(D.nameOf(b.path))
        );
      };
    },
  ...
  function scopedItems(s, w, ref) {
    ...
    if (q) {
      ...
      items = items.filter(function (i) {
        return (
          i.path.toLowerCase().indexOf(needle) >= 0 ||
          ((w.storeById[i.store] || {}).name || '')
            .toLowerCase()
            .indexOf(needle) >= 0
        );
      });
  ...
  function selectedItem(s, w) {
    ...
    var store = s.v.sel.slice(0, cut),
      path = s.v.sel.slice(cut + 1);
    var hit = w.items.filter(function (i) {
      return i.store === store && i.path === path;
    });
    return hit[0] || null;
  }
  ...
  function menuTrigger(o) {
    return (
      '<span class="menuwrap">' +
      UI.btn(o.label + M.icon('chev', null, { cls: 'chevron' }), {
        variant: o.variant,
        attrs:
  ...
  function newMenuHtml() {
    return D.KIND_LIST.map(function (k) {
      return (
        '<button type="button" role="menuitem" class="kind-menu-item" tabindex="-1" ' +
        'data-act="call" data-fn="itemsNew" data-arg="' +
        k +
        '">' +
        UI.kindIcon(k) +
        esc(D.kindLabel(k)) +
  ...
  function sortMenuHtml(s) {
    ...
    return SORT_KEYS.map(function (k) {
      ...
      return (
        ...
        (on ? 'true' : 'false') +
        '" tabindex="-1" ' +
        'data-act="call" data-fn="itemsSort" data-arg="' +
        k +
        '">' +
        (on ? M.icon('check') : '<span class="ic"></span>') +
        esc(SORT_LABELS[k]) +
        '</button>'
      );
    }).join('');
  ...
  function newItemButton(s, menuKey) {
    return menuTrigger({
      ...
      variant: 'primary',
      menuKey: menuKey,
      open: s.v.menu === menuKey,
    });
  }
  function sortMenu(s) {
    return menuTrigger({
      label: esc(SORT_LABELS[sortOf(s)]),
      menuKey: 'sort',
      open: s.v.menu === 'sort',
    });
  }
  ...
  var MENU_TRIGGER = {
    ...
    sort: {
      trigger: '#win .toolbar .menuwrap .btn:not(.primary)',
      align: 'end',
    },
  };
    ...
    var kinds = [
      {
        id: 'All',
        label: 'All',
        attrs: 'data-act="set" data-key="v.kind" data-val=""',
      },
    ].concat(
          ...
          items: [
            {
              ...
              icon: 'list',
              title: 'List',
              attrs: 'data-act="set" data-key="v.view" data-val=""',
            },
            {
              id: 'grid',
              icon: 'grid',
              title: 'Cards',
              attrs: 'data-act="set" data-key="v.view" data-val="grid"',
            },
          ],
        }),
        UI.btn('', {
        ...
        store && store.kind === 'team'
          ? UI.btn('', {
              ...
              attrs:
                ...
                esc(GROUP_PAGES[store.id] || 'set-groups') +
                '"',
            })
          : '',
      ),
  ...
  function listBody(s, w, items) {
    ...
    return (
      ...
      UI.listHeader({ sort: sortOf(s), key: 'v.sort' }) +
      ...
      items
        .map(function (item) {
          ...
          return UI.listRow({
            ...
            server: (w.serverById[(w.storeById[item.store] || {}).server] || {})
              .name,
            attrs:
              'data-act="call" data-fn="itemsSelect" data-arg="' +
              esc(key) +
              '"',
          });
        })
  ...
  function itemActions(w, item) {
    ...
    var head =
      kind === 'Password' || kind === 'Resource'
        ...
        : kind === 'File'
          ? act('Download', 'download', 'itemsDownload')
          : act('Open target', 'arrow', 'itemsOpenTarget');
    return (
      '<span class="acts">' +
      head +
      act('Copy path', 'path', 'itemsCopyPath') +
      act(
        removeDisabled
  ...
  function tile(s, w, item) {
    ...
    var sub =
      kind === 'Link'
        ? pathChip(item.path) + ' target read when opened'
        : kind === 'File'
          ? pathChip(item.path) + ' ' + esc(D.fmtSize(item.size))
          : D.prefixOf(item.path)
            ? pathChip(item.path)
            : '<span class="dim">at the root</span>';
    return (
      '<div class="' +
      (selected ? 'tile sel' : 'tile') +
  ...
  function tileSection(w, store) {
    if (store.kind === 'team') {
      return (
        '<div class="gsec">' +
        UI.stack(store.parties) +
        '<span>' +
        esc(store.name) +
        '</span>' +
        '<span class="n">· ' +
        esc(D.peopleGroups(store.parties)) +
        '</span></div>'
      );
    }
    return (
      ...
      esc(store.name) +
      '<span class="n">· ' +
      esc((w.serverById[store.server] || {}).name || '') +
      '</span></div>'
    );
  }
  function gridBody(s, w, items, ref) {
    ...
    if (!ref) {
      return D.storeDisplayOrder(w.stores)
        .map(function (store) {
          var section = cards.filter(function (i) {
            return i.store === store.id;
          });
    ...
    return (
      ...
      esc(meta ? meta.plural : 'Everything') +
      '<span class="n">· ' +
      items.length +
      '</span></div>' +
      '<div class="tiles">' +
  ...
  function emptyBody(s, store) {
    ...
    return (
      ...
      '</div>' +
      '<h2>No ' +
      (meta ? esc(meta.plural.toLowerCase()) : 'items') +
      ' here</h2>' +
      '<p>' +
      esc((meta || D.KINDS.Password).blurb) +
      '</p>' +
      newItemButton(s, 'new-empty') +
      '</div>'
    );
  }
  ...
  function takeover(s, w, store) {
    ...
    var action =
      notice.actionKind === 'finish-setup'
        ? UI.btn('Finish setup', {
            variant: 'primary',
            attrs:
              'data-act="call" data-fn="itemsFinishSetup" data-arg="' +
              esc(store.id) +
              '"',
          })
        : UI.btn(
            notice.actionKind === 'open-server'
              ? 'Open server'
              : 'Review server',
            {
              attrs:
    ...
    var head = UI.pageHeader({
      ...
      action:
        (notice.severity === 'warn' && notice.actionKind === 'finish-setup'
          ? ''
          : action) + gear,
    });
    return (
  ...
  function passwordFields(masked, shown) {
    if (shown !== null && shown !== undefined) {
      ...
      var structured = learned.some(function (p) {
        return p[0] === 'password';
      });
      if (structured) {
        return learned.map(function (p) {
          return { field: p[0], value: p[1], secret: p[0] === 'password' };
        });
      }
  ...
  function wireRole(wire) {
    ...
    if (role.kind === 'member')
      return {
        role: 'Member',
        visibility: role.visibility == null ? 0 : role.visibility,
      };
    return { role: role.kind === 'admin' ? 'Admin' : 'Owner' };
  }
  function revealActions(revealed) {
    return revealed
      ? '<button type="button" data-act="call" data-fn="itemsHide">' +
          M.icon('eyeoff') +
          'Hide</button>' +
          '<button type="button" data-act="call" data-fn="itemsCopyValue">' +
          M.icon('copy') +
          'Copy</button>'
      : '<button type="button" data-act="call" data-fn="itemsShow">' +
          M.icon('eye') +
          'Show</button>' +
          '<button type="button" data-act="call" data-fn="itemsCopyValue">' +
          M.icon('copy') +
  ...
  function detailsPanel(s, w) {
    ...
    if (editing && fileMode) {
      preview = UI.inset(
        UI.insetRow({
          ...
          label: 'Replace',
          value:
            '<span class="dim">Drop a file here, or use the native picker when saving</span>',
        }),
        { variant: 'preview' },
      );
    } else if (editing) {
      var draft =
        s.v.draft != null
          ? s.v.draft
          : editableValue(item, D.plaintextOf(item) || '');
      preview = UI.inset(
        '<textarea aria-label="Contents" data-bind="v.draft">' +
          esc(draft) +
          '</textarea>',
        { variant: 'preview' },
      );
      ...
      preview = UI.inset(
        passwordFields(item.value, shownValue)
          .map(function (f) {
            if (!f.secret) {
              return UI.insetRow({
                variant: 'preview',
                label: esc(FIELD_LABELS[f.field || ''] || f.field),
                value: esc(f.value),
              });
            }
            return UI.insetRow({
      ...
      preview = UI.inset(
        '<div class="pad"><div class="fileglyph"><span class="g">' +
          M.icon('file') +
          '</span>' +
          '<span><b>' +
          esc(D.nameOf(item.path)) +
          '</b><span>' +
          esc(D.fmtSize(item.size)) +
          ' · version ' +
          item.version +
          '</span></span></div>' +
          '<div class="row2">' +
          UI.btn('Download', {
            variant: 'primary',
            icon: 'download',
            attrs: 'data-act="call" data-fn="itemsPanelDownload"',
          }) +
          '</div></div>',
        { variant: 'preview' },
      );
      ...
      preview = UI.inset(
        '<div class="pad"><div class="fileglyph"><span class="g Link">' +
          M.icon('link') +
          '</span>' +
          '<span><b>' +
          esc(D.nameOf(item.path)) +
          ...
          (shownValue === null
            ...
            : 'points to <code>' + esc(shownValue) + '</code>') +
          '</span></span></div>' +
          ...
          '<div class="row2">' +
          UI.btn(shownValue === null ? 'Read target' : 'Open target', {
            variant: 'primary',
            icon: 'arrow',
            attrs:
              'data-act="call" data-fn="' +
              (shownValue === null ? 'itemsShow' : 'itemsPanelOpenTarget') +
              '"' +
              (shownValue === null
                ? ''
                : ' data-arg="' + esc(D.itemKey(item)) + '"'),
          }) +
          '</div></div>',
        { variant: 'preview' },
      );
    ...
    var footnote =
      kind === 'Link'
        ? shownValue === null
          ? 'Read target loads the link from version ' +
            item.version +
            '. FOKS hides it again when the window loses focus.'
          : 'Version ' +
            item.version +
            ' links to ' +
            shownValue +
            ' on ' +
            serverName +
            '. Opening it reads the current item at that path.'
        : '';

    var meta =
      '<div class="meta"><b>Path</b><code>' +
      esc(item.path) +
      '</code>' +
      '<b>Kind</b><span>' +
      esc(displayKind) +
      ...
      '</span>' +
      '<b>Read role</b><span>' +
      UI.chip(esc(roleText(item.read))) +
      '</span>' +
      '<b>Write role</b><span>' +
      UI.chip(esc(roleText(item.write))) +
      '</span></div>';
    ...
    var who =
      ...
      (team && readers
        ? '<p>' +
          esc(D.peopleLabel(readers.length)) +
          ' can read this — everyone in ' +
          esc(store.name) +
          ' at <b>' +
          esc(roleText(item.read)) +
          '</b> or above.</p>' +
          parties
            .map(function (party) {
              ...
              return (
                ...
                ' · ' +
                esc(D.shortId(party.party_id_hex)) +
                ' · gen ' +
                party.generation +
                (canRead ? '' : ' · cannot read this') +
                '</small></span>' +
                UI.chip(esc(roleText(party.destination_role))) +
                '</div>'
              );
            })
            .join('')
    ...
    var foot = editing
      ? UI.btn('Cancel', {
          attrs: 'data-act="call" data-fn="itemsCancelEdit"',
        }) +
        UI.btn('Save version ' + (item.version + 1), {
          variant: 'primary',
          attrs: 'data-act="call" data-fn="itemsSave"',
        })
      : UI.btn('Edit', {
          icon: 'pencil',
          disabled: !change || kind === 'Link',
          title: !change
            ? 'You need ' +
              roleText(item.write) +
        ...
        UI.btn('Copy path', {
          attrs: 'data-act="call" data-fn="itemsCopyPath"',
        }) +
        UI.btn('Remove', {
          variant: 'danger',
          icon: 'trash',
    ...
    return (
      '<aside class="details" aria-label="Details for ' +
      esc(D.nameOf(item.path)) +
      '">' +
      '<div class="dh">' +
      UI.kindIcon(displayKind) +
      '<span class="t"><h2>' +
      esc(D.nameOf(item.path)) +
      '</h2>' +
      '<small>' +
      esc(displayKind) +
      ' in ' +
      esc(store ? store.name : '') +
      '</small></span>' +
      '<button type="button" class="x" title="Close" aria-label="Close" ' +
      'data-act="call" data-fn="itemsCloseDetails">' +
      M.icon('x') +
      '</button></div>' +
      '<div class="scroll">' +
      ...
      preview +
      (editing
        ? '<div class="pfn">Save succeeds only if this item is still version ' +
          item.version +
          '. If it changed, refresh it and try again.</div>'
        : footnote
          ? '<div class="pfn">' + esc(footnote) + '</div>'
          : '') +
  ...
  function saveInOptions(s, w) {
    ...
    return {
      label: 'Save in',
      value: (w.storeById[storeId] || {}).id || createStoreFor(s, w),
      ...
      optionAttrs: function (id) {
        return (
          'data-act="call" data-fn="itemsSaveIn" data-arg="' + esc(id) + '"'
        );
      },
      options: w.navOrder.map(function (candidate) {
        return {
          id: candidate.id,
          title: esc(candidate.name),
          detail: esc(candidate.description),
  ...
  function draftOf(s, kind) {
    var site =
      s.v.site != null ? s.v.site : kind === 'Password' ? 'github.com' : '';
    var rname = s.v.rname || '';
    var derived =
      kind === 'Password'
        ? '/logins/' + site
        : kind === 'Resource'
          ? rname
            ? '/agents/' + rname.toLowerCase().replace(/[^a-z0-9._-]+/g, '-')
            : DRAFT_PATH.Resource
          : DRAFT_PATH[kind];
    return {
      kind: kind,
      ...
      resourceName: rname,
      target: s.v.target || '',
      path: s.v.dpath != null ? s.v.dpath : derived,
    };
  }
  function roleWireOf(s, side) {
    ...
    return {
      role: 'Member',
      visibility: Number(vv(s, side + 'Vis', '0')) || 0,
    };
  }
  function roleLabel(wire) {
    return wire.role === 'Member'
      ? 'Member · visibility ' + (wire.visibility || 0)
      : wire.role;
  }
  /* write-workflows.tsx:198-360 */
  function accessBlock(s, w, store) {
    if (store.kind !== 'team') return '';
    var readWire = roleWireOf(s, 'read'),
      writeWire = roleWireOf(s, 'write');
    var candidate = {
      store: store.id,
      path: '/preview',
      kind: 'Secret',
    ...
    var excluded = roster.filter(function (p) {
      return admitted.indexOf(p) < 0;
    });
    var changers =
      D.readersOf({
        store: store.id,
    ...
    function cards(side, wire, forWrite) {
      return ['Owner', 'Admin', 'Member'].map(function (label) {
        ...
        return UI.radioCard({
          title: label,
          detail: detail,
          selected: wire.role === label,
          attrs:
            'data-act="set" data-key="v.' +
            side +
            'Role" data-val="' +
            label +
            '"',
    ...
    function visRow(side, wire) {
      ...
      return UI.insetRow({
        ...
        forId: id,
        value:
          '<input min="-32768" max="32767" aria-label="' +
          (side === 'read' ? 'Read' : 'Write') +
          ' visibility" id="' +
          id +
          '" type="number" value="' +
          (wire.visibility || 0) +
    ...
    return (
      UI.sectionLabel('Who can read', {
        action:
          '<span class="pv">would be readable by <b>' +
          admitted.length +
          ' of ' +
          roster.length +
          '</b></span>',
      ...
      UI.inset(
        UI.radioGroup(cards('read', readWire, false), {
          label: 'Who can read',
        }) + visRow('read', readWire),
      ) +
      '<p class="hint">At <b>' +
      esc(roleLabel(readWire)) +
      '</b> this item would be readable by <b>' +
      admitted.length +
      ' of ' +
      roster.length +
      '</b> in ' +
      ...
      (excluded.length
        ? ' ' +
          esc(excluded.map(D.partyName).join(', ')) +
          ' is not counted when its role or group membership provides no access here.'
        : '') +
      '</p>' +
      UI.sectionLabel(
        'Who can change <span class="pv">changeable by <b>' +
          changers.length +
          ' of ' +
          roster.length +
          '</b></span>',
      ...
      UI.inset(
        UI.radioGroup(cards('write', writeWire, true), {
          label: 'Who can change',
        }) + visRow('write', writeWire),
      ) +
  ...
  function newSheet(s, w) {
    ...
    var chooser = w.stores.length
      ...
      : UI.insetRow({
          label: 'Vault',
          value: '<span class="dim">No vault is available to save into.</span>',
        });
    ...
    if (kind === 'Password') {
      ...
      fields = UI.insetRow({
        label: 'File',
        value:
          '<span class="dim">Drop a file here, or use the native picker</span>',
      });
    }

    var body =
      UI.sectionLabel('Save in') +
      UI.inset(chooser) +
      (store && blocked
        ? UI.band({ text: esc(store.name + ': ' + blocked) })
        : '') +
      (store ? accessBlock(s, w, store) : '') +
      UI.sectionLabel(esc(D.kindLabel(kind))) +
      UI.inset(fields) +
      UI.toggle({
        label: 'Advanced',
        className: 'sheet-advanced',
        open: s.v.advanced === '1',
        attrs:
          'data-act="set" data-key="v.advanced" data-val="' +
          (s.v.advanced === '1' ? '' : '1') +
          '"',
        body:
          UI.inset(
    ...
    return labelledSheet({
      ariaLabel: 'New ' + D.kindLabel(kind).toLowerCase(),
      width: 'mid',
      ...
      footer:
        UI.btn('Cancel', {
          attrs: 'data-act="call" data-fn="itemsCloseSheet"',
        }) +
        UI.btn(
          kind === 'File' && !s.v.src
            ...
            : 'Create in this vault',
          {
            variant: 'primary',
            disabled: !canWrite,
            attrs: 'data-act="call" data-fn="itemsCreate"',
          },
        ),
    });
  }
  function existsSheet(s, w) {
    ...
    var clash = w.items.filter(function (i) {
      return i.store === storeId && i.path === path;
    })[0];
    ...
    return labelledSheet({
      ...
      subtitle:
        'New ' + esc(D.kindLabel(kind).toLowerCase()) + ' · not created',
      body:
        '<p>Nothing was created and nothing was overwritten. An item already exists at this path' +
        (clash
          ? ', and ' +
            esc(D.nameOf(path)) +
            ...
            esc(store ? store.name : '')
          : '') +
        '. Open the existing item to review it, or choose a different name or path.</p>',
      footer:
        UI.btn('Change the path', {
          attrs: 'data-act="call" data-fn="itemsChangePath"',
        }) +
        UI.btn(clash ? 'Open version ' + clash.version : 'Open existing item', {
          variant: 'primary',
          attrs: 'data-act="call" data-fn="itemsOpenExisting"',
        }),
    });
  }
  ...
  function conflictSheet(s, w) {
    ...
    return labelledSheet({
      ...
      body:
        '<p>Another member saved a newer version while you were editing. Review the latest version and reapply your changes.</p>' +
      ...
      footer:
        UI.btn('Discard my edit', {
          attrs: 'data-act="set" data-key="v.sheet" data-val="discard"',
        }) +
        UI.btn('Refresh and review', {
          variant: 'primary',
          attrs: 'data-act="call" data-fn="itemsRefreshConflict"',
        }),
    });
  }
  function discardSheet(s, w) {
    ...
    return labelledSheet({
      ...
      body:
        '<p>Your draft has not been saved. Discarding it here is the only copy gone — the item itself ' +
        'is untouched at its current version.</p>',
      footer:
        UI.btn('Keep editing', {
          attrs: 'data-act="set" data-key="v.sheet" data-val="conflict"',
        }) +
        UI.btn('Discard my edit', {
          variant: 'primary',
          className: 'danger',
          attrs: 'data-act="call" data-fn="itemsDiscard"',
        }),
    });
  }
  ...
  function removeSheet(s, w) {
    ...
    if (!item) return '';
    return (
      '<div class="backdrop" role="alertdialog" aria-modal="true" aria-labelledby="items-remove-title" ' +
      ...
      sheetPanel({
        ...
        footer:
          ...
          UI.btn('Remove', {
            variant: 'primary',
            className: 'danger',
            attrs: 'data-act="call" data-fn="itemsConfirmRemove"',
          }),
      }) +
      '</div>'
    );
  ...
  function sync(s) {
    ...
    var homelab = D.stores.filter(function (st) {
  ...
  function renderPage(ref) {
    return function (s) {
      ...
      if (
        (s.v.sheet === 'conflict' || s.v.sheet === 'discard') &&
        !s.v.sel &&
        s.v.details == null
      ) {
        var subject = conflictItem(s, w);
        if (subject) {
          s.v.sel = D.itemKey(subject);
          s.v.details = 'shown';
        }
      }
      ...
      if (!ref)
        body += w.bands
          .map(function (band) {
            return UI.band({ text: esc(band.text) });
          })
          .join('');
      if (viewOf(s) === 'grid' && items.length > 200) {
        body += UI.band({
          text: 'Showing the first 200 cards. Narrow the list with search or a kind filter to see the rest.',
        });
      }
      body += !items.length
        ? emptyBody(s, store)
      ...
      if (
        ...
        tail =
          s.v.sheet === 'conflict' ? conflictSheet(s, w) : discardSheet(s, w);
      }
      return {
        appClass: open ? 'with-details' : '',
        main: UI.main(
          pageHeader(s, store) + toolbar(s, w, store, open) + UI.body(body),
        ),
  ...
  function renderOverlay(s) {
    ...
    var covered = !!(
      ref &&
      w.storeById[ref] &&
      w.storeById[ref].state !== 'normal'
    );
    var menu = SHEET_KIND[sheet]
      ? s.v.menu === 'save-in' && w.stores.length
        ? UI.cardSelectMenu(saveInOptions(s, w))
        : ''
      : covered
        ? ''
        : s.v.menu === 'new' || s.v.menu === 'new-empty'
          ? menuPortal('What to create', newMenuHtml())
          : s.v.menu === 'sort'
            ? menuPortal('Sort the list by', sortMenuHtml(s))
            : '';
  ...
  M.fns.itemsDetails = function () {
    ...
    M.set(
      'v.details',
      detailsShown(M.s, w, page && page.ref ? w.storeById[page.ref] : null)
        ? 'hidden'
        : 'shown',
    );
  };
  M.fns.itemsCloseDetails = function () {
    M.set('v.details', 'hidden');
  };
  M.fns.itemsSort = function (arg) {
    M.s.v.menu = null;
    M.set('v.sort', arg === 'name' ? null : arg);
  };
  M.fns.itemsShow = function () {
    M.set('v.reveal', '1');
  };
  M.fns.itemsHide = function () {
    M.set('v.reveal', null);
  };
  ...
  M.fns.itemsDownload = function (arg) {
    ...
    M.s.v.details = 'shown';
    M.toast(
      'Downloaded ' + D.nameOf(item.path) + ' at version ' + item.version,
    );
    M.render();
  };
  ...
  function openTargetOf(arg) {
    ...
    var hit = w.items.filter(function (i) {
      return i.store === item.store && i.path === target;
    })[0];
  ...
  M.fns.itemsCreate = function () {
    ...
    pushOp(
      'new~' +
        kind +
        '~' +
        (SHORT[storeId] || storeId) +
        '~' +
        path +
        '~' +
        wireWord(roles.read) +
        '~' +
        wireWord(roles.write),
    );
    clearDraft();
    s.v.sheet = null;
  ...
  M.fns.itemsSave = function () {
    ...
    if (!(D.kindOf(row) === 'File' || s.v.sheet === 'replace')) {
      TYPED[D.itemKey(row)] =
        s.v.draft != null
          ? s.v.draft
          : editableValue(row, D.plaintextOf(row) || '');
    }
    pushOp(
      'save~' + (SHORT[row.store] || row.store) + '~' + row.path + '~' + next,
    );
    s.v.draft = null;
    s.v.sheet = null;
  ...
  document.addEventListener('click', function (ev) {
    ...
    var el = ev.target;
    if (!el || !el.closest) return;
    if (el.closest('.menuwrap, .menu-portal, .card-select, .mock-controls'))
      return;
    M.set('v.menu', null);
  });
  ...
  var CONTROLS = [
    {
      key: 'view',
      label: 'View',
      note: 'state.view — the list/cards segmented control.',
      values: [
        { v: '', label: 'List' },
        { v: 'grid', label: 'Cards' },
      ],
    },
    {
      key: 'kind',
      label: 'Kind filter',
      note: 'state.kind — the segmented control; the empty state follows it.',
      values: [
        { v: '', label: 'All' },
        { v: 'Password', label: 'Passwords' },
        { v: 'Resource', label: 'Notes' },
        { v: 'File', label: 'Files' },
        { v: 'Link', label: 'Links' },
      ],
    },
    {
      key: 'sort',
      label: 'Sort',
      note: 'scope.ts SORTS. Grouped = store name then item name; version is descending.',
      values: [
        { v: '', label: 'Name' },
        { v: 'kind', label: 'Kind' },
        { v: 'group', label: 'Grouped' },
    ...
    },
    {
      key: 'search',
      label: 'Search',
      note: 'Paths and store names only — never contents. Rows grow their full path.',
      values: [
        { v: '', label: 'None' },
        { v: 'wifi', label: 'wifi' },
        { v: 'logins', label: 'logins' },
        { v: 'Household', label: 'Household' },
        { v: 'zzzz', label: 'zzzz (no match)' },
      ],
    },
    {
      key: 'sel',
      label: 'Selection',
      note: 'M.data.itemKey — "<storeRef>|<path>". A selection forces the panel open.',
      values: [
        { v: '', label: 'None' },
        { v: 'acct:personal|/logins/github.com', label: 'github.com' },
        {
          v: 'acct:personal|/agents/anthropic-api-key',
        ...
        {
          v: 'acct:personal|/documents/passport-scan.pdf',
          label: 'passport-scan.pdf',
        },
        {
          v: 'team:household|/documents/emergency.pdf',
          label: 'emergency.pdf',
        },
    ...
    },
    {
      key: 'details',
      label: 'Details panel',
      note: 'The toolbar toggle. Null follows the selection; deselecting leaves it open.',
      values: [
        { v: '', label: 'Follows the selection' },
        { v: 'shown', label: 'Open' },
        { v: 'hidden', label: 'Closed' },
      ],
    },
    {
      key: 'reveal',
      label: 'Reveal',
      note: 'The one exact-version read: Show, or Read target on a Link.',
      values: [
        { v: '', label: 'Masked' },
        { v: '1', label: 'Revealed' },
      ],
    },
    {
      key: 'sheet',
      label: 'Sheet',
      note: 'The write overlays; edit/replace are the details panel’s two editing modes.',
      values: [
        { v: '', label: 'None' },
        { v: 'new-password', label: 'New password' },
        { v: 'new-resource', label: 'New note' },
        { v: 'new-file', label: 'New file' },
        { v: 'new-link', label: 'New link' },
        { v: 'edit', label: 'Edit' },
        { v: 'replace', label: 'Replace' },
        { v: 'remove', label: 'Remove' },
        { v: 'exists', label: 'Already exists' },
        { v: 'conflict', label: 'Edit conflict' },
    ...
    },
    {
      key: 'menu',
      label: 'Menu',
      note: 'MenuButton open-ness is view state here; save-in is the sheet’s CardSelect.',
      values: [
        { v: '', label: 'None' },
        { v: 'new', label: 'New' },
        { v: 'new-empty', label: 'New (from the empty state)' },
        { v: 'sort', label: 'Sort' },
    ...
    },
    {
      ...
      note: 'Which store the New sheet writes to — the store being looked at, or defaultCreateStore, when unset.',
      values: [
        { v: '', label: 'Default' },
        { v: 'acct:personal', label: 'Personal' },
        { v: 'acct:work', label: 'Work (Acme)' },
        { v: 'team:eng', label: 'Engineering' },
        { v: 'team:household', label: 'Household' },
        { v: 'team:homelab', label: 'Homelab' },
      ],
    },
    {
      key: 'readRole',
      label: 'Create read role',
      note: 'The group create’s read role; readVis carries the Member band.',
      values: [
        { v: '', label: 'Member' },
        { v: 'Admin', label: 'Admin' },
        { v: 'Owner', label: 'Owner' },
    ...
    {
      key: 'writeRole',
      label: 'Create write role',
      note: 'The group create’s write role; writeVis carries the Member band.',
      values: [
        { v: '', label: 'Admin' },
        { v: 'Owner', label: 'Owner' },
    ...
    {
      key: 'advanced',
      label: 'Advanced disclosure',
      note: 'The New sheet’s Path disclosure.',
      values: [
        { v: '', label: 'Closed' },
        { v: '1', label: 'Open' },
      ],
    ...
    {
      key: 'ekind',
      label: 'Refused create kind',
      note: 'Which create the `exists` refusal is about — the sheet’s “New {kind} · not created”.',
      values: [
        { v: '', label: 'Password' },
        { v: 'Resource', label: 'Note' },
        { v: 'File', label: 'File' },
        { v: 'Link', label: 'Link' },
      ],
    },
    {
      key: 'epath',
      label: 'Refused create path',
      note: 'The path the `exists` refusal is about; the clashing item’s version comes from the catalog.',
      values: [
        { v: '', label: 'From the kind' },
        { v: '/logins/github.com', label: '/logins/github.com' },
    ...
    },
    {
      ...
      note: 'The editor’s textarea — what “Refresh and review” brings back beside the refreshed version.',
      values: [
        { v: '', label: 'From the item' },
        { v: 'flint-Harbor-19-quay', label: 'flint-Harbor-19-quay' },
      ],
    },
    {
      key: 'inspect',
      label: 'Response disclosure',
      note: 'The details panel’s “Inspect response” disclosure.',
      values: [
        { v: '', label: 'Closed' },
        { v: '1', label: 'Open' },
      ],
    ...
    {
      key: 'applied',
      label: 'Writes applied',
      note: 'The mutations, replayed onto the fixture each render — clicking Create / Save / Remove in the frame pushes the same ops.',
      values: [
        { v: '', label: 'Nothing written' },
        { v: 'created-password', label: 'Created gitlab.com' },
        { v: 'created-note', label: 'Created deploy-token' },
        { v: 'created-file', label: 'Created insurance.pdf' },
        { v: 'created-link', label: 'Created current-key' },
        { v: 'saved-github', label: 'Saved github.com v10' },
         ...
         sidebar's Homelab caption follows it), not just the Homelab page. */
    {
      ...
      note: 'resumeGroupCreation landed: Homelab becomes an active ad-hoc vault instead of the Setup-incomplete takeover.',
      values: [
        { v: '', label: 'Not resumed' },
        { v: 'yes', label: 'Resumed' },
      ],
  ...
  Object.keys(ITEM_PAGES).forEach(function (id) {
    ...
    M.page({
      ...
      ref: ref,
      note: ref
        ? 'items-screen.tsx scoped to ' +
          ref +
          '; a store-access problem replaces the whole page.'
