...
try {
  M.showDim = localStorage.getItem('foksMockDim') !== 'hidden';
} catch {
  /* file:// */
}
...
M.icon = function (name, size, extra) {
  ...
  if (!body)
    return (
      '<svg class="ic" viewBox="0 0 24 24" aria-hidden="true" data-missing-icon="' +
      esc(name) +
      '"></svg>'
    );
  var attrs =
    'class="ic' +
    (extra && extra.cls ? ' ' + extra.cls : '') +
    '" viewBox="0 0 24 24" aria-hidden="true" focusable="false" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"';
...
M.toast = function (text, opts) {
  ...
  var live = M.toasts.filter(function (t) {
    return (t.dedupeKey || t.text) === key;
  })[0];
  var entry;
  if (live) {
    clearTimeout(live.timer);
    clearTimeout(live.exitTimer);
    entry = live;
    entry.text = text;
    entry.tone = opts.tone === 'warning' ? 'warning' : 'info';
    ...
    entry = {
      ...
      tone: opts.tone === 'warning' ? 'warning' : 'info',
      action: opts.action || null,
      show: true,
    };
    M.toasts.push(entry);
    /* MAX_VISIBLE_TOASTS: `.slice(-5)` drops the oldest. */
    while (M.toasts.length > M.TOAST_MAX) {
      var gone = M.toasts.shift();
      clearTimeout(gone.timer);
      clearTimeout(gone.exitTimer);
    }
  }
  ...
  if (!persistent) {
    var ms = Math.max(
      0,
      opts.durationMs === undefined ? M.TOAST_MS : opts.durationMs,
    );
...
M.fns.toastDismiss = function (id) {
  M.dismissToast(id);
};
M.fns.toastAction = function (id) {
  var t = M.toasts.filter(function (x) {
    return String(x.id) === String(id);
  })[0];
  try {
    if (t && t.action && t.action.onAction) t.action.onAction();
  } finally {
    M.dismissToast(id);
  }
};
...
M.app = function (group) {
  ...
  M.appGroups = M.appGroups.filter(function (g) {
...
M.flow = function (flow) {
  if (
    !M.flows.some(function (f) {
      return f.id === flow.id;
    })
  )
    M.flowOrder.push(flow.id);
  M.flows = M.flows.filter(function (f) {
    return f.id !== flow.id;
  });
  M.flows.push(flow);
  return flow;
...
M.current = function () {
  return M.pages[M.s.page] || M.pages[M.pageOrder[0]];
};
...
function viewGroups() {
  ...
  var live = {};
  (cur ? cur.controls : []).forEach(function (g) {
    live[g.key] = true;
    out.push({ g: g, applies: true });
  });
...
M.go = function (pageId, patch) {
  if (pageId !== M.s.page && M.pages[pageId]) {
    ...
    M.leaveKeys.forEach(function (k) {
      M.s.v[k] = null;
    });
    ...
    var keeps = {};
    M.pages[pageId].controls.forEach(function (g) {
      keeps[g.key] = true;
    });
    (M.pages[pageId].keeps || []).forEach(function (k) {
      keeps[k] = true;
    });
    Object.keys(M.s.v).forEach(function (k) {
      if (!keeps[k]) M.s.v[k] = null;
    });
  }
...
function encodeState() {
  ...
  M.appGroups.forEach(function (g) {
    if (M.s[g.key] != null && M.s[g.key] !== g.values[0].v)
      p.set(g.key, M.s[g.key]);
  });
  Object.keys(M.s.v).forEach(function (k) {
    if (M.s.v[k] != null) p.set(k, M.s.v[k]);
  });
  if (M.currentFlow) {
    p.set('flow', M.currentFlow.id);
    p.set('step', String(M.step + 1));
  }
  return '?' + p.toString();
}
function decodeState() {
  ...
  p.forEach(function (val, key) {
    if (key === 'page') {
      if (M.pages[val]) M.s.page = val;
    } else if (key === 'flow' || key === 'step') {
      /* below */
    } else if (appKeys[key]) M.s[key] = val;
    else M.s.v[key] = val;
  });
  if (flowId) {
    var f = M.flows.filter(function (x) {
      return x.id === flowId;
    })[0];
    if (f) {
      M.currentFlow = f;
...
M.flowMove = function (delta) {
  ...
  var at = M.currentFlow
    ? M.flowOrder.indexOf(M.currentFlow.id)
    : delta > 0
      ? -1
      : M.flowOrder.length;
  var next = at + delta;
  ...
  if (!lit && current != null && current !== '') {
    ...
    html += chip(
      ...
      'data-act="set" data-key="' +
        ...
        esc(current) +
        '" title="' +
        esc(text + ' — set to a value this group does not declare.') +
        '"' +
        (applies ? '' : ' disabled'),
      'custom',
    );
...
document.addEventListener(
  ...
  function (ev) {
    var el = ev.target;
    if (el && el.classList && el.classList.contains('mc-scroll'))
      markScroll(el);
  },
  true,
);
function renderControls() {
  var cur = M.current();
  /* app-wide */
  document.getElementById('mcAppGroups').innerHTML = M.appGroups
    .map(function (g) {
      var applies = !g.appliesTo || g.appliesTo(cur, M.s);
      return groupHTML(
        { key: g.key, label: g.label, values: g.values, note: g.note },
        applies,
        M.s[g.key],
      );
    })
  ...
  var applying = groups.filter(function (x) {
    return x.applies;
  });
  var rest = groups.filter(function (x) {
    return !x.applies;
  });
  var html = applying
    .map(function (x) {
      return groupHTML(x.g, true, M.s.v[x.g.key], 'v.');
    })
    .join('');
  if (!applying.length)
    html += '<div class="mc-note">This page has no view-specific state.</div>';
  ...
  var strays = Object.keys(M.s.v).filter(function (k) {
    return M.s.v[k] != null && M.s.v[k] !== '' && !mine[k];
  });
  if (strays.length) {
    html +=
      '<div class="mc-note">Also set, but not declared by this page: ' +
      strays
        .map(function (k) {
          return '<code>' + esc(k) + '=' + esc(M.s.v[k]) + '</code>';
        })
        .join(', ') +
  ...
  if (rest.length && M.showDim) {
    html +=
      ...
      rest
        .map(function (x) {
          return groupHTML(x.g, false, M.s.v[x.g.key], 'v.');
        })
        .join('');
  }
  dimBtn.hidden = !rest.length;
  dimBtn.textContent =
    (M.showDim ? 'Hide ' : 'Show ') + rest.length + ' groups other pages read';
  ...
  document.getElementById('mcFlowList').innerHTML = M.flowOrder
    .map(function (id, i) {
      var f = M.flows.filter(function (x) {
        return x.id === id;
      })[0];
      ...
      return chip(
        num,
        M.currentFlow && M.currentFlow.id === f.id,
        'data-act="flow" data-flow="' +
          esc(f.id) +
          '" title="' +
          esc(tip) +
          '" aria-label="' +
  ...
  if (M.currentFlow) {
    ...
    stepsEl.innerHTML = M.currentFlow.steps
      .map(function (st, i) {
        ...
        var where =
          ...
          st.label +
          ' — ' +
          (pg ? pg.path.join(' › ') : st.page);
        return (
          '<button class="mc-step' +
          (i === M.step ? ' on' : '') +
          '" data-act="flow-step" data-step="' +
          i +
          '"' +
          ' title="' +
          esc(where) +
    ...
    nav.querySelector('[data-act="flow-next"]').disabled =
      M.step >= M.currentFlow.steps.length - 1;
    document.getElementById('mcStepNote').textContent =
      (M.currentFlow.steps[M.step] && M.currentFlow.steps[M.step].note) || '';
    var flowIdx = M.flowOrder.indexOf(M.currentFlow.id) + 1;
    document.getElementById('mcFlowSub').innerHTML =
      (flowIdx ? '#' + flowIdx + ' · ' : '') +
      esc(M.currentFlow.title) +
      ' · step ' +
      (M.step + 1) +
      ' of ' +
      M.currentFlow.steps.length +
      ' <button class="mock-chip sub" data-act="flow-stop">leave</button>';
  } else {
    stepsEl.innerHTML = '';
    nav.hidden = true;
    document.getElementById('mcFlowSub').textContent =
      'pick one to step through it';
  }
  /* page picker */
  ...
  btn.innerHTML =
    path
      .slice(0, -1)
      .map(function (c) {
        return '<span class="crumb">' + esc(c) + '</span>';
      })
      .join('') +
    '<span class="cur">' +
    esc(path[path.length - 1]) +
  ...
  var sibs = cur
    ? M.pageOrder.filter(function (id) {
        var p = M.pages[id];
        return (
          p.path.length === cur.path.length &&
          p.path.slice(0, -1).join('/') === cur.path.slice(0, -1).join('/')
        );
      })
    : [];
  quick.innerHTML =
    sibs.length > 1
      ? sibs
          ...
          .join('')
      : '';
  ['mcAppGroups', 'mcViewGroups'].forEach(function (id) {
    markScroll(document.getElementById(id));
  });
}
...
function renderTree() {
  ...
  M.pageOrder.forEach(function (id) {
    ...
    p.path.slice(0, -1).forEach(function (seg) {
      if (!node.kids[seg]) {
        node.kids[seg] = { kids: {}, order: [], pages: [] };
        node.order.push(seg);
      }
      node = node.kids[seg];
    });
...
function renderOverlays() {
    ...
    M.toasts
      .map(function (t) {
        ...
        return (
          ...
          t.tone +
          '"' +
          (warn ? ' role="alert" aria-live="assertive"' : '') +
          '>' +
          '<span class="toast-message">' +
          esc(t.text) +
          '</span>' +
          (t.action
            ? '<button type="button" class="toast-action" data-act="call" data-fn="toastAction" data-arg="' +
              t.id +
              '">' +
              esc(t.action.label) +
              '</button>'
            : '') +
          '<button type="button" class="toast-dismiss" aria-label="Dismiss notification" data-toast-dismiss="true" data-act="call" data-fn="toastDismiss" data-arg="' +
          t.id +
          '">×</button></div>'
        );
      })
  ...
  var pageOverlay =
    cur && cur.overlay && !M.inTakeover ? cur.overlay(M.s) || '' : '';
  ...
  if (!firstPaintDone) {
    M.overlayFirst = !!pageOverlay;
    firstPaintDone = true;
  } else if (!pageOverlay) M.overlayFirst = false;
  var forced =
    M.overlayOrderOverride !== undefined
      ? M.overlayOrderOverride
      : cur && cur.overlayOrder !== undefined
        ? cur.overlayOrder
        : undefined;
  var first =
    forced === 'before' ? true : forced === 'after' ? false : M.overlayFirst;
  var parts = splitPortals(pageOverlay);
  var html =
    (first ? parts[0] : '') + toasts + (first ? '' : parts[0]) + parts[1];
  ...
  if (app) {
    ...
    var modal =
      M.appInert ||
      !!document.querySelector('#win > [aria-modal="true"]') ||
...
M.render = function () {
  ...
  if (!cur) {
    document.getElementById('win').innerHTML =
      '<div style="padding:20px">No pages registered.</div>';
    return;
  }
  ...
  if (out.takeover) {
    ...
    html +=
      out.titlebar !== undefined
        ? out.titlebar
        : M.renderTitlebar
          ? M.renderTitlebar(M.s, cur)
          : '';
    ...
    html +=
      (out.windowTail !== undefined
        ? out.windowTail
        : M.windowTail
          ? M.windowTail(M.s, cur)
          : '') || '';
  }
  ...
  if (active && win.contains(active)) {
    sig =
      active.getAttribute('data-key') != null
        ? '[data-key="' +
        ...
        : active.getAttribute('aria-label')
          ? '[aria-label="' +
            active.getAttribute('aria-label').replace(/"/g, '\\"') +
            '"]'
          : active.id
            ? '#' + active.id
            : null;
  }
  var hadBackdrop = !!document.querySelector(
    '#overlays .backdrop, #win .backdrop',
  );
  win.innerHTML = html;
  M._refocus = function () {
    ...
    if (backdrop && !hadBackdrop) {
      var first = backdrop.querySelector(
        'input:not([disabled]), textarea, select, button:not([disabled]), [tabindex="0"]',
      );
      if (first) {
        try {
  ...
  win.className =
    'window web-mock-window' + (out.windowClass ? ' ' + out.windowClass : '');
  renderOverlays();
  renderControls();
  ...
  M.appGroups.forEach(function (g) {
    var val = g.values.filter(function (o) {
      return String(o.v) === String(M.s[g.key]);
    })[0];
    if (val && val.band) lines.push(val.band);
  });
  var bandText = lines.filter(Boolean).join('<br />');
  band.hidden = !bandText;
...
function runAct(t, ev) {
  ...
  if (act === 'set')
    ...
    if (fn) fn(t.getAttribute('data-arg'), t, ev);
  } else if (act === 'go') {
...
document.addEventListener('click', function (ev) {
  ...
  if (ev.target.closest('#mcPickerBtn')) {
    ...
    document
      .getElementById('mcPickerBtn')
      .setAttribute('aria-expanded', String(!tree.hidden));
    ...
    if (!tree.hidden) {
      ...
      if (here)
        tree.scrollTop = Math.max(
          0,
          here.offsetTop - tree.offsetTop - tree.clientHeight / 2,
        );
    }
    return;
  }
  ...
  if (act === 'set') {
    ...
    var fn = M.fns[t.getAttribute('data-fn')];
    if (fn) fn(t.getAttribute('data-arg'), t, ev);
    else M.hint('No handler "' + t.getAttribute('data-fn') + '".');
  } else if (act === 'stop') {
    ...
    if (M.currentFlow && M.currentFlow.id === id) M.stopFlow();
    else M.startFlow(id, 0);
  } else if (act === 'dim-toggle') {
    M.showDim = !M.showDim;
    try {
      localStorage.setItem('foksMockDim', M.showDim ? 'shown' : 'hidden');
    } catch {
      /* file:// */
    }
    renderControls();
  } else if (act === 'flow-step') {
    M.step = parseInt(t.getAttribute('data-step'), 10);
    applyStep();
  } else if (act === 'flow-prev') M.flowStep(-1);
  else if (act === 'flow-next') M.flowStep(1);
  else if (act === 'flow-stop') M.stopFlow();
  if (t.closest('#mcTree')) {
    /* keep the tree open only for summaries */
  }
});
...
document.addEventListener('keydown', function (ev) {
  ...
  if (M.currentFlow && ev.key === 'ArrowRight') {
    M.flowStep(1);
    ev.preventDefault();
  } else if (M.currentFlow && ev.key === 'ArrowLeft') {
    M.flowStep(-1);
    ev.preventDefault();
...
document.addEventListener('input', function (ev) {
  ...
  if (key.indexOf('v.') === 0) M.s.v[key.slice(2)] = ev.target.value;
  else M.s[key] = ev.target.value;
...
document.addEventListener('change', function (ev) {
  ...
  if (key && ev.target.type === 'checkbox')
    M.set(key, ev.target.checked ? '1' : '');
});
