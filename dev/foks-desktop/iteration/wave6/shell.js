...
const FX = {
  ...
  accounts: [ { alias:"personal", username:"rae", server:"personal" }, { alias:"work", username:"rae.chen", server:"acme" } ],
  stores: [
  ...
  parties: [
    ...
    { store:"team:eng", username:"rae.chen", label:"you", party_kind:"user", generation:6, locally_manageable:true, party_id_hex:"016ab09cd4e11f207877c31b900b44aa193d90c1e45f22c8d13e91c7a29c04e211", source_role:{role:"Admin"}, destination_role:{role:"Admin"} },
    { store:"team:eng", username:"dana.okafor", party_kind:"user", generation:5, locally_manageable:true, party_id_hex:"012f8b6d13c07a4e5991b2f8d04a6c3e17bd50927fe83a1c4670d9b2e518f4a06c", source_role:{role:"Member", visibility:0}, destination_role:{role:"Member", visibility:0} },
    { store:"team:eng", username:null, party_kind:"named-team", team_name:"homelab @ foks.example.net", generation:3, locally_manageable:false, party_id_hex:"038b17e4a055c93d2fe60148ab7f2c9d3140e5b8a71c6f93d20ba48e5739d1c0f6", scoped_host_id_hex:"9f31c2aa07", source_role:{role:"Owner"}, destination_role:{role:"Member", visibility:0} },
    { store:"team:eng", username:"deploy-bot", party_kind:"user", generation:6, locally_manageable:false, party_id_hex:"01c93d2f8b17e4a0148ab7f2e60155c9a71c6f93d23140e5b839d1c0f60ba48e57", source_role:{role:"Member", visibility:0}, destination_role:{role:"Member", visibility:0} },
    { store:"team:household", username:"rae", label:"you", party_kind:"user", generation:2, locally_manageable:true, party_id_hex:"016ab09cd4e11f207877c31b900b44aa193d90c1e45f22c8d13e91c7a29c04e211", source_role:{role:"Owner"}, destination_role:{role:"Owner"} },
  ...
  notifications: [
    { id:"lease-acme", severity:"crit", title:"Nothing on foks.acme-corp.com can be read or changed", detail:"The compatibility lease for foks.acme-corp.com has expired. Work and Engineering groups are unavailable until the agent renews it.", action:"Wait for lease refresh" },
...
/* ===================== corrections (WAVE4-BRIEF §1) =====================
   ...
   ======================================================================== */
FX.parties.splice(2, 0, { store:"team:eng", username:"priya.n", party_kind:"user", generation:5, locally_manageable:true, party_id_hex:"0184f0c2a71b3d95e6082c47fa19b6d3e05c84719fb2a6c308d5e71f94c3b0d726", source_role:{role:"Admin"}, destination_role:{role:"Admin"} });
(p => { p.locally_manageable = true; p.note = "a service account — a user of foks.acme-corp.com like any other, so its role can be changed from here"; })(FX.parties.find(p => p.username === 'deploy-bot'));
(p => { p.note = "a member group; change it from Engineering's Federation page (Groups), not from here"; })(FX.parties.find(p => p.party_kind === 'named-team'));
...
function setLease(state) {
  ...
  acme.lease = state === 'lapsed' ? { state:'lapsed', expires_in:null } : { state:'fresh', expires_in:'12 d' };
...
const COPY = {
  lease_lapsed: "Access to this server is blocked until the agent renews its compatibility lease. Items on this server are hidden and cannot be changed. Other servers are unaffected.",
...
const fmtSize = b => b === 0 ? '0 B' : b < 1000 ? b + ' B' : b < 1e6 ? (b/1000).toFixed(b < 10000 ? 1 : 0) + ' KB' : (b/1e6).toFixed(1) + ' MB';
const fmtRole = r => r.role === 'Member' ? `Member · visibility ${r.visibility}` : r.role;
...
const pname = p => p.username || p.team_name || p.party_id_hex.slice(0, 10) + '…';
const initials = s => s.replace(/@.*/, '').split(/[.\-_ ]/).filter(Boolean).slice(0, 2).map(w => w[0].toUpperCase()).join('');
const HUES = ["#5e5ce6","#ff9f0a","#30b0c7","#ff2d55","#34c759","#af52de","#0a7cff","#a2845e"];
const hue = s => HUES[[...s].reduce((a, c) => a + c.charCodeAt(0), 0) % HUES.length];

/* avatars / stacks */
const avatar = (p, cls='av') => p.party_kind === 'named-team'
...
const stackOf = (parties, cls='') => `<span class="stack ${cls}">${parties.slice(0, 2).map(p => avatar(p)).join('')}</span>`;
...
const KINDS = {
  Password:{ plural:'Passwords', icon:'key',  blurb:'A login: user, password and website — read inline.' },
  Resource:{ plural:'Resources', icon:'term', blurb:'A token, key, connection string or note — read inline.' },
  File:    { plural:'Files',     icon:'file', blurb:'A document or bundle — read in version-bound chunks.' },
  Link:    { plural:'Links',     icon:'link', blurb:'A pointer to another path in the same store.' },
};
const KIND_LIST = Object.keys(KINDS);
const kindOf = it => it.kind === 'Secret'
  ? ((/^password:/m.test(it.value || '') || it.path.startsWith('/logins/')) ? 'Password' : 'Resource')
  : it.kind;
/* the node type the kind is a reading of */
const rtype = it => ({ Secret:'small_file', File:'file', Link:'symlink', Folder:'directory' })[it.kind];
const rtypeWords = it => ({ Secret:'a Secret', File:'a File node', Link:'a symlink', Folder:'a directory' })[it.kind];
const isLogin = it => kindOf(it) === 'Password' && it.path.startsWith('/logins/');
/* tinted square (rows, details header, sheets) */
const kic = (k, extra='') => `<span class="kic ${k} ${extra}">${ic(KINDS[k].icon)}</span>`;
/* solid square or site initial (tiles) */
function kico(it, size='') {
  if (isLogin(it)) { const d = nameOf(it.path); return `<span class="kico ${size}" style="background:${hue(d)}">${esc(d[0].toUpperCase())}</span>`; }
  const k = kindOf(it); return `<span class="kico ${k} ${size}">${ic(KINDS[k].icon)}</span>`;
}
const kglyph = (k, size='') => `<span class="kico ${k} ${size}">${ic(KINDS[k].icon)}</span>`;

/* ===================== roles, readers ===================== */
/* Member(-0x4000) < Member(0) < Admin < Owner. 0 is the default; lower bands see less. */
const parseRole = r => {
  if (typeof r !== 'string') return { role:r.role, visibility: r.visibility ?? 0 };
  const m = /^Member(?:\s*·\s*visibility\s*(-?\d+))?$/.exec(r);
  return m ? { role:'Member', visibility: m[1] != null ? +m[1] : 0 } : { role:r, visibility:0 };
};
const roleRank = r => ({ Owner:3, Admin:2, Member:1 })[parseRole(r).role] || 0;
/* does `held` (a party's destination role) admit reading something whose read role is `need`? */
function admits(held, need) {
  const h = parseRole(held), n = parseRole(need), hr = roleRank(h), nr = roleRank(n);
...
function admissionActive(p, storeId) {
  ...
  const f = (FX.federation || []).find(f => f.store === storeId && f.remote_team_id_hex === p.party_id_hex);
  return f ? !!f.active : true;
}
function readersOf(it) {
  const st = store(it.store);
  if (st.kind !== 'team') return null;
  return partiesOf(st.id).filter(p => admissionActive(p, st.id) && admits(p.destination_role, it.read));
}
/* "5 people · 1 group" — a party that is a team is not a person */
function peopleGroups(parties) {
  const groups = parties.filter(p => p.party_kind === 'named-team').length, ppl = parties.length - groups;
  const a = `${ppl} ${ppl === 1 ? 'person' : 'people'}`;
  return groups ? `${a} · ${groups} ${groups === 1 ? 'group' : 'groups'}` : a;
}
const readChip = it => {
  const rs = readersOf(it);
  return rs ? `<span class="chip" title="${esc(rs.map(pname).join(', '))}">${people(rs.length)}</span>`
            : `<span class="chip">only you</span>`;
};

/* ===================== lease / listability ===================== */
const leaseLapsed = sid => serverOf(sid).state === 'lease-lapsed';
const storeReadable = sid => { const st = store(sid); return !leaseLapsed(sid) && (st.kind !== 'team' || st.active); };
const catalog = () => FX.items.filter(i => i.kind !== 'Folder' && storeReadable(i.store));
const notesNow = () => FX.notifications.filter(n => n.id !== 'lease-acme' || FX.leaseState === 'lapsed');
const whereOf = it => { const st = store(it.store); return `${st.name} · ${serverOf(it.store).name}`; };
...
const storeDescription = st => {
  ...
  return st.kind === 'account' ? serverOf(st.id).name : peopleGroups(partiesOf(st.id));
};

/* ===================== sidebar (04's, with Set up again) ===================== */
/* active: 'all' | a store id | 'issues' | 'start' | 'team' | 'servers' | 'settings'
   opts: { stores, statusExtra, note, setup:{steps,current,caption}, footDisabled,
           issues (count), setupAgain (href or false) } */
function sidebar(active, opts = {}) {
  const o = Object.assign({ stores: FX.stores, setupAgain: '02-first-run.html?state=who' }, opts);
  const navRow = (id, glyph, name, cap, dot, off, tail = '') =>
    `<button class="nav ${active === id ? 'on' : ''} ${off ? 'off' : ''}" data-act="nav" data-id="${esc(id)}">${glyph}<span class="t">${esc(name)}${cap ? `<small>${esc(cap)}</small>` : ''}</span>${dot ? '<span class="dot" title="Has issues"></span>' : ''}${tail}</button>`;

  if (o.setup) {  /* first-run mode of the same sidebar */
    const rows = o.setup.steps.map((s, i) =>
      `<button class="nav ${s.state || 'todo'}" data-act="go" data-id="${i}"><span class="sdot"></span><span class="t">${esc(s.label)}</span></button>`).join('');
    return rows +
      `<div class="foot">
  ...
  const storeRow = st => {
    ...
    return navRow(st.id, glyph, st.name, storeDescription(st), connectionError, connectionError);
  };
  const badge = o.issues != null ? o.issues : notesNow().length;
  const foot = `<div class="foot">
      ...
      <a class="nav ${active === 'issues' ? 'on' : ''}" href="01-vault.html?state=issues">${ic('bell')}<span class="t">Issues</span>${badge ? `<span class="badge">${badge}</span>` : ''}</a>
  ...
  return navRow('all', ic('grid'), 'All items', '', false, false) +
    `<h6>Vaults</h6>` + o.stores.filter(s => s.kind === 'account').map(storeRow).join('') +
    `<h6>Groups</h6>` + o.stores.filter(s => s.kind === 'team').map(storeRow).join('') +
    (o.statusExtra || '') +
    (o.note ? `<p class="fn">${o.note}</p>` : '') + foot;
}
...
function headerParts(location) {
  if (location && typeof location === 'object') return { title: location.title, sub: location.subtitle || '', lead:'', tail:'' };
  if (location === 'all') {
    return { title:'All items', sub:'', lead:'', tail:'' };
  }
  const st = store(location);
  if (st.kind === 'team') {
    return { title: st.name, sub: storeDescription(st), lead: stack(st.id, 'lg'), tail: storeDescriptionState(st) === 'normal' ? `<button class="btn cap" data-act="sheet" data-id="manage" data-store="${st.id}" title="People and roles in ${esc(st.name)}">Manage</button>` : '' };
  }
...
function openSheet(html) { const o = document.getElementById('overlay'); if (o) o.innerHTML = `<div class="backdrop">${html}</div>`; }
function closeSheet() { const o = document.getElementById('overlay'); if (o) o.innerHTML = ''; }
function flash(msg, host) {
  const w = host || document.getElementById('win'); if (!w) return;
  const old = w.querySelector('.flash'); if (old) old.remove();
  const el = document.createElement('div'); el.className = 'flash'; el.textContent = msg; w.appendChild(el);
  clearTimeout(flash.t); flash.t = setTimeout(() => el.remove(), 1900);
}
function review(states, current, extra = '') {
  const r = document.getElementById('review'); if (!r) return;
  r.innerHTML = `<b>Review controls · not part of the app</b>` +
    states.map(k => `<button class="${current === k ? 'on' : ''}" data-act="state" data-id="${k}">${k}</button>`).join('') + extra;
}
const getState = (def, param = 'state') => new URLSearchParams(location.search).get(param) || def;
