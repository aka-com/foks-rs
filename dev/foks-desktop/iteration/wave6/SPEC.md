...
01-vault.html     Items: All items, vaults, groups, the New sheet, errors,
                  Manage, Issues, and the footer panes — the Servers and
...
03-groups.html    Groups: the group list, the roster with Reads N of M, the
                  party panel with Connect an agent, Federation, Store,
                  Danger, and the invite / add / demote / remove / admit /
...
| Left sidebar | `wave1b/04-card-vault.html` | "All items" on top with no section label; VAULTS show their server; active GROUPS show their people/group count without a server. A blocked, unavailable, lapsed, or inactive row is dimmed, carries an amber dot, and reads "Connection error". Footer rows are Join or create a group · Servers & devices · Issues with badge · Settings, plus **Set up again** below Settings. |
...
lease · inactive · exists · conflict · issues · join · invite ·
