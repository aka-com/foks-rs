...
| 2 Inconsistent visibility band ranges | **Resolved** | Both steppers utilize `clampVis` (−16384…16383) against `FX.copy.visibility`. Both forms now align on a single visibility axis, making band ≥ 1 reachable across items and members. (See item N1 for subsequent validation issues). |
...
| 6 Default views conceal safety state | **Resolved** | `FX.servers[acme].lease` is initialized to `fresh`, and individual views call `setLease` explicitly to establish required test states with accompanying notes, eliminating silent fixture overrides. (See item N3 regarding badge count implications). |
