...
**Standing rule for this audit: wire fidelity beats UI comfort.** Where a screen shows
something the protocol cannot carry, the screen loses the affordance. We do not invent
client-side substitutes to preserve a nicer picture.
...
Consequence, and it is retroactive: **any UI that branches on a kind of failure is not
implementable today.** That includes the rollback banner kept in finding 5 and the
capability-denied state introduced in finding 4. The only failure the client can respond
to specifically is the ambiguous deadline — which is, fortunately, the one that most
...
`ListTeamKv` contract.
Everything else renders one generic failure state carrying the daemon's message verbatim.
No substring matching on daemon prose, anywhere, ever.
...
## Round 4 findings — the reframing

Rounds 1–3 checked operations, payloads and preconditions and assumed the product model
itself was sound. It is not.
...
`list_kv` calls `require(Capability::Kv)` on its first line (`kv.rs:5`). **Reads stop
first.** Telling a user their data stays readable when it does not is the worst class of
error in this set.
...
Also: the PUK is **caller-supplied** (`YubiRetryConfiguration.puk`), never generated or
returned, so displaying a generated unlock code is a security-shaped invention. And every
resume path requires the PIN (`ResumeYubiAccount { …, pin }`), which no mock prompts for.
...
What survives is still the right shape: one catalog keyed by store, response-backed store
details, a queue of things needing attention, and resumable work presented as normal. The
protocol work in `ENGINEERING-PLAN.md` §5 is what turns the gated parts on, and this audit
has reordered its priorities — **group E (error taxonomy) moves up**, because without it
several screens cannot tell the user why anything failed.
