 ...
 * visible one, how tall the spacers representing the unmounted rows must
...
export interface VirtualListWindow {
  ...
  /** Spacer height representing the rows before `start`. */
  padTop: number;
  /** Spacer height representing the rows from `end` onward. */
  padBottom: number;
}

const EMPTY: VirtualListWindow = { start: 0, end: 0, padTop: 0, padBottom: 0 };

/** A valid layout height: negative, NaN, and Infinity all mean "unknown". */
...
export function virtualListWindow(input: VirtualListInput): VirtualListWindow {
  ...
  // until the browser adjusts the scroll position.
  const top = Math.min(
