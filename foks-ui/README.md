...
queries the agent's authenticated pending-operation list before offering a
...
```sh
pnpm run foks:dev:frontend # Vite dev server on http://127.0.0.1:1421 (strict)
pnpm run foks:build:frontend # production bundle into foks-ui/dist/
pnpm run test:foks-ui      # serial node:test via tsx — model, location, invariants, render
pnpm run acceptance:foks-ui # build with the mock, then drive Chromium at 1280×860
...
- **The mock stands in** only for an explicit `VITE_FOKS_MOCK=1` build or a
  ...
  ```sh
  VITE_FOKS_MOCK=1 pnpm run foks:dev:frontend
  ```

  `src/mock-bridge.ts` answers from `src/fixture.ts` with no network or timers;
  it reads only the app's versioned nonsecret first-run checkpoint so a mocked
  quit/resume walk sees completed setup facts. It is
...
whole. The design is the source of truth: change it there, then bring the
change here.
...
`document.documentElement.scrollWidth === 1280`**; PNGs land in
...
Where the shell is, what it has selected and how it is showing it are all in
the address bar, so every state reloads into itself and the acceptance run can
