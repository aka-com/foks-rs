...
const STATE_ALIASES: Readonly<Record<string, Location>> = {
  ...
  // Servers used to be a place of its own; it is a Settings section now, and
  // every name that pointed at the old page lands on that section.
 ...
 * scene layer sits above it and says what else the name implies. The lease is
...
const SCENE_ALIASES: Readonly<Record<string, Partial<Scene>>> = {
  ...
  grid: { view: 'grid' },
  // The lapsed world, standing on the store that lapses with it.
  ...
  // Alerts is only worth looking at in the world that fills it.
  alerts: { lease: 'lapsed' },
};
