...
const readFlights = new WeakMap<Bridge, Map<string, Promise<ReadItemResponse>>>();

function readOnce(bridge: Bridge, request: ItemRequest): Promise<ReadItemResponse> {
  const key = JSON.stringify([request.storeId, request.path, request.version]);
  const flights = readFlights.get(bridge) ?? new Map<string, Promise<ReadItemResponse>>();
...
function assertExactRead(request: ItemRequest, response: ReadItemResponse): void {
  if (
    response.store !== request.storeId ||
    response.path !== request.path ||
    response.version !== request.version
  ) {
    throw new Error('The local agent returned a value for a different catalog selection.');
  }
}
 ...
 */
function passwordFields(masked: string | undefined, shown: string | null): PasswordField[] {
  if (shown !== null) {
    const learned = parseFields(shown);
    if (learned.some(([field]) => field === 'password')) {
      return learned.map(([field, value]) => ({ field, value, secret: field === 'password' }));
    }
...
export interface DetailsPanelProps {
  ...
  resumeDraft?: { store: string; path: string; value: string; epoch: number } | null;
}
...
export function DetailsPanel({
  ...
  const request = useMemo<ItemRequest | null>(
    () => item ? { storeId: item.store, path: item.path, version: item.version } : null,
    [item],
  );
  ...
  useEffect(() => {
    ...
    if (item.store !== resumeDraft.store || item.path !== resumeDraft.path) return;
  ...
  useEffect(() => {
    ...
    void bridge.onDropHover(({ hovering }) => setDropHover(hovering)).then(
      (off) => { if (disposed) off(); else hoverOff = off; },
      (error) => { if (!disposed) setEditError(normalizeCommandError(error).message); },
    );
    void bridge.onDropPaths((paths) => {
      if (paths.length !== 1) {
        setReplacementPath(null);
        setEditError('Drop one file at a time. The original file was not changed.');
      } else {
      ...
      (off) => { if (disposed) off(); else pathsOff = off; },
      (error) => { if (!disposed) setEditError(normalizeCommandError(error).message); },
    );
    return () => { disposed = true; hoverOff?.(); pathsOff?.(); };
    ...
    return (
      <aside className="details" aria-label="Details">
        ...
        <div className="scroll">
          <p className="hint">Select an item to view its details, location, and access.</p>
        </div>
      </aside>
    );
  }
  ...
  const copyValue = async (): Promise<void> => {
    ...
    try {
      ...
      setMessage(`${kind === 'Password' ? 'Password' : 'Value'} copied`);
    } catch (error) {
      if (normalizeCommandError(error).code === 'agent-lost') onCommandError(error, item);
    ...
    try {
      if (fileMode) {
        const response = replacementPath
          ? await bridge.replaceDroppedFile({ ...request, sourcePath: replacementPath })
  ...
  const preview = editing && fileMode ? (
      <Inset variant="preview" className={dropHover ? 'drop-hover' : undefined}>
        <InsetRow label="Replace">
          <span className={replacementPath ? 'mono' : 'dim'}>
            {replacementPath ? replacementPath.split(/[\\/]/).at(-1) : dropHover ? 'Drop to use this file' : 'Drop a file here, or use the native picker when saving'}
          </span>
        </InsetRow>
      </Inset>
    ) : editing ? (
      ...
      <Inset variant="preview">
        {passwordFields(item.value, shownValue).map(({ field, value, secret }, index) =>
          secret ? (
            <InsetRow
              key={index}
              ...
              action={
                shownValue === null ? (
                  <>
                    <button type="button" disabled={reading} onClick={() => void show()}>
                      <Icon name="eye" />
                  ...
                  <>
                    <button type="button" onClick={() => { concealEpoch.current += 1; setRead(null); }}>
        ...
        <InsetRow
          ...
          action={
            shownValue === null ? (
              <>
                <button type="button" disabled={reading} onClick={() => void show()}>
                  <Icon name="eye" />{reading ? 'Reading…' : 'Show'}
                </button>
                <button type="button" onClick={() => void copyValue()}>
                  <Icon name="copy" />Copy
                </button>
            ...
            <Button
              ...
              onClick={() => {
                ...
                void bridge.downloadFile(request).then(
                  ({ saved }) => setMessage(saved ? `Downloaded version ${item.version}` : 'Download cancelled'),
                  (error) => {
                    if (normalizeCommandError(error).code === 'agent-lost') onCommandError(error, item);
                    else setMessage(normalizeCommandError(error).message);
                  },
        ...
        <div className="pad">
          <div className="fileglyph">
            ...
            <span>
              ...
              <span>
                {shownValue === null
                  ? (reading ? 'reading the version-bound target…' : 'target masked until read')
          ...
          <div className="row2">
            ...
            >
              {shownValue === null ? (reading ? 'Reading…' : 'Read target') : 'Open target'}
            </Button>
          </div>
        </div>
      </Inset>
    ...
    <aside className="details" aria-label={`Details for ${nameOf(item.path)}`}>
      ...
      <div className="scroll">
        {editing || displayKind !== 'Resource' ? (
          <SectionLabel>
            {editing ? 'Edit' : displayKind === 'Password' ? 'Login' : displayKind}
          </SectionLabel>
        ) : null}
        {preview}
        {editing || footnote ? <div className="pfn">{editing
          ? `Save succeeds only if this item is still version ${item.version}. If it changed, refresh it and try again.`
          : footnote}</div> : null}
        {readError ? <p className="action-error" role="alert">{readError}</p> : null}
        {editError ? <p className="action-error" role="alert">{editError}</p> : null}
      ...
      <div className="dfoot">
        {editing ? <>
          ...
          <Button variant="primary" disabled={saving} onClick={() => void saveEdit()}>{saving ? 'Saving…' : `Save version ${item.version + 1}`}</Button>
        </> : <><Button
          icon="pencil"
        ...
        <Button
          ...
          icon="trash"
          disabled={!canChange}
          title={canChange ? `Removes version ${item.version} exactly` : `Your current role does not admit the ${roleText(item.write)} write role`}
