...
import {
  queuedDeviceWork,
  runDeviceMutation,
} from '../src/screens/devices/operation-controller';
...
} from '../src/screens/devices/enrollment-workflow';
...
test('explicit card refresh discards a late result after the account changes', async () => {
  ...
  const access = {
    run: <T>(_operation: unknown, _target: unknown, task: () => Promise<T>) =>
      task(),
  } as ReturnType<typeof useWorkflowAccess>;
  let current = true;
  const pending = refreshConnectedCards(
    bridge,
    access,
    'p',
    () => current,
...
test('device removal re-reads native targets and checks access inside the queue before writing', async () => {
  const calls: string[] = [];
  const bridge = {
    listAccountDevices: async (id: string) => {
      calls.push(`list:${id}`);
      return [];
    },
  ...
  const access: ReturnType<typeof useWorkflowAccess> = {
    availability: () => ({ available: true }),
    props: () => ({ disabled: false, title: undefined }),
    require: (operation: string) => {
...
test('credential requests preserve explicit resume and account-bound recovery targets', () => {
  assert.deepEqual(
    ...
    }),
    {
      ...
      args: { profile: 'p', alias: 'key' },
    },
  );
  assert.equal(
    credentialCommand({
      action: 'recover-management',
      profile: 'p',
      alias: 'key',
      pin: '',
      other: '',
