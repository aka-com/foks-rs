...
test('domain golden records exactly reproduce the native-compatible aggregate', async () => {
  ...
  const existing = parseUniqueJson(await readFile(
    new URL('../src-tauri/wire-contract.json', import.meta.url), 'utf8',
  ));
  assert.deepEqual(aggregate, existing);
});

test('fixture assembly rejects duplicate record ownership and nested JSON keys', () => {
  assert.throws(() => mergeDomains({
    first: { record: { value: 1 } },
    second: { record: { value: 1 } },
  }), /duplicate record record/);
  assert.throws(() => parseUniqueJson('{"record":1,"record":2}'), /duplicate key record/);
  assert.throws(() => parseUniqueJson('{"record":{"value":1,"value":2}}'), /duplicate key value/);
  assert.throws(() => parseUniqueJson('{"record":[{"value":1,"value":2}]}'), /duplicate key value/);
  assert.throws(() => parseUniqueJson('{"record":1,"\\u0072ecord":2}'), /duplicate key record/);
...
test('every golden record has explicit domain and shape or semantic-assertion coverage', async () => {
  ...
  const represented = new Set(Object.values(inventory.representedShapes).flat());
  ...
  assert.ok(inventory.uncoveredPublicShapes.application.includes('AgentProcessInfo'));
  assert.ok(inventory.uncoveredPublicShapes.application.includes('MaintenanceSnapshot'));
  assert.ok(inventory.uncoveredPublicShapes.invitations.includes('InvitationRow / InvitationReply'));
  assert.ok(inventory.uncoveredPublicShapes.chat.includes('ChatAction / ChatReply'));
  assert.ok(inventory.uncoveredPublicShapes.vault.includes('CatalogFailureDto'));
});
