...
test('retryable ambiguous mutations make one attempt and require reconciliation', async () => {
  ...
  assert.equal(result.recovery, 'reconcile');
});
...
test('resumable failures never submit or resume automatically', async () => {
  ...
  assert.equal(result.outcome, 'unknown');
  assert.equal(result.recovery, 'explicit-resume');
});
...
test('definitive rejection and admission refusal remain distinct', async () => {
  for (const [error, expected] of [
    ...
    [
      { ...failure('busy'), details: { reason: 'admission-not-started' } },
      'not-started',
    ],
  ] as const) {
    let attempts = 0;
...
test('read recovery is explicit and cannot replay a retryable read automatically', async () => {
  ...
  if (result.outcome === 'read-failed')
    assert.equal(result.recovery, 'read-recovery');
  assert.deepEqual(
    await attemptRead({ kind: 'read-recovery' }, async () => 3),
    {
      outcome: 'read',
      value: 3,
    },
