...
const FIXTURE_LOCATION_NAMES = [
  'personal', 'work', 'household', 'group', 'homelab',
  'group-people', 'group-channels', 'group-files', 'party', 'federation',
  'items', 'danger', 'rekey-menu', 'invite', 'add', 'demote', 'remove', 'admit',
  'servers-server', 'servers-lapsed', 'servers-rollback', 'servers-reset',
  'servers-unprobed', 'servers-check', 'settings-macs-work',
];
const FIRST_RUN_NAMES = [
  'boot', 'who', 'local', 'address', 'no-address', 'checked', 'compare',
  'error', 'account', 'existing', 'protect', 'phrase', 'waiting', 'added',
  'local-done', 'identity-pending', 'operation-pending', 'checklist-invited',
  'checklist-own',
];
const FIXTURE_SCENE_NAMES = [
  'password', 'show', 'resource', 'file', 'link', 'conflict', 'store', 'manage',
  'party-remove', 'groups-lease', 'groups-inactive', 'group-new-document',
  'grid', 'folders', 'lease', 'inactive',
];
...
test('the facade exposes the leaf codecs and keeps decodeScene acceptance-compatible', () => {
  ...
  for (const name of [
    ...FIXTURE_LOCATION_NAMES, ...FIRST_RUN_NAMES, ...FIXTURE_SCENE_NAMES,
...
test('production never synthesizes fixture addresses or accepts bare first-run scenes', () => {
  ...
  assert.deepEqual(decodeAcceptanceScene('?state=manage').location, {
    kind: 'group-settings', ref: 'team:household', tab: 'people',
  });
});

test('production ignores reveal, demo, lease and URL attempts to enable fixtures', () => {
  for (const name of ['show', 'lease', 'groups-lease', 'alerts', 'password', 'all']) {
    const query = search(name, {
      lease: 'lapsed', reveal: 'true', demo: 'password', fixtures: 'true',
      fixture: 'true', acceptance: 'true',
    });
  ...
  assert.equal(decodeAcceptanceScene('?state=all&lease=lapsed').lease, 'lapsed');
...
test('public legacy destinations are available without fixture capabilities', () => {
  for (const [name, location] of Object.entries(PUBLIC_ALIASES)) {
    ...
    assert.deepEqual(decodeProductionScene(query), {
      ...INITIAL_SCENE, location,
    }, name);
  }
  assert.deepEqual(decodeProductionLocation(search('settings-enrol', {
    store: ACCOUNT, device: 'yubi:primary key',
  })), {
    kind: 'devices', section: 'keys', store: ACCOUNT, device: 'yubi:primary key',
  });
  assert.deepEqual(decodeProductionLocation(search('servers-list', {
    store: ACCOUNT, profile: 'remote',
  })), {
    kind: 'settings', section: 'servers', store: ACCOUNT, profile: 'remote',
  });
});
...
test('canonical production routes round-trip opaque store references and explicit first-run state', () => {
  const locations: Location[] = [
    ...
    { kind: 'devices', store: ACCOUNT, section: 'keys', device: 'yubi:primary key' },
  ...
  for (const location of locations) {
    const query = new URL(locationHref('https://desktop.example/', location)).search;
    assert.deepEqual(decodeProductionLocation(query), location);
    assert.deepEqual(decodeLocation(query), location);
  }
  assert.deepEqual(decodeProductionLocation(search('team-chat', {
    store: TEAM, channel: CHANNEL,
  })), { kind: 'chat', ref: TEAM, channel: CHANNEL });
  assert.equal(decodeProductionLocation('?state=team-chat'), null);
  assert.equal(decodeProductionLocation(search('chat', {
    store: TEAM, channel: 'invalid',
  })), null);
  assert.deepEqual(decodeProductionLocation(search('chat', {
    channel: CHANNEL,
  })), { kind: 'chat' });
});
...
test('folded and retired settings sections remain production deep links', () => {
  ...
  for (const [section, location] of Object.entries(sections)) {
    const query = search('settings', {
      section, store: ACCOUNT, profile: 'ignored', account: 'ignored',
    });
    assert.deepEqual(decodeProductionLocation(query), {
      ...location, store: ACCOUNT,
    }, section);
...
test('production preserves explicit selection, folders, filters and sorting without scene effects', () => {
  const query = search('show', {
    sel: `${TEAM}|/documents/key`, folder: '/documents', closed: '/a,/b',
    view: 'folders', kind: 'Resource', sort: 'group', lease: 'lapsed',
    reveal: 'true', demo: 'password',
  });
  const scene = decodeProductionScene(query);
  assert.deepEqual(scene, {
    ...INITIAL_SCENE,
    selection: { store: TEAM, path: '/documents/key' },
    folder: '/documents', closedFolders: ['/a', '/b'], kind: 'Document', sort: 'group',
  });
  assert.deepEqual(
    decodeProductionScene(new URL(sceneHref('https://desktop.example/', scene)).search),
    scene,
  );
  ...
  assert.deepEqual(decodeProductionScene('?state=all&sel=bad&kind=bad&sort=bad'), INITIAL_SCENE);
});
