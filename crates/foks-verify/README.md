...
`foks-verify` turns an untrusted FOKS v0.1.9 probe response into a
sealed `VerifiedHostSnapshot` that `foks-client-db` can commit atomically.
...
The verifier performs no network or database I/O. Discovery/RPC code should
download bytes, call `verify_public_host`, and only then commit the returned
snapshot.
