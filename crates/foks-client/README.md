...
write token to a single versioned native Keychain or Secret Service manifest.
A native database ID is claimed by one profile and locked within its client
root during checked operations, rejecting whole-database rollback,
substitution, and equal-revision copies. The private-file backend does not
provide this copied-state or external rollback protection.
