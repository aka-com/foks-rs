...
V1 is implemented and tested as a standalone FOKS subsystem. It must not change
or depend on any AKA crate. Permitted implementation paths are:
...
.gitattributes                    # only FOKS fixture attributes
Cargo.toml
Cargo.lock
rust-project.json
MODULE.bazel                      # only for an unavoidable dependency annotation
MODULE.bazel.lock                 # only when regenerated by that annotation
```

The root manifest and lock files are shared build metadata, not an authorization
to modify another crate. V1 must not change files under:
...
No `foks-*` production or test crate may directly or transitively depend on an
`aka-*` package. A dedicated boundary check enforces the dependency graph and
permitted source paths.
...
5. Do the changed paths, Cargo graph, test roots, and listen addresses remain
   inside the standalone FOKS isolation boundary?
