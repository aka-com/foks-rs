...
#[test]
pub(crate) fn migration_two_accounts_owner_proofs_late_link_and_erased_token_authorization_binding()
{
    ...
    let status = alice
        ...
        .unwrap();
    assert_eq!(status.committed_authorization_binding, Some(commitment));
    assert!(!status.access_available);
