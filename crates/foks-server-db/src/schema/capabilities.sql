...
CREATE TABLE team_view_challenges (
    ...
    member_id BLOB NOT NULL REFERENCES users(uid) ON DELETE CASCADE,
...
CREATE TABLE team_admin_tokens (
    ...
    team_id BLOB NOT NULL REFERENCES teams(team_id) ON DELETE CASCADE,
    member_id BLOB NOT NULL REFERENCES users(uid) ON DELETE CASCADE,
    member_source_role_type INTEGER NOT NULL CHECK (member_source_role_type BETWEEN 1 AND 3),
    member_source_visibility INTEGER NOT NULL,
    ...
    ptk_verify_key BLOB NOT NULL CHECK (length(ptk_verify_key) = 33),
    expires_at INTEGER NOT NULL CHECK (expires_at >= 0),
    activation_hash BLOB CHECK (activation_hash IS NULL OR length(activation_hash) = 32)
) STRICT;

CREATE INDEX team_admin_tokens_scope
    ON team_admin_tokens(team_id, member_id, expires_at);
