...
binary is `apps/desktop/src-tauri`'s `foks-desktop`. Agent calls in that app run on blocking
workers, never the webview thread. The scripted backend accepts secrets only
through private files, never process arguments.

For the first-run onboarding transcript, invoke the backend sequentially per line;
the process may exit between lines. `pending` provides the authenticated checkpoint
read used to determine whether to execute the resume command. Secrets are passed via
mode-0600 files rather than arguments or checkpoint fields:
...
every mutation boundary. The end-to-end process-exit and resume verification requires a runnable
integration test harness against a live agent instance.
...
Security key transcript commands can similarly be executed across independent
backend processes. PINs, PUKs, passphrases, and invites are read from private files;
the physical card serial is selected from `yubi-cards`, while the provisioning device
serial is generated by the client runtime:

```text
foks-desktop-backend --agent-socket SOCKET yubi-cards work
foks-desktop-backend --agent-socket SOCKET yubi-accounts work
foks-desktop-backend --agent-socket SOCKET yubi-create work work_key --username satoshi --device-name "YubiKey 42" --card-serial 42 --pin-file PIN --puk-file PUK
foks-desktop-backend --agent-socket SOCKET pending work
...
reading, and a generated positive provisioning serial. Like the initial onboarding
transcript test, this does not execute against live hardware and does not verify
physical security key interaction in CI.
