...
/// never backs the job off into a retry storm.
...
impl CheckedProfileSession<'_> {
    ...
    /// failing, so the job neither backs off nor silently disappears.
    ...
    fn prepare_unlocked_yubi_responders(
        ...
        for _ in 0..=local.len() {
            for actor in &local {
                ...
                // hop's remote; it must not repeat what just ran here.
                cascade
                    .yubi_refreshed
    ...
    /// security-sync flow, where the caller has one unlocked device and wants
...
/// A host that is refusing calls for rate-limiting reasons will refuse every
/// credential identically, so candidate enumeration must stop rather than
/// spend the remaining budget and report a misleading "no credential" error.
